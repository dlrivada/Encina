...
namespace Encina.ADO.MySQL.Tests.Outbox;
...
public sealed class OutboxStoreADOPropertyTests : IClassFixture<MySqlFixture>
{
    private readonly MySqlFixture _fixture;

    public OutboxStoreADOPropertyTests(MySqlFixture fixture)
    {
        _fixture = fixture;
        ...
        if (pending.Count > 1)
        {
            for (int i = 0; i < pending.Count - 1; i++)
            {
                Assert.True(pending[i].CreatedAtUtc <= pending[i + 1].CreatedAtUtc,
                    $"Message at index {i} has DATETIME(6) {pending[i].CreatedAtUtc}, " +
                    $"which is after message at index {i + 1} with DATETIME(6) {pending[i + 1].CreatedAtUtc}");
            }
        }
    }
