...
namespace Encina.Dapper.PostgreSQL.Tests.Inbox;

/// <summary>
/// Contract tests for <see cref="InboxStoreDapper"/>.
/// Verifies compliance with <see cref="IInboxStore"/> interface contract.
/// Uses real PostgreSQL database via Testcontainers.
/// </summary>
public sealed class InboxStoreDapperContractTests : IClassFixture<PostgreSqlFixture>
{
    ...
    public InboxStoreDapperContractTests(PostgreSqlFixture database)
    {
        _database = database;
    ...
    [Fact]
    public void InboxStoreDapper_ImplementsIInboxStore()
    {
        // Assert
    ...
    [Fact]
    public void InboxStoreDapper_CanBeUsedThroughInterface()
    {
        // Arrange
        IInboxStore interfaceReference = new InboxStoreDapper(_database.CreateConnection());
