...
public sealed class StreamPipelineBehaviorTests
{
    ...
    public async Task StreamBehavior_ShouldWrapHandlerExecution()
    {
        ...
        loggingBehavior.Logs.ShouldContain("Stream started");
        loggingBehavior.Logs.ShouldContain("Item 1");
        loggingBehavior.Logs.ShouldContain("Item 2");
        loggingBehavior.Logs.ShouldContain("Item 3");
        loggingBehavior.Logs.ShouldContain("Stream completed: 3 items");
    }
    ...
    public async Task StreamBehavior_Transform_ShouldModifyItems()
    {
        ...
        values.ShouldBe([10, 20, 30]); // Original: 1, 2, 3 → Transformed: 10, 20, 30
    }
    ...
    public async Task StreamBehavior_WithContextPropagation_ShouldAccessContext()
    {
        ...
        // Assert
        contextCapture.ShouldContain(c => c.StartsWith("CorrelationId:", StringComparison.Ordinal));
    }
    ...
    public void AssemblyScanning_ShouldNotDuplicateBehaviors_WhenManuallyRegisteredFirst()
    {
        ...
        countAfter.ShouldBe(1, "scanner should not add duplicate when implementation type already registered");
    }
    ...
    public void AssemblyScanning_CannotDeduplicateFactoryRegistrations()
    {
        ...
        countBefore.ShouldBe(0, "factory registrations have null implementation type");
        // Scanner finds the public class and adds it
        countAfter.ShouldBe(1, "scanner adds the type-based registration");

        // Total for this service type includes the factory + scanned registration
        var totalRegistrations = services.Count(s =>
            s.ServiceType == typeof(IStreamPipelineBehavior<StreamNumbersQuery, int>));
        totalRegistrations.ShouldBeGreaterThan(1, "factory registration is not deduplicated");
    }
