...
/// <summary>
/// Integration tests for <see cref="AuditStoreDapper"/> with PostgreSQL.
/// </summary>
[Trait("Category", "Integration")]
[Trait("Database", "PostgreSQL")]
[Collection("Dapper-PostgreSQL")]
public class AuditStoreDapperPostgreSqlIntegrationTests : IAsyncLifetime
{
    private readonly PostgreSqlFixture _fixture;
    private IDbConnection _connection = null!;
    private AuditStoreDapper _store = null!;

    public AuditStoreDapperPostgreSqlIntegrationTests(PostgreSqlFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    private async Task ClearDataAsync()
    {
        if (_connection is NpgsqlConnection npgsqlConnection)
        {
            await using var command = new NpgsqlCommand("""DELETE FROM "SecurityAuditEntries";""", npgsqlConnection);
            await command.ExecuteNonQueryAsync();
        }
    }
    ...
    public async Task RecordAsync_EntryWithAllFields_ShouldPersistAllFields()
    {
        ...
        {
            Id = Guid.NewGuid(),
            CorrelationId = "test-correlation-dapper-pg",
            UserId = "user-456",
            TenantId = "tenant-789",
            Action = "Update",
            EntityType = "Product",
        ...
        var queryResult = await _store.GetByCorrelationIdAsync("test-correlation-dapper-pg");
    ...
    {
        await ClearDataAsync();
        var correlationId = "corr-dapper-pg";
        var now = DateTimeOffset.UtcNow;
