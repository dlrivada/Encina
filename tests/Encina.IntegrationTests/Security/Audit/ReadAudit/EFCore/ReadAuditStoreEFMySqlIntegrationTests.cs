...
/// <summary>
/// Integration tests for <see cref="ReadAuditStoreEF"/> with MySQL.
/// </summary>
[Trait("Category", "Integration")]
[Trait("Database", "MySQL")]
[Collection("EFCore-MySQL")]
public class ReadAuditStoreEFMySqlIntegrationTests : IAsyncLifetime
{
    private readonly EFCoreMySqlFixture _fixture;

    public ReadAuditStoreEFMySqlIntegrationTests(EFCoreMySqlFixture fixture) => _fixture = fixture;
    ...
    {
        var store = CreateStore();
        await store.LogReadAsync(CreateTestEntry(entityId: "PAT-EF-MY"));
        var result = await store.GetAccessHistoryAsync("Patient", "PAT-EF-MY");
        result.IsRight.ShouldBeTrue();
    ...
    public async Task GetUserAccessHistoryAsync_ShouldFilter()
    {
        ...
        await store.LogReadAsync(CreateTestEntry(userId: "ef-my-ruser", accessedAtUtc: now));
        var result = await store.GetUserAccessHistoryAsync("ef-my-ruser", now.AddHours(-1), now.AddHours(1));
        result.IsRight.ShouldBeTrue();
        result.IfRight(entries => entries.ShouldNotBeEmpty());
    }
    ...
    public async Task QueryAsync_WithPagination_ShouldWork()
    {
        var store = CreateStore();
        for (var i = 0; i < 10; i++)
            await store.LogReadAsync(CreateTestEntry(entityId: $"PAT-MY-{i}"));
    ...
    {
        var store = CreateStore();
        await store.LogReadAsync(CreateTestEntry(entityId: "OLD-MY", accessedAtUtc: DateTimeOffset.UtcNow.AddDays(-100)));
        await store.LogReadAsync(CreateTestEntry(entityId: "RECENT-MY", accessedAtUtc: DateTimeOffset.UtcNow));
