...
/// <summary>
/// Integration tests for <see cref="ReadAuditStoreADO"/> with MySQL.
/// </summary>
[Trait("Category", "Integration")]
[Trait("Database", "MySQL")]
[Collection("ADO-MySQL")]
public class ReadAuditStoreADOMySqlIntegrationTests : IAsyncLifetime
{
    private readonly MySqlFixture _fixture;
    private IDbConnection _connection = null!;
    private ReadAuditStoreADO _store = null!;

    public ReadAuditStoreADOMySqlIntegrationTests(MySqlFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        if (_connection is MySqlConnection mysqlConnection)
            await CreateReadAuditSchemaAsync(mysqlConnection);
    ...
    private static async Task CreateReadAuditSchemaAsync(MySqlConnection connection)
    {
        const string sql = """
    ...
    public async Task GetUserAccessHistoryAsync_WithinDateRange_ShouldReturnEntries()
    {
        ...
        await _store.LogReadAsync(CreateTestEntry(userId: "user-my", accessedAtUtc: now));
        var result = await _store.GetUserAccessHistoryAsync("user-my", now.AddHours(-1), now.AddHours(1));
        result.IsRight.ShouldBeTrue();
        result.IfRight(entries => entries.ShouldHaveSingleItem());
    }
