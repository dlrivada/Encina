...
/// <summary>
/// Integration tests for <see cref="AuditStoreEF"/> with MySQL.
/// </summary>
[Trait("Category", "Integration")]
[Trait("Database", "MySQL")]
[Collection("EFCore-MySQL")]
public class AuditStoreEFMySqlIntegrationTests : IAsyncLifetime
{
    private readonly EFCoreMySqlFixture _fixture;

    public AuditStoreEFMySqlIntegrationTests(EFCoreMySqlFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task RecordAsync_EntryWithAllFields_ShouldPersistAllFields()
    {
        ...
        {
            Id = Guid.NewGuid(),
            CorrelationId = "test-correlation-ef-mysql",
            UserId = "user-456",
            TenantId = "tenant-789",
            Action = "Update",
            EntityType = "Product",
        ...
        var queryResult = await store.GetByCorrelationIdAsync("test-correlation-ef-mysql");
        queryResult.IsRight.ShouldBeTrue();
    ...
    {
        var store = CreateStore();
        await store.RecordAsync(CreateTestEntry(entityType: "Order", entityId: "order-my-1"));
        await store.RecordAsync(CreateTestEntry(entityType: "Order", entityId: "order-my-2"));

        var result = await store.GetByEntityAsync("Order", "order-my-1");
        result.IsRight.ShouldBeTrue();
        result.IfRight(entries =>
        {
            entries.ShouldHaveSingleItem();
            entries[0].EntityId.ShouldBe("order-my-1");
        });
    }
    ...
    {
        var store = CreateStore();
        await store.RecordAsync(CreateTestEntry(userId: "ef-my-u1"));
        await store.RecordAsync(CreateTestEntry(userId: "ef-my-u1"));
        await store.RecordAsync(CreateTestEntry(userId: "ef-my-u2"));

        var result = await store.GetByUserAsync("ef-my-u1", null, null);
        result.IsRight.ShouldBeTrue();
    ...
    {
        var store = CreateStore();
        var oldEntry = CreateTestEntry(userId: "ef-my-user", timestampUtc: DateTime.UtcNow.AddDays(-10));
        var recentEntry = CreateTestEntry(userId: "ef-my-user", timestampUtc: DateTime.UtcNow.AddDays(-1));
        await store.RecordAsync(oldEntry);
        await store.RecordAsync(recentEntry);

        var result = await store.GetByUserAsync("ef-my-user", DateTime.UtcNow.AddDays(-5), DateTime.UtcNow);
        result.IsRight.ShouldBeTrue();
        result.IfRight(entries =>
        {
            entries.ShouldHaveSingleItem();
    ...
    {
        var store = CreateStore();
        var correlationId = "corr-ef-mysql";
        var now = DateTimeOffset.UtcNow;
    ...
    {
        var store = CreateStore();
        var entry2 = CreateTestEntry(userId: "ef-my-filter", outcome: AuditOutcome.Failure);
        await store.RecordAsync(CreateTestEntry(userId: "ef-my-filter", outcome: AuditOutcome.Success));
        await store.RecordAsync(entry2);

        var query = new AuditQuery { UserId = "ef-my-filter", Outcome = AuditOutcome.Failure };
    ...
    {
        var store = CreateStore();
        await store.RecordAsync(CreateTestEntry(entityType: "EFMYOrder"));
        await store.RecordAsync(CreateTestEntry(entityType: "EFMYProduct"));
        await store.RecordAsync(CreateTestEntry(entityType: "EFMYOrder"));

        var query = new AuditQuery { EntityType = "EFMYOrder" };
        var result = await store.QueryAsync(query);
        result.IsRight.ShouldBeTrue();
