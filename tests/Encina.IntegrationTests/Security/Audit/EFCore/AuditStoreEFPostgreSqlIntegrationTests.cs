...
/// <summary>
/// Integration tests for <see cref="AuditStoreEF"/> with PostgreSQL.
/// </summary>
[Trait("Category", "Integration")]
[Trait("Database", "PostgreSQL")]
[Collection("EFCore-PostgreSQL")]
public class AuditStoreEFPostgreSqlIntegrationTests : IAsyncLifetime
{
    private readonly EFCorePostgreSqlFixture _fixture;

    public AuditStoreEFPostgreSqlIntegrationTests(EFCorePostgreSqlFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task RecordAsync_EntryWithAllFields_ShouldPersistAllFields()
    {
        ...
        {
            Id = Guid.NewGuid(),
            CorrelationId = "test-correlation-ef-pg",
            UserId = "user-456",
            TenantId = "tenant-789",
            Action = "Update",
            EntityType = "Product",
        ...
        var queryResult = await store.GetByCorrelationIdAsync("test-correlation-ef-pg");
        queryResult.IsRight.ShouldBeTrue();
    ...
    {
        var store = CreateStore();
        await store.RecordAsync(CreateTestEntry(entityType: "Order", entityId: "order-pg-1"));
        await store.RecordAsync(CreateTestEntry(entityType: "Order", entityId: "order-pg-2"));

        var result = await store.GetByEntityAsync("Order", "order-pg-1");
        result.IsRight.ShouldBeTrue();
        result.IfRight(entries =>
        {
            entries.ShouldHaveSingleItem();
            entries[0].EntityId.ShouldBe("order-pg-1");
        });
    }
    ...
    {
        var store = CreateStore();
        await store.RecordAsync(CreateTestEntry(userId: "ef-pg-u1"));
        await store.RecordAsync(CreateTestEntry(userId: "ef-pg-u1"));
        await store.RecordAsync(CreateTestEntry(userId: "ef-pg-u2"));

        var result = await store.GetByUserAsync("ef-pg-u1", null, null);
        result.IsRight.ShouldBeTrue();
    ...
    {
        var store = CreateStore();
        var oldEntry = CreateTestEntry(userId: "ef-pg-user", timestampUtc: DateTime.UtcNow.AddDays(-10));
        var recentEntry = CreateTestEntry(userId: "ef-pg-user", timestampUtc: DateTime.UtcNow.AddDays(-1));
        await store.RecordAsync(oldEntry);
        await store.RecordAsync(recentEntry);

        var result = await store.GetByUserAsync("ef-pg-user", DateTime.UtcNow.AddDays(-5), DateTime.UtcNow);
        result.IsRight.ShouldBeTrue();
        result.IfRight(entries =>
        {
            entries.ShouldHaveSingleItem();
    ...
    {
        var store = CreateStore();
        var entry2 = CreateTestEntry(userId: "ef-pg-filter", outcome: AuditOutcome.Failure);
        await store.RecordAsync(CreateTestEntry(userId: "ef-pg-filter", outcome: AuditOutcome.Success));
        await store.RecordAsync(entry2);

        var query = new AuditQuery { UserId = "ef-pg-filter", Outcome = AuditOutcome.Failure };
    ...
    {
        var store = CreateStore();
        await store.RecordAsync(CreateTestEntry(entityType: "EFPGOrder"));
        await store.RecordAsync(CreateTestEntry(entityType: "EFPGProduct"));
        await store.RecordAsync(CreateTestEntry(entityType: "EFPGOrder"));

        var query = new AuditQuery { EntityType = "EFPGOrder" };
        var result = await store.QueryAsync(query);
        result.IsRight.ShouldBeTrue();
