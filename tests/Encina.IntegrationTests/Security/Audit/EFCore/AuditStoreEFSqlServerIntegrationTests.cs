...
/// <summary>
/// Integration tests for <see cref="AuditStoreEF"/> with SQL Server.
/// </summary>
[Trait("Category", "Integration")]
[Trait("Database", "SqlServer")]
[Collection("EFCore-SqlServer")]
public class AuditStoreEFSqlServerIntegrationTests : IAsyncLifetime
{
    private readonly EFCoreSqlServerFixture _fixture;

    public AuditStoreEFSqlServerIntegrationTests(EFCoreSqlServerFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task QueryAsync_WithFilters_ShouldReturnMatchingEntries()
    {
        ...
        var query = new AuditQuery { UserId = "ef-ss-filter", Outcome = AuditOutcome.Failure };
    ...
    public async Task QueryAsync_WithEntityTypeFilter_ShouldReturnMatchingEntries()
    {
        ...
        var query = new AuditQuery { EntityType = "EFSSOrder" };
        var result = await store.QueryAsync(query);
        result.IsRight.ShouldBeTrue();
