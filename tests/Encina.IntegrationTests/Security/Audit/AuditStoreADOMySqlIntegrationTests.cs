...
/// <summary>
/// Integration tests for <see cref="AuditStoreADO"/> with MySQL.
/// </summary>
[Trait("Category", "Integration")]
[Trait("Database", "MySQL")]
[Collection("ADO-MySQL")]
public class AuditStoreADOMySqlIntegrationTests : IAsyncLifetime
{
    private readonly MySqlFixture _fixture;
    private IDbConnection _connection = null!;
    private AuditStoreADO _store = null!;

    public AuditStoreADOMySqlIntegrationTests(MySqlFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task RecordAsync_EntryWithAllFields_ShouldPersistAllFields()
    {
        ...
        {
            Id = Guid.NewGuid(),
            CorrelationId = "test-correlation-mysql",
            UserId = "user-456",
            TenantId = "tenant-789",
            Action = "Update",
            EntityType = "Product",
        ...
        var queryResult = await _store.GetByCorrelationIdAsync("test-correlation-mysql");
        queryResult.IsRight.ShouldBeTrue();
        queryResult.IfRight(entries =>
        {
            entries.ShouldHaveSingleItem();
    ...
    {
        await ClearDataAsync();
        var correlationId = "correlation-mysql";
        var now = DateTimeOffset.UtcNow;
    ...
    public async Task QueryAsync_WithPagination_ShouldReturnPagedResults()
    {
        ...
        result.IfRight(p =>
        {
            p.TotalCount.ShouldBe(25);
            p.Items.Count.ShouldBe(10);
            p.TotalPages.ShouldBe(3);
        });
    }
    ...
    public async Task QueryAsync_WithFilters_ShouldReturnMatchingEntries()
    {
        ...
        await _store.RecordAsync(CreateTestEntry(userId: "user-1", action: "Create", outcome: AuditOutcome.Success));
        await _store.RecordAsync(entry2);
        await _store.RecordAsync(CreateTestEntry(userId: "user-2", action: "Create", outcome: AuditOutcome.Success));
