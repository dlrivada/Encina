using System.Data;
using Encina.ADO.SqlServer.ABAC;
...
/// <summary>
/// Integration tests for <see cref="PolicyStoreADO"/> with SQL Server.
/// Tests full CRUD and round-trip persistence of XACML 3.0 policy sets and policies.
/// </summary>
[Trait("Category", "Integration")]
[Trait("Database", "SqlServer")]
[Collection("ADO-SqlServer")]
public sealed class PolicyStoreADOSqlServerIntegrationTests : IAsyncLifetime
{
    private readonly SqlServerFixture _fixture;
    private PolicyStoreADO _store = null!;

    public PolicyStoreADOSqlServerIntegrationTests(SqlServerFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    {
        // Arrange
        var policySet = CreateMinimalPolicySet("ps-ado-ss-1");
        ...
        var getResult = await _store.GetPolicySetAsync("ps-ado-ss-1");
    ...
    {
        // Arrange
        var original = CreateMinimalPolicySet("ps-ado-ss-upsert");
        ...
        var getResult = await _store.GetPolicySetAsync("ps-ado-ss-upsert");
        getResult.IsRight.ShouldBeTrue();
    ...
    {
        // Arrange
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-ado-ss-all-1"));
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-ado-ss-all-2"));
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-ado-ss-all-3"));
    ...
    {
        // Act
        var result = await _store.GetPolicySetAsync("non-existent-ado-ss");
    ...
    {
        // Arrange
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-ado-ss-delete"));

        // Act
        var result = await _store.DeletePolicySetAsync("ps-ado-ss-delete");

        // Assert
        result.IsRight.ShouldBeTrue();

        var getResult = await _store.GetPolicySetAsync("ps-ado-ss-delete");
        getResult.IsRight.ShouldBeTrue();
    ...
    {
        // Arrange
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-ado-ss-exists"));

        // Act
        var result = await _store.ExistsPolicySetAsync("ps-ado-ss-exists");
    ...
    {
        // Act
        var result = await _store.ExistsPolicySetAsync("ps-ado-ss-not-exists");
    ...
    {
        // Arrange
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-ado-ss-cnt-1"));
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-ado-ss-cnt-2"));
    ...
    {
        // Arrange
        var policy = CreateMinimalPolicy("p-ado-ss-1");
        ...
        var getResult = await _store.GetPolicyAsync("p-ado-ss-1");
        getResult.IsRight.ShouldBeTrue();
    ...
    {
        // Arrange
        var original = CreateMinimalPolicy("p-ado-ss-upsert");
        ...
        var getResult = await _store.GetPolicyAsync("p-ado-ss-upsert");
        getResult.IsRight.ShouldBeTrue();
    ...
    {
        // Arrange
        await _store.SavePolicyAsync(CreateMinimalPolicy("p-ado-ss-all-1"));
        await _store.SavePolicyAsync(CreateMinimalPolicy("p-ado-ss-all-2"));
    ...
    {
        // Act
        var result = await _store.GetPolicyAsync("non-existent-ado-ss-p");
    ...
    {
        // Arrange
        await _store.SavePolicyAsync(CreateMinimalPolicy("p-ado-ss-delete"));

        // Act
        var result = await _store.DeletePolicyAsync("p-ado-ss-delete");

        // Assert
        result.IsRight.ShouldBeTrue();

        var getResult = await _store.GetPolicyAsync("p-ado-ss-delete");
        getResult.IsRight.ShouldBeTrue();
        getResult.IfRight(opt => opt.IsNone.ShouldBeTrue());
    }
    ...
    {
        // Arrange
        await _store.SavePolicyAsync(CreateMinimalPolicy("p-ado-ss-exists"));

        // Act
        var result = await _store.ExistsPolicyAsync("p-ado-ss-exists");
    ...
    {
        // Act
        var result = await _store.ExistsPolicyAsync("p-ado-ss-not-exists");
    ...
    public async Task PolicySet_RoundTrip_PreservesAllProperties()
    {
        ...
        var result = await _store.GetPolicySetAsync("ps-ado-ss-rt-1");

        // Assert
        result.IsRight.ShouldBeTrue();
        result.IfRight(opt => opt.IfSome(retrieved =>
        {
            retrieved.Id.ShouldBe("ps-ado-ss-rt-1");
            ...
            retrievedPolicy.Id.ShouldBe("p-ado-ss-rt-1");
