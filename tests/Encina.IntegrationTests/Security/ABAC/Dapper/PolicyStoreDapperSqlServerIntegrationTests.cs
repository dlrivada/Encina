using System.Data;
using Encina.Dapper.SqlServer.ABAC;
...
/// <summary>
/// Integration tests for <see cref="PolicyStoreDapper"/> with SQL Server.
/// Tests full CRUD and round-trip persistence of XACML 3.0 policy sets and policies.
/// </summary>
[Trait("Category", "Integration")]
[Trait("Database", "SqlServer")]
[Collection("Dapper-SqlServer")]
public sealed class PolicyStoreDapperSqlServerIntegrationTests : IAsyncLifetime
{
    private readonly SqlServerFixture _fixture;
    private PolicyStoreDapper _store = null!;

    public PolicyStoreDapperSqlServerIntegrationTests(SqlServerFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    {
        // Arrange
        var policySet = CreateMinimalPolicySet("ps-dap-ss-1");
        ...
        var getResult = await _store.GetPolicySetAsync("ps-dap-ss-1");
    ...
    {
        // Arrange
        var original = CreateMinimalPolicySet("ps-dap-ss-upsert");
        ...
        var getResult = await _store.GetPolicySetAsync("ps-dap-ss-upsert");
        getResult.IsRight.ShouldBeTrue();
    ...
    {
        // Arrange
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-ss-all-1"));
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-ss-all-2"));
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-ss-all-3"));
    ...
    {
        // Act
        var result = await _store.GetPolicySetAsync("non-existent-dap-ss");
    ...
    {
        // Arrange
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-ss-delete"));

        // Act
        var result = await _store.DeletePolicySetAsync("ps-dap-ss-delete");

        // Assert
        result.IsRight.ShouldBeTrue();

        var getResult = await _store.GetPolicySetAsync("ps-dap-ss-delete");
        getResult.IsRight.ShouldBeTrue();
    ...
    {
        // Arrange
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-ss-exists"));

        // Act
        var result = await _store.ExistsPolicySetAsync("ps-dap-ss-exists");
    ...
    {
        // Act
        var result = await _store.ExistsPolicySetAsync("ps-dap-ss-not-exists");
    ...
    {
        // Arrange
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-ss-cnt-1"));
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-ss-cnt-2"));
    ...
    {
        // Arrange
        var policy = CreateMinimalPolicy("p-dap-ss-1");
        ...
        var getResult = await _store.GetPolicyAsync("p-dap-ss-1");
        getResult.IsRight.ShouldBeTrue();
    ...
    {
        // Arrange
        var original = CreateMinimalPolicy("p-dap-ss-upsert");
        ...
        var getResult = await _store.GetPolicyAsync("p-dap-ss-upsert");
        getResult.IsRight.ShouldBeTrue();
    ...
    {
        // Arrange
        await _store.SavePolicyAsync(CreateMinimalPolicy("p-dap-ss-all-1"));
        await _store.SavePolicyAsync(CreateMinimalPolicy("p-dap-ss-all-2"));
    ...
    {
        // Act
        var result = await _store.GetPolicyAsync("non-existent-dap-ss-p");
    ...
    {
        // Arrange
        await _store.SavePolicyAsync(CreateMinimalPolicy("p-dap-ss-delete"));

        // Act
        var result = await _store.DeletePolicyAsync("p-dap-ss-delete");

        // Assert
        result.IsRight.ShouldBeTrue();

        var getResult = await _store.GetPolicyAsync("p-dap-ss-delete");
        getResult.IsRight.ShouldBeTrue();
        getResult.IfRight(opt => opt.IsNone.ShouldBeTrue());
    }
    ...
    {
        // Arrange
        await _store.SavePolicyAsync(CreateMinimalPolicy("p-dap-ss-exists"));

        // Act
        var result = await _store.ExistsPolicyAsync("p-dap-ss-exists");
    ...
    {
        // Act
        var result = await _store.ExistsPolicyAsync("p-dap-ss-not-exists");
    ...
    public async Task PolicySet_RoundTrip_PreservesNestedPolicies()
    {
        ...
        var result = await _store.GetPolicySetAsync("ps-dap-ss-rt");

        // Assert
        result.IsRight.ShouldBeTrue();
        result.IfRight(opt => opt.IfSome(retrieved =>
        {
            retrieved.Id.ShouldBe("ps-dap-ss-rt");
    ...
    public async Task Policy_RoundTrip_WithTargetExpression()
    {
        ...
        var policy = new Policy
        {
            ...
            Rules = [new Rule { Id = "r-dap-ss-tgt", Effect = Effect.Permit, Obligations = [], Advice = [] }],
            Obligations = [],
            Advice = [],
            VariableDefinitions = []
        };
        ...
        var result = await _store.GetPolicyAsync("p-dap-ss-target-rt");
