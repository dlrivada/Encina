...
/// <summary>
/// Integration tests for <see cref="PolicyStoreDapper"/> with MySQL.
/// Tests full CRUD and round-trip persistence of XACML 3.0 policy sets and policies.
/// </summary>
[Trait("Category", "Integration")]
[Trait("Database", "MySQL")]
[Collection("Dapper-MySQL")]
public sealed class PolicyStoreDapperMySqlIntegrationTests : IAsyncLifetime
{
    private readonly MySqlFixture _fixture;
    private PolicyStoreDapper _store = null!;

    public PolicyStoreDapperMySqlIntegrationTests(MySqlFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    {
        // Arrange
        var policySet = CreateMinimalPolicySet("ps-dap-my-1");
        ...
        var getResult = await _store.GetPolicySetAsync("ps-dap-my-1");
    ...
    {
        // Arrange
        var original = CreateMinimalPolicySet("ps-dap-my-upsert");
        ...
        var getResult = await _store.GetPolicySetAsync("ps-dap-my-upsert");
        getResult.IsRight.ShouldBeTrue();
    ...
    {
        // Arrange
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-my-all-1"));
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-my-all-2"));
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-my-all-3"));
    ...
    {
        // Act
        var result = await _store.GetPolicySetAsync("non-existent-dap-my");
    ...
    {
        // Arrange
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-my-delete"));

        // Act
        var result = await _store.DeletePolicySetAsync("ps-dap-my-delete");

        // Assert
        result.IsRight.ShouldBeTrue();

        var getResult = await _store.GetPolicySetAsync("ps-dap-my-delete");
        getResult.IsRight.ShouldBeTrue();
    ...
    {
        // Arrange
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-my-exists"));

        // Act
        var result = await _store.ExistsPolicySetAsync("ps-dap-my-exists");
    ...
    {
        // Act
        var result = await _store.ExistsPolicySetAsync("ps-dap-my-not-exists");
    ...
    {
        // Arrange
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-my-cnt-1"));
        await _store.SavePolicySetAsync(CreateMinimalPolicySet("ps-dap-my-cnt-2"));
    ...
    {
        // Arrange
        var policy = CreateMinimalPolicy("p-dap-my-1");
        ...
        var getResult = await _store.GetPolicyAsync("p-dap-my-1");
        getResult.IsRight.ShouldBeTrue();
    ...
    {
        // Arrange
        var original = CreateMinimalPolicy("p-dap-my-upsert");
        ...
        var getResult = await _store.GetPolicyAsync("p-dap-my-upsert");
        getResult.IsRight.ShouldBeTrue();
    ...
    {
        // Arrange
        await _store.SavePolicyAsync(CreateMinimalPolicy("p-dap-my-all-1"));
        await _store.SavePolicyAsync(CreateMinimalPolicy("p-dap-my-all-2"));
    ...
    {
        // Act
        var result = await _store.GetPolicyAsync("non-existent-dap-my-p");
    ...
    {
        // Arrange
        await _store.SavePolicyAsync(CreateMinimalPolicy("p-dap-my-delete"));

        // Act
        var result = await _store.DeletePolicyAsync("p-dap-my-delete");

        // Assert
        result.IsRight.ShouldBeTrue();

        var getResult = await _store.GetPolicyAsync("p-dap-my-delete");
        getResult.IsRight.ShouldBeTrue();
        getResult.IfRight(opt => opt.IsNone.ShouldBeTrue());
    }
    ...
    {
        // Arrange
        await _store.SavePolicyAsync(CreateMinimalPolicy("p-dap-my-exists"));

        // Act
        var result = await _store.ExistsPolicyAsync("p-dap-my-exists");
    ...
    {
        // Act
        var result = await _store.ExistsPolicyAsync("p-dap-my-not-exists");
    ...
    public async Task PolicySet_RoundTrip_PreservesNestedPolicies()
    {
        ...
        var result = await _store.GetPolicySetAsync("ps-dap-my-rt");

        // Assert
        result.IsRight.ShouldBeTrue();
        result.IfRight(opt => opt.IfSome(retrieved =>
        {
            retrieved.Id.ShouldBe("ps-dap-my-rt");
    ...
    public async Task Policy_RoundTrip_WithTargetExpression()
    {
        ...
        var policy = new Policy
        {
            ...
            Rules = [new Rule { Id = "r-dap-my-tgt", Effect = Effect.Permit, Obligations = [], Advice = [] }],
            Obligations = [],
            Advice = [],
            VariableDefinitions = []
        };
        ...
        var result = await _store.GetPolicyAsync("p-dap-my-target-rt");
