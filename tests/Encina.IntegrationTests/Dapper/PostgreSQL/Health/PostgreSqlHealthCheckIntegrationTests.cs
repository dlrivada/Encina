...
namespace Encina.IntegrationTests.Dapper.PostgreSQL.Health;

/// <summary>
/// Integration tests for <see cref="PostgreSqlHealthCheck"/> using a real PostgreSQL database.
/// </summary>
...
public sealed class PostgreSqlHealthCheckIntegrationTests : IAsyncLifetime
{
    ...
    public async Task CheckHealthAsync_WithCustomName_UsesCustomName()
    {
        ...
        // Arrange
        var options = new ProviderHealthCheckOptions { Name = "my-custom-postgres" };
        var serviceProvider = CreateServiceProvider();
        ...
        // Assert
        Assert.Equal("my-custom-postgres", healthCheck.Name);
        Assert.Equal(HealthStatus.Healthy, result.Status);
    }
