...
using Encina.Dapper.Oracle.UnitOfWork;
...
namespace Encina.IntegrationTests.Dapper.Oracle.UnitOfWork;

/// <summary>
/// Integration tests for <see cref="UnitOfWorkDapper"/> using real Oracle.
/// </summary>
...
public class UnitOfWorkDapperIntegrationTests : IAsyncLifetime
{
    ...
    private static async Task CreateTestProductsSchemaAsync(OracleConnection connection)
    {
        ...
        try
        {
            await using var dropCommand = new OracleCommand("DROP TABLE TEST_UOW_PRODUCTS", connection);
        ...
        const string indexSql = "CREATE INDEX IX_TEST_UOW_PRODUCTS_IS_ACTIVE ON TEST_UOW_PRODUCTS(IS_ACTIVE)";
    ...
    private async Task ClearDataAsync()
    {
        ...
        {
            await using var command = new OracleCommand("DELETE FROM TEST_UOW_PRODUCTS", oracleConnection);
    ...
    public async Task Transaction_CommitMultipleEntities_AllPersisted()
    {
        ...
        // Assert
        var count = await _connection.ExecuteScalarAsync<decimal>("SELECT COUNT(*) FROM TEST_UOW_PRODUCTS");
        count.ShouldBe(3);
    }
...
/// <summary>
/// Test product entity for Dapper Oracle UnitOfWork integration tests.
/// </summary>
