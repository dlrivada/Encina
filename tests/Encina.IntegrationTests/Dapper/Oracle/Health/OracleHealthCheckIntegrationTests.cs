...
namespace Encina.IntegrationTests.Dapper.Oracle.Health;

/// <summary>
/// Integration tests for <see cref="OracleHealthCheck"/> using a real Oracle database.
/// </summary>
...
public sealed class OracleHealthCheckIntegrationTests : IClassFixture<OracleFixture>
{
    ...
    public async Task CheckHealthAsync_WithCustomName_UsesCustomName()
    {
        ...
        // Arrange
        var options = new ProviderHealthCheckOptions { Name = "my-custom-dapper-oracle" };
        var serviceProvider = CreateServiceProvider();
        ...
        // Assert
        Assert.Equal("my-custom-dapper-oracle", healthCheck.Name);
        Assert.Equal(HealthStatus.Healthy, result.Status);
    }
