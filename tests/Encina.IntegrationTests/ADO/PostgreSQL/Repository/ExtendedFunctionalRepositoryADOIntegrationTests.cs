...
/// <summary>
/// Extended integration tests for <see cref="FunctionalRepositoryADO{TEntity, TId}"/> using real PostgreSQL.
...
public class ExtendedFunctionalRepositoryADOIntegrationTests : IAsyncLifetime
{
    ...
    private static async Task CreateExtendedTestItemsSchemaAsync(NpgsqlConnection connection)
    {
        const string sql = """
            ...
            CREATE INDEX ix_extended_test_items_created_at_utc ON extended_test_items(created_at_utc);
            CREATE INDEX ix_extended_test_items_amount ON extended_test_items(amount);
            """;
    ...
    private async Task ClearDataAsync()
    {
        ...
        {
            await using var command = new NpgsqlCommand("DELETE FROM extended_test_items", npgsqlConnection);
            await command.ExecuteNonQueryAsync();
        }
    }
    ...
    public async Task TypeMapping_DateTimePrecision_PreservesMicroseconds()
    {
        ...
        var preciseTime = new DateTime(2026, 1, 15, 10, 30, 45, 123, DateTimeKind.Utc)
            .AddTicks(4560); // 456 microseconds (within PostgreSQL precision)
        ...
        result.IfRight(e =>
        {
            // PostgreSQL TIMESTAMP preserves microsecond precision
...
/// <summary>
/// Extended test item entity for comprehensive ADO.NET PostgreSQL repository integration tests.
/// </summary>
