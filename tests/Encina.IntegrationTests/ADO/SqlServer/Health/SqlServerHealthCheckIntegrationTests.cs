...
namespace Encina.IntegrationTests.ADO.SqlServer.Health;

/// <summary>
/// Integration tests for ADO.NET <see cref="SqlServerHealthCheck"/> using a real SQL Server database.
/// </summary>
...
public sealed class SqlServerHealthCheckIntegrationTests : IClassFixture<SqlServerFixture>
{
    ...
    public async Task CheckHealthAsync_WithCustomName_UsesCustomName()
    {
        ...
        // Assert
        Assert.Equal("my-custom-ado-sqlserver", healthCheck.Name);
        Assert.Equal(HealthStatus.Healthy, result.Status);
    }
