using Encina.ADO.SqlServer.DPIA;
using Encina.Compliance.DPIA;
using Encina.Compliance.DPIA.Model;
using Encina.TestInfrastructure.Fixtures;
using LanguageExt;

namespace Encina.IntegrationTests.ADO.SqlServer.DPIA;

[Collection("ADO-SqlServer")]
[Trait("Category", "Integration")]
[Trait("Provider", "ADO.SqlServer")]
public class DPIAStoreADOSqlServerTests : IAsyncLifetime
{
    private readonly SqlServerFixture _fixture;
    private DPIAStoreADO _store = null!;
    private DPIAAuditStoreADO _auditStore = null!;

    public DPIAStoreADOSqlServerTests(SqlServerFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    {
        // Arrange
        var assessment = CreateAssessment(requestTypeName: "UniqueTypeName_SqlServer_1");
        await _store.SaveAssessmentAsync(assessment, CancellationToken.None);

        // Act
        var result = await _store.GetAssessmentAsync("UniqueTypeName_SqlServer_1", CancellationToken.None);
    ...
    {
        // Act
        var result = await _store.GetAssessmentAsync("NonExistentTypeName_SqlServer", CancellationToken.None);
    ...
    public async Task GetExpiredAssessmentsAsync_ShouldReturnOnlyExpiredApproved()
    {
        ...
        var expiredAssessment = CreateAssessment(
            requestTypeName: "ExpiredCommand_SqlServer",
        ...
        var futureAssessment = CreateAssessment(
            requestTypeName: "FutureCommand_SqlServer",
            status: DPIAAssessmentStatus.Approved,
    ...
    {
        // Arrange
        var assessment1 = CreateAssessment(requestTypeName: "AllTest1_SqlServer");
        var assessment2 = CreateAssessment(requestTypeName: "AllTest2_SqlServer");
    ...
    {
        // Arrange
        var assessment = CreateAssessment(requestTypeName: "DeleteTest_SqlServer");
        await _store.SaveAssessmentAsync(assessment, CancellationToken.None);
    ...
    {
        // Arrange
        var assessment = CreateAssessment(requestTypeName: "AuditRoundTrip_SqlServer");
        await _store.SaveAssessmentAsync(assessment, CancellationToken.None);
    ...
    {
        // Arrange
        var assessment1 = CreateAssessment(requestTypeName: "AuditIsolation1_SqlServer");
        var assessment2 = CreateAssessment(requestTypeName: "AuditIsolation2_SqlServer");
        await _store.SaveAssessmentAsync(assessment1, CancellationToken.None);
        await _store.SaveAssessmentAsync(assessment2, CancellationToken.None);
    ...
    {
        // Arrange
        var assessment = CreateAssessment(requestTypeName: "FullLifecycle_SqlServer");
