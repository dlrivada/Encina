...
namespace Encina.IntegrationTests.ADO.SqlServer.Tenancy;

/// <summary>
/// Integration tests for multi-tenancy support in ADO.NET SQL Server provider.
...
public class TenancyADOIntegrationTests : IAsyncLifetime
{
    ...
    private async Task ClearDataAsync()
    {
        ...
        {
            await TenancySchema.ClearTenancyDataAsync(sqlConnection);
        }
    }
    ...
    private void SwitchTenant(string tenantId)
    {
        ...
        // Recreate repository with new tenant context
        _repository = new TenantAwareFunctionalRepositoryADO<TenantTestEntity, Guid>(
            _connection, _mapping, _tenantProvider, _tenancyOptions);
    }
    ...
    public async Task ListAsync_WithSpecification_AppliesTenantFilterWithSpec()
    {
        ...
        // Act - Query active entities as Tenant1
        SwitchTenant(Tenant1);
    ...
    public async Task AddAsync_AutomaticallyAssignsTenantId()
    {
        ...
        entity.TenantId = string.Empty; // Ensure it's empty before insert
    ...
    public async Task UpdateAsync_CanOnlyUpdateOwnTenantEntities()
    {
        ...
        // Insert entity as Tenant1
        SwitchTenant(Tenant1);
        var entity = CreateEntity("Original");
        await _repository.AddAsync(entity);

        // Try to update as Tenant2
        SwitchTenant(Tenant2);
        ...
        // Assert - Should fail due to tenant mismatch
        result.IsLeft.ShouldBeTrue();
    ...
    public async Task DeleteAsync_CanOnlyDeleteOwnTenantEntities()
    {
        ...
        // Try to delete as Tenant2
        SwitchTenant(Tenant2);

        // Act
        var result = await _repository.DeleteAsync(entity.Id);

        // Assert - Should fail (entity not found for current tenant)
        result.IsLeft.ShouldBeTrue();

        // Verify entity still exists for Tenant1
        SwitchTenant(Tenant1);
        var retrieved = await _repository.GetByIdAsync(entity.Id);
        retrieved.IsRight.ShouldBeTrue();
    }
    ...
    public async Task TenantSwitch_ChangesVisibleData()
    {
        ...
        // Act & Assert - As Tenant1, only see Tenant1 data
        SwitchTenant(Tenant1);
        ...
        // Act & Assert - As Tenant2, only see Tenant2 data
        SwitchTenant(Tenant2);
        ...
        decimal amount = 100m)
    {
        ...
        {
            Id = Guid.NewGuid(),
            TenantId = string.Empty, // Will be auto-assigned
            Name = name,
            Description = null,
            Amount = amount,
            IsActive = isActive,
...
/// Allows switching between tenants during test execution.
/// </summary>
