...
/// <summary>
/// Extended integration tests for <see cref="FunctionalRepositoryADO{TEntity, TId}"/> using real SQLite.
...
public class ExtendedFunctionalRepositoryADOIntegrationTests : IClassFixture<SqliteFixture>, IAsyncLifetime
{
    ...
    private static async Task CreateExtendedTestItemsSchemaAsync(SqliteConnection connection)
    {
        ...
        const string sql = """
            ...
            CREATE INDEX IF NOT EXISTS IX_ExtendedTestItems_IsActive ON ExtendedTestItems(IsActive);
            CREATE INDEX IF NOT EXISTS IX_ExtendedTestItems_Category ON ExtendedTestItems(Category);
            CREATE INDEX IF NOT EXISTS IX_ExtendedTestItems_CreatedAtUtc ON ExtendedTestItems(CreatedAtUtc);
            CREATE INDEX IF NOT EXISTS IX_ExtendedTestItems_Amount ON ExtendedTestItems(Amount);
            """;
    ...
    private async Task ClearDataAsync()
    {
        if (_connection is SqliteConnection sqliteConnection)
        {
            ...
            await using var command = new SqliteCommand("DELETE FROM ExtendedTestItems", sqliteConnection);
            await command.ExecuteNonQueryAsync();
        }
    }
    ...
    [Fact]
    public async Task TypeMapping_DecimalPrecision_PreservesPrecision()
    {
        // Arrange
        await ClearDataAsync();
        // SQLite stores decimals as REAL (double), so precision may vary slightly
        ...
        result.IfRight(e =>
        {
            ...
            Math.Abs(e.Amount - 123.4567m).ShouldBeLessThan(0.0001m);
        });
    }
    ...
    public async Task TypeMapping_DateTimePrecision_PreservesMilliseconds()
    {
        // Arrange
        await ClearDataAsync();
        // SQLite stores DateTime as TEXT (ISO 8601), preserving up to 7 decimal places
        ...
        result.IfRight(e =>
        {
            // SQLite ISO 8601 storage should preserve milliseconds
            (e.CreatedAtUtc - preciseTime).TotalMilliseconds.ShouldBeLessThan(1);
        });
    }
...
/// <summary>
/// Extended test item entity for comprehensive ADO.NET SQLite repository integration tests.
/// </summary>
