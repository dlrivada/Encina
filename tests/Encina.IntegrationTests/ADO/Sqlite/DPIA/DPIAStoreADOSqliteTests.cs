using Encina.ADO.Sqlite.DPIA;
using Encina.Compliance.DPIA;
using Encina.Compliance.DPIA.Model;
using Encina.TestInfrastructure.Fixtures;
using LanguageExt;

namespace Encina.IntegrationTests.ADO.Sqlite.DPIA;

[Collection("ADO-Sqlite")]
[Trait("Category", "Integration")]
[Trait("Provider", "ADO.Sqlite")]
public class DPIAStoreADOSqliteTests : IAsyncLifetime
{
    private readonly SqliteFixture _fixture;
    private DPIAStoreADO _store = null!;
    private DPIAAuditStoreADO _auditStore = null!;

    public DPIAStoreADOSqliteTests(SqliteFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    {
        // Arrange
        var assessment = CreateAssessment(requestTypeName: "UniqueTypeName_Sqlite_1");
        await _store.SaveAssessmentAsync(assessment, CancellationToken.None);

        // Act
        var result = await _store.GetAssessmentAsync("UniqueTypeName_Sqlite_1", CancellationToken.None);
    ...
    {
        // Act
        var result = await _store.GetAssessmentAsync("NonExistentTypeName_Sqlite", CancellationToken.None);
    ...
    public async Task GetExpiredAssessmentsAsync_ShouldReturnOnlyExpiredApproved()
    {
        ...
        var expiredAssessment = CreateAssessment(
            requestTypeName: "ExpiredCommand_Sqlite",
        ...
        var futureAssessment = CreateAssessment(
            requestTypeName: "FutureCommand_Sqlite",
            status: DPIAAssessmentStatus.Approved,
    ...
    {
        // Arrange
        var assessment1 = CreateAssessment(requestTypeName: "AllTest1_Sqlite");
        var assessment2 = CreateAssessment(requestTypeName: "AllTest2_Sqlite");
    ...
    {
        // Arrange
        var assessment = CreateAssessment(requestTypeName: "DeleteTest_Sqlite");
        await _store.SaveAssessmentAsync(assessment, CancellationToken.None);
    ...
    {
        // Arrange
        var assessment = CreateAssessment(requestTypeName: "AuditRoundTrip_Sqlite");
        await _store.SaveAssessmentAsync(assessment, CancellationToken.None);
    ...
    {
        // Arrange
        var assessment1 = CreateAssessment(requestTypeName: "AuditIsolation1_Sqlite");
        var assessment2 = CreateAssessment(requestTypeName: "AuditIsolation2_Sqlite");
        await _store.SaveAssessmentAsync(assessment1, CancellationToken.None);
        await _store.SaveAssessmentAsync(assessment2, CancellationToken.None);
    ...
    {
        // Arrange
        var assessment = CreateAssessment(requestTypeName: "FullLifecycle_Sqlite");
