...
/// <summary>
/// Extended integration tests for <see cref="FunctionalRepositoryADO{TEntity, TId}"/> using real MySQL.
...
public class ExtendedFunctionalRepositoryADOIntegrationTests : IAsyncLifetime
{
    ...
    public ExtendedFunctionalRepositoryADOIntegrationTests(MySqlFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        _mapping = new EntityMappingBuilder<ExtendedTestItem, Guid>()
            ...
            .MapProperty(p => p.UpdatedAtUtc, "updated_at_utc")
            .Build()
            .ShouldBeSuccess();
    ...
    private static async Task CreateExtendedTestItemsSchemaAsync(MySqlConnection connection)
    {
        const string sql = """
            DROP TABLE IF EXISTS extended_test_items;
            CREATE TABLE extended_test_items (
                ...
                INDEX ix_extended_test_items_is_active (is_active),
                INDEX ix_extended_test_items_category (category),
                INDEX ix_extended_test_items_created_at_utc (created_at_utc),
                INDEX ix_extended_test_items_amount (amount)
    ...
    private async Task ClearDataAsync()
    {
        ...
        {
            await using var command = new MySqlCommand("DELETE FROM extended_test_items", mysqlConnection);
            await command.ExecuteNonQueryAsync();
        }
    }
    ...
    public async Task TypeMapping_DateTimePrecision_PreservesMicroseconds()
    {
        ...
        // Arrange
        await ClearDataAsync();
        // MySQL DATETIME(6) has microsecond precision
        var preciseTime = new DateTime(2026, 1, 15, 10, 30, 45, 123, DateTimeKind.Utc)
            .AddTicks(4560); // 456 microseconds
        ...
        result.IfRight(e =>
        {
            // MySQL DATETIME(6) preserves microsecond precision
            (e.CreatedAtUtc - preciseTime).TotalMicroseconds.ShouldBeLessThan(1);
        });
    }
...
/// <summary>
/// Extended test item entity for comprehensive ADO.NET MySQL repository integration tests.
/// </summary>
