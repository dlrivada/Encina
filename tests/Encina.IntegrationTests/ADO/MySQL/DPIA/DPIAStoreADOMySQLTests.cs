using Encina.ADO.MySQL.DPIA;
using Encina.Compliance.DPIA;
using Encina.Compliance.DPIA.Model;
using Encina.TestInfrastructure.Fixtures;
using LanguageExt;

namespace Encina.IntegrationTests.ADO.MySQL.DPIA;

[Collection("ADO-MySQL")]
[Trait("Category", "Integration")]
[Trait("Provider", "ADO.MySQL")]
public class DPIAStoreADOMySQLTests : IAsyncLifetime
{
    private readonly MySqlFixture _fixture;
    private DPIAStoreADO _store = null!;
    private DPIAAuditStoreADO _auditStore = null!;

    public DPIAStoreADOMySQLTests(MySqlFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    {
        // Arrange
        var assessment = CreateAssessment(requestTypeName: "UniqueTypeName_MySQL_1");
        await _store.SaveAssessmentAsync(assessment, CancellationToken.None);

        // Act
        var result = await _store.GetAssessmentAsync("UniqueTypeName_MySQL_1", CancellationToken.None);
    ...
    {
        // Act
        var result = await _store.GetAssessmentAsync("NonExistentTypeName_MySQL", CancellationToken.None);
    ...
    public async Task GetExpiredAssessmentsAsync_ShouldReturnOnlyExpiredApproved()
    {
        ...
        var expiredAssessment = CreateAssessment(
            requestTypeName: "ExpiredCommand_MySQL",
        ...
        var futureAssessment = CreateAssessment(
            requestTypeName: "FutureCommand_MySQL",
            status: DPIAAssessmentStatus.Approved,
    ...
    {
        // Arrange
        var assessment1 = CreateAssessment(requestTypeName: "AllTest1_MySQL");
        var assessment2 = CreateAssessment(requestTypeName: "AllTest2_MySQL");
    ...
    {
        // Arrange
        var assessment = CreateAssessment(requestTypeName: "DeleteTest_MySQL");
        await _store.SaveAssessmentAsync(assessment, CancellationToken.None);
    ...
    {
        // Arrange
        var assessment = CreateAssessment(requestTypeName: "AuditRoundTrip_MySQL");
        await _store.SaveAssessmentAsync(assessment, CancellationToken.None);
    ...
    {
        // Arrange
        var assessment1 = CreateAssessment(requestTypeName: "AuditIsolation1_MySQL");
        var assessment2 = CreateAssessment(requestTypeName: "AuditIsolation2_MySQL");
        await _store.SaveAssessmentAsync(assessment1, CancellationToken.None);
        await _store.SaveAssessmentAsync(assessment2, CancellationToken.None);
    ...
    {
        // Arrange
        var assessment = CreateAssessment(requestTypeName: "FullLifecycle_MySQL");
