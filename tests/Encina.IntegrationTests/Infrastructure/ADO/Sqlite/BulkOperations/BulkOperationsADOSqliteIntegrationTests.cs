...
namespace Encina.IntegrationTests.Infrastructure.ADO.Sqlite.BulkOperations;

/// <summary>
/// Integration tests for <see cref="BulkOperationsSqlite{TEntity, TId}"/> with real SQLite.
/// Uses in-memory SQLite for fast integration tests.
/// </summary>
/// <remarks>
/// These tests verify the actual multi-row INSERT, UPDATE, DELETE, and MERGE operations
/// work correctly against a real SQLite database.
/// </remarks>
...
public sealed class BulkOperationsADOSqliteIntegrationTests : IAsyncLifetime
{
    ...
    private BulkOperationsSqlite<BulkTestOrder, Guid> _bulkOps = null!;

    public BulkOperationsADOSqliteIntegrationTests(SqliteFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    private async Task DropBulkTestTableAsync()
    {
        ...
        if (_connection.State == ConnectionState.Open && _connection is SqliteConnection sqliteConnection)
        {
            await using var command = sqliteConnection.CreateCommand();
            command.CommandText = sql;
            await command.ExecuteNonQueryAsync();
