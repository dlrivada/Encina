...
/// <summary>
/// PostgreSQL integration tests proving that the tenant isolation filter and the soft-delete
...
public sealed class TenancyAndSoftDeleteEFPostgreSqlTests : IAsyncLifetime
{
    ...
    public TenancyAndSoftDeleteEFPostgreSqlTests(EFCorePostgreSqlFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        await using var context = CreateContext<TenantThenSoftDeletePostgreSqlDbContext>("setup");
    ...
    public async Task TenantFilter_AppliedBeforeSoftDelete_OnlyReturnsCurrentTenantNonDeletedRows()
    {
        ...
        await SeedTwoTenantsWithSoftDeletedRowsAsync<TenantThenSoftDeletePostgreSqlDbContext>();

        await using var queryContext = CreateContext<TenantThenSoftDeletePostgreSqlDbContext>("tenant-a");
    ...
    public async Task SoftDeleteFilter_AppliedBeforeTenant_OnlyReturnsCurrentTenantNonDeletedRows()
    {
        ...
        await SeedTwoTenantsWithSoftDeletedRowsAsync<SoftDeleteThenTenantPostgreSqlDbContext>();

        await using var queryContext = CreateContext<SoftDeleteThenTenantPostgreSqlDbContext>("tenant-a");
        var entities = await queryContext.TenantSoftDeleteTestEntities.ToListAsync();
    ...
    private async Task ClearDataAsync()
    {
        try
        {
            await using var context = CreateContext<TenantThenSoftDeletePostgreSqlDbContext>("admin");
    ...
    private static async Task EnsureSchemaCreatedAsync(TenantDbContext context)
    {
        ...
        catch (DbException ex) when (ex is PostgresException { SqlState: "42P07" })
        {
