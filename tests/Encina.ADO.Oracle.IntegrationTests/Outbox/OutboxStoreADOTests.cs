...
namespace Encina.ADO.Oracle.Tests.Outbox;

/// <summary>
/// Integration tests for <see cref="OutboxStoreADO"/>.
/// Tests the ADO.NET implementation of the Outbox pattern for reliable event publishing.
/// Uses real Oracle database via Testcontainers for end-to-end verification.
/// </summary>
[Trait("Category", "Integration")]
[Trait("Provider", "ADO.Oracle")]
public sealed class OutboxStoreADOTests : IClassFixture<OracleFixture>
{
    private readonly OracleFixture _fixture;
    private readonly OutboxStoreADO _store;

    public OutboxStoreADOTests(OracleFixture fixture)
    {
        _fixture = fixture;
    ...
    /// <summary>
    /// Tests that SaveChangesAsync returns a completed task (no-op for ADO.NET).
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that OutboxStoreADO can use a custom table name.
    /// </summary>
    [Fact]
    public async Task Constructor_CustomTableName_ShouldUseCustomTable()
    {
        ...
        // Create custom table inline using ADO.NET
        if (connection is OracleConnection oracleConn)
        {
            using var command = new OracleCommand($@"
                BEGIN
                    EXECUTE IMMEDIATE 'CREATE TABLE {customTableName} (
        ...
        var customStore = new OutboxStoreADO(connection, customTableName);
