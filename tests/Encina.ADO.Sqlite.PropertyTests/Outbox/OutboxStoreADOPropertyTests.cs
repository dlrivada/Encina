...
public sealed class OutboxStoreADOPropertyTests : IClassFixture<SqliteFixture>
{
    private readonly SqliteFixture _fixture;

    public OutboxStoreADOPropertyTests(SqliteFixture fixture)
    {
        _fixture = fixture;
        ...
        if (pending.Count > 1)
        {
            for (int i = 0; i < pending.Count - 1; i++)
            {
                Assert.True(pending[i].CreatedAtUtc <= pending[i + 1].CreatedAtUtc,
                    $"Message at index {i} has TEXT {pending[i].CreatedAtUtc}, " +
                    $"which is after message at index {i + 1} with TEXT {pending[i + 1].CreatedAtUtc}");
            }
        }
    }
