...
/// <summary>
/// Guard clause tests for Encina.Security.Secrets.AzureKeyVault types.
/// Verifies that null and invalid arguments are properly rejected.
/// </summary>
public class AzureKeyVaultGuardTests
{
    #region AzureKeyVaultSecretProvider Constructor Guards

    [Fact]
    public void Constructor_NullClient_ThrowsArgumentNullException()
    {
        var logger = Substitute.For<ILogger<AzureKeyVaultSecretProvider>>();
    ...
    private static AzureKeyVaultSecretProvider CreateProvider()
    {
        var client = Substitute.For<SecretClient>();
        var logger = Substitute.For<ILogger<AzureKeyVaultSecretProvider>>();
        return new AzureKeyVaultSecretProvider(client, logger);
    }
