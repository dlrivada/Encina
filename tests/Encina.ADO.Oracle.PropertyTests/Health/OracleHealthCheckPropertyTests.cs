...
namespace Encina.ADO.Oracle.PropertyTests.Health;

/// <summary>
/// Property-based tests for <see cref="OracleHealthCheck"/> invariants (ADO.NET).
/// </summary>
public sealed class OracleHealthCheckPropertyTests
{
    [Fact]
    public void DefaultName_IsAlwaysEncinaAdoOracle()
    {
        Assert.Equal("encina-ado-oracle", OracleHealthCheck.DefaultName);
    }
    ...
    public bool Constructor_WithNullOptions_UsesDefaultName(int seed)
    {
        ...
        return healthCheck.Name == OracleHealthCheck.DefaultName;
    }
    ...
    [Property(MaxTest = 50)]
    public bool Tags_WithDefaultOptions_ContainsOracle(int seed)
    {
        var serviceProvider = CreateMockServiceProvider();
        var healthCheck = new OracleHealthCheck(serviceProvider, null);
        return healthCheck.Tags.Contains("oracle");
    }
