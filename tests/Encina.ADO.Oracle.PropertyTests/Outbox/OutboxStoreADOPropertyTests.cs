...
/// <summary>
/// Property-based integration tests for <see cref="OutboxStoreADO"/>.
...
public sealed class OutboxStoreADOPropertyTests : IClassFixture<OracleFixture>
{
    private readonly OracleFixture _fixture;

    public OutboxStoreADOPropertyTests(OracleFixture fixture)
    {
        _fixture = fixture;
    ...
    public async Task AddedMessage_AlwaysRetrievableInPending(string notificationType, string content)
    {
        ...
        var store = new OutboxStoreADO(_fixture.CreateConnection());
        ...
        if (pending.Count > 1)
        {
            for (int i = 0; i < pending.Count - 1; i++)
            {
                Assert.True(pending[i].CreatedAtUtc <= pending[i + 1].CreatedAtUtc,
                    $"Message at index {i} has VARCHAR2(4000) {pending[i].CreatedAtUtc}, " +
                    $"which is after message at index {i + 1} with VARCHAR2(4000) {pending[i + 1].CreatedAtUtc}");
            }
        }
    }
