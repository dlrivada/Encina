...
/// <summary>
/// Property-based tests for <see cref="MongoDbHealthCheck"/> invariants.
/// </summary>
public sealed class MongoDbHealthCheckPropertyTests
{
    [Fact]
    public void DefaultName_IsAlwaysEncinaMongodb()
    {
        Assert.Equal("encina-mongodb", MongoDbHealthCheck.DefaultName);
    }
    ...
    public bool Constructor_WithNullOptions_UsesDefaultName(int seed)
    {
        ...
        return healthCheck.Name == MongoDbHealthCheck.DefaultName;
    }
    ...
    [Property(MaxTest = 50)]
    public bool Tags_WithDefaultOptions_ContainsMongodb(int seed)
    {
        var serviceProvider = CreateMockServiceProvider();
        var healthCheck = new MongoDbHealthCheck(serviceProvider, null);
        return healthCheck.Tags.Contains("mongodb");
    }
