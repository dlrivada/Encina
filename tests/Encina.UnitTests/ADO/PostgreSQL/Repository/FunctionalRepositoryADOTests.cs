...
/// <summary>
/// Unit tests for <see cref="FunctionalRepositoryADO{TEntity, TId}"/> for PostgreSQL.
...
public class FunctionalRepositoryADOTests : IDisposable
{
    ...
    private readonly IEntityMapping<TestEntityPgADO, Guid> _mapping;
    private readonly FunctionalRepositoryADO<TestEntityPgADO, Guid> _repository;
    ...
    public FunctionalRepositoryADOTests()
    {
        ...
        _mapping = new EntityMappingBuilder<TestEntityPgADO, Guid>()
        ...
        _repository = new FunctionalRepositoryADO<TestEntityPgADO, Guid>(_mockConnection, _mapping);
    }
    ...
    public void Constructor_NullConnection_ThrowsArgumentNullException()
    {
        ...
        Should.Throw<ArgumentNullException>(() =>
            new FunctionalRepositoryADO<TestEntityPgADO, Guid>(null!, _mapping));
    }
    ...
    public void Constructor_NullMapping_ThrowsArgumentNullException()
    {
        ...
        // Act & Assert
        Should.Throw<ArgumentNullException>(() =>
            new FunctionalRepositoryADO<TestEntityPgADO, Guid>(connection, null!));
    }
    ...
    {
        // Arrange
        var spec = new ActiveEntityPgADOSpec();
    ...
    {
        // Arrange
        var spec = new UnsupportedPgSpec();
    ...
    {
        // Arrange
        _mockCommand.ExecuteScalar().Returns(false);
    ...
    public async Task AddAsync_DuplicateKey_ReturnsLeftWithAlreadyExistsError()
    {
        // Arrange
        var entity = CreateTestEntity();
        // PostgreSQL unique violation error (23505) simulation via InvalidOperationException
        _mockCommand.ExecuteNonQuery().Throws(new InvalidOperationException("duplicate key value violates unique constraint"));
    ...
    [Fact]
    public async Task AddAsync_PostgresUniqueConstraintViolation_ReturnsLeftWithAlreadyExistsError()
    {
        // Arrange
        var entity = CreateTestEntity();
        // PostgreSQL error code 23505 for unique violation
        _mockCommand.ExecuteNonQuery().Throws(new InvalidOperationException("23505: unique constraint violation"));
    ...
    public async Task DeleteAsync_ByEntity_NullEntity_ThrowsArgumentNullException()
    {
        // Act & Assert
        await Should.ThrowAsync<ArgumentNullException>(async () =>
            await _repository.DeleteAsync((TestEntityPgADO)null!));
    }
    ...
    [Fact]
    public async Task AddAsync_WithNativeGuid_PassesGuidDirectly()
    {
        ...
        // Assert
        result.IsRight.ShouldBeTrue();
        // The GUID should be passed directly without string conversion
        // CreateParameter is called multiple times (once per column), verify it was called at least once
        _mockCommand.Received().CreateParameter();
    }

    [Fact]
    public async Task AddAsync_WithNativeBoolean_PassesBooleanDirectly()
    {
        // Arrange - PostgreSQL has native boolean support
        var entity = CreateTestEntity();
        ...
        // Assert
        result.IsRight.ShouldBeTrue();
        // The boolean should be passed directly without conversion to 1/0
    }
    ...
    private static TestEntityPgADO CreateTestEntity(
        string name = "Test Entity",
        bool isActive = true,
        decimal amount = 100m)
    {
        return new TestEntityPgADO
        {
            Id = Guid.NewGuid(),
            Name = name,
            Amount = amount,
...
/// <summary>
/// Test entity for ADO.NET PostgreSQL repository unit tests.
/// </summary>
public class TestEntityPgADO
{
...
/// <summary>
/// Specification for active entities (PostgreSQL ADO tests).
/// </summary>
public class ActiveEntityPgADOSpec : Specification<TestEntityPgADO>
{
...
/// <summary>
/// Specification for minimum amount (PostgreSQL ADO tests).
/// </summary>
public class MinAmountPgADOSpec : Specification<TestEntityPgADO>
{
    private readonly decimal _minAmount;

    public MinAmountPgADOSpec(decimal minAmount) => _minAmount = minAmount;
