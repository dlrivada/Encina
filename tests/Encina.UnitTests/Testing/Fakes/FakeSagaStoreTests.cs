...
/// <summary>
/// Unit tests for <see cref="FakeSagaStore"/>.
/// </summary>
public sealed class FakeSagaStoreTests
{
    ...
    {
        // Act & Assert
        await Should.ThrowAsync<ArgumentNullException>(() => _sut.UpdateAsync(null!));
    }
    ...
    [Fact]
    public async Task WasSagaStarted_ByTypeName_ReturnsCorrectly()
    {
        // Arrange
        var saga = new FakeSagaState
        {
        ...
        // Assert
        _sut.WasSagaStarted("MyApp.OrderFulfillmentSaga").ShouldBeTrue();
        _sut.WasSagaStarted("NonExistent").ShouldBeFalse();
    }
    ...
    [Fact]
    public void GetSaga_NonExistentId_ReturnsNull()
    {
        // Act
        var result = _sut.GetSaga(Guid.NewGuid());
    ...
    public void Constructor_WithTimeProvider_UsesProvidedProvider()
    {
        ...
        // Act
        var store = new FakeSagaStore(fakeTime);
