...
public sealed class CommandMetricsPipelineBehaviorTests
{
    ...
    public void Constructor_WithNullMetrics_ThrowsArgumentNullException()
    {
        Should.Throw<ArgumentNullException>(() =>
            new CommandMetricsPipelineBehavior<TestCommand, string>(null!, NullFunctionalFailureDetector.Instance));
    }
    ...
    public async Task Handle_WithNullRequest_TracksFailure()
    {
        ...
        var behavior = new CommandMetricsPipelineBehavior<TestCommand, string>(metrics, NullFunctionalFailureDetector.Instance);
        ...
        failure.RequestKind.ShouldBe("command");
        failure.RequestName.ShouldBe("TestCommand");
    ...
    public async Task Handle_WithSuccessfulHandler_TracksSuccess()
    {
        ...
        success.RequestKind.ShouldBe("command");
        success.RequestName.ShouldBe("TestCommand");
    }
    ...
    {
        var metrics = new SpyMetrics();
        var detector = new TestFailureDetector(detectsFailure: true, failureReason: "payment.declined");
        ...
        metrics.Failures.Count.ShouldBe(1);
        metrics.Failures[0].Reason.ShouldBe("payment.declined");
