...
public class InMemoryTokenMappingStoreTests
{
    ...
    public async Task GetByTokenAsync_ExistingToken_ShouldReturnSome()
    {
        ...
        result.IsRight.ShouldBeTrue();
        result.Match(
            Right: option => option.IsSome.ShouldBeTrue(),
            Left: _ => Assert.Fail("Expected Right"));
    }
    ...
    public async Task GetByTokenAsync_ShouldReturnCorrectMapping()
    {
        ...
        option.Match(
            Some: m =>
            {
                m.Token.ShouldBe("tok-verify");
                m.OriginalValueHash.ShouldBe("hash-verify");
                m.KeyId.ShouldBe("key-verify");
                m.EncryptedOriginalValue.ShouldBe(new byte[] { 0x01, 0x02, 0x03 });
            },
            None: () => Assert.Fail("Expected Some"));
    }
    ...
    public async Task GetByOriginalValueHashAsync_ShouldReturnCorrectMapping()
    {
        ...
        option.Match(
            Some: m =>
            {
                m.Token.ShouldBe("tok-hash-lookup");
                m.OriginalValueHash.ShouldBe("hash-lookup");
                m.KeyId.ShouldBe("key-lookup");
            },
            None: () => Assert.Fail("Expected Some"));
    }
