...
/// <summary>
/// Property-based tests for <see cref="AmazonSQSHealthCheck"/> invariants.
/// </summary>
public sealed class AmazonSQSHealthCheckPropertyTests
{
    [Fact]
    public void DefaultName_IsAlwaysEncinaAmazonSqs()
    {
        Assert.Equal("encina-amazon-sqs", AmazonSQSHealthCheck.DefaultName);
    }
    ...
    public bool Constructor_WithNullOptions_UsesDefaultName(int seed)
    {
        ...
        var healthCheck = new AmazonSQSHealthCheck(serviceProvider, null);
        return healthCheck.Name == AmazonSQSHealthCheck.DefaultName;
    }
    ...
    public bool Constructor_WithCustomName_UsesCustomName(PositiveInt seed)
    {
        ...
        var healthCheck = new AmazonSQSHealthCheck(serviceProvider, options);
    ...
    [Property(MaxTest = 50)]
    public bool Tags_WithDefaultOptions_ContainsAmazonSqs(int seed)
    {
        var serviceProvider = CreateMockServiceProvider();
        var healthCheck = new AmazonSQSHealthCheck(serviceProvider, null);
        return healthCheck.Tags.Contains("amazon-sqs");
    }
