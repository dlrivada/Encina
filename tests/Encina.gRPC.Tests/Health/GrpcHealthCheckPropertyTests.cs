...
/// <summary>
/// Property-based tests for <see cref="GrpcHealthCheck"/> invariants.
/// </summary>
public sealed class GrpcHealthCheckPropertyTests
{
    [Fact]
    public void DefaultName_IsAlwaysEncinaGrpc()
    {
        Assert.Equal("encina-grpc", GrpcHealthCheck.DefaultName);
    }
    ...
    public bool Constructor_WithNullOptions_UsesDefaultName(int seed)
    {
        ...
        return healthCheck.Name == GrpcHealthCheck.DefaultName;
    }
    ...
    [Property(MaxTest = 50)]
    public bool Tags_WithDefaultOptions_ContainsGrpc(int seed)
    {
        var serviceProvider = CreateMockServiceProvider();
        var healthCheck = new GrpcHealthCheck(serviceProvider, null);
        return healthCheck.Tags.Contains("grpc");
    }
