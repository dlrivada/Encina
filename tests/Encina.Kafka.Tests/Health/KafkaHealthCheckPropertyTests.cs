...
/// <summary>
/// Property-based tests for <see cref="KafkaHealthCheck"/> invariants.
/// </summary>
public sealed class KafkaHealthCheckPropertyTests
{
    [Fact]
    public void DefaultName_IsAlwaysEncinaKafka()
    {
        Assert.Equal("encina-kafka", KafkaHealthCheck.DefaultName);
    }
    ...
    public bool Constructor_WithNullOptions_UsesDefaultName(int seed)
    {
        ...
        return healthCheck.Name == KafkaHealthCheck.DefaultName;
    }
    ...
    [Property(MaxTest = 50)]
    public bool Tags_WithDefaultOptions_ContainsKafka(int seed)
    {
        var serviceProvider = CreateMockServiceProvider();
        var healthCheck = new KafkaHealthCheck(serviceProvider, null);
        return healthCheck.Tags.Contains("kafka");
    }
