...
Audit store performance is dominated by database write latency, which varies by provider (SQLite < 1ms, SQL Server ~5ms, PostgreSQL ~3ms). BenchmarkDotNet cannot meaningfully benchmark I/O-bound operations — use Load Tests instead.
...
- **Integration Tests**: Cover all 13 database providers with real I/O
