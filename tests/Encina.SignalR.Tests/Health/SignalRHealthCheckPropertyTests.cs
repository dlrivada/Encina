...
/// <summary>
/// Property-based tests for <see cref="SignalRHealthCheck"/> invariants.
/// </summary>
public sealed class SignalRHealthCheckPropertyTests
{
    [Fact]
    public void DefaultName_IsAlwaysEncinaSignalr()
    {
        Assert.Equal("encina-signalr", SignalRHealthCheck.DefaultName);
    }
    ...
    public bool Constructor_WithNullOptions_UsesDefaultName(int seed)
    {
        ...
        return healthCheck.Name == SignalRHealthCheck.DefaultName;
    }
    ...
    [Property(MaxTest = 50)]
    public bool Tags_WithDefaultOptions_ContainsSignalr(int seed)
    {
        var serviceProvider = CreateMockServiceProvider();
        var healthCheck = new SignalRHealthCheck(serviceProvider, null);
        return healthCheck.Tags.Contains("signalr");
    }
