...
/// <summary>
/// Describes a property that requires output encoding, including its attribute metadata
/// and compiled getter/setter delegates for high-performance access.
/// </summary>
/// <param name="Property">The property info.</param>
/// <param name="Attribute">The encoding attribute applied to the property.</param>
...
internal sealed record EncodablePropertyInfo(
    PropertyInfo Property,
    EncodingAttribute Attribute,
    Func<object, object?> Getter,
    Action<object, object?> Setter);

/// <summary>
/// Thread-safe static cache for discovering and caching properties decorated with
/// <see cref="EncodingAttribute"/> subclasses on a per-type basis.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// </remarks>
internal static class EncodingPropertyCache
{
    private static readonly ConcurrentDictionary<Type, EncodablePropertyInfo[]> Cache = new();

    /// <summary>
    /// Gets the encodable property descriptors for the specified type.
    /// </summary>
    /// <param name="type">The type to discover encodable properties on.</param>
    /// <returns>
    /// An array of <see cref="EncodablePropertyInfo"/> for properties decorated with
    /// any <see cref="EncodingAttribute"/> subclass. Returns an empty array if the type
    /// has no encodable properties.
    /// </returns>
    internal static EncodablePropertyInfo[] GetProperties(Type type)
    {
        return Cache.GetOrAdd(type, static t => DiscoverProperties(t));
    }

    /// <summary>
    /// Gets the string property descriptors for auto-encoding mode
    ...
    /// <see cref="EncodingAttribute"/> subclass and builds compiled delegates.
    /// </summary>
    private static EncodablePropertyInfo[] DiscoverProperties(Type type)
    {
        var properties = type.GetProperties(BindingFlags.Public | BindingFlags.Instance);
        var encodable = new List<EncodablePropertyInfo>();
        ...
        foreach (var property in properties)
        {
            ...
            var attribute = property.GetCustomAttribute<EncodingAttribute>(inherit: true);
            ...
            encodable.Add(new EncodablePropertyInfo(property, attribute, getter, setter));
        }
