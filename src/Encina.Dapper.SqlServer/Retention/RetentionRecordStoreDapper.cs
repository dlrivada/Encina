...
/// <summary>
/// Dapper implementation of <see cref="IRetentionRecordStore"/> for SQL Server.
...
public sealed class RetentionRecordStoreDapper : IRetentionRecordStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var nowUtc = _timeProvider.GetUtcNow();
            DateTimeOffset? deletedAtUtc = newStatus == RetentionStatus.Deleted ? nowUtc : null;
    ...
    private static RetentionRecordEntity MapToEntity(dynamic row)
    {
        return new RetentionRecordEntity
        {
            ...
            ExpiresAtUtc = (DateTimeOffset)row.ExpiresAtUtc,
            StatusValue = (int)row.StatusValue,
            DeletedAtUtc = row.DeletedAtUtc is null or DBNull ? null : (DateTimeOffset?)row.DeletedAtUtc,
