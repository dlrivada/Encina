    ...
    where TCommand : ICommand<TResponse>
{
    ...
    public async ValueTask<Either<EncinaError, TResponse>> Handle(TCommand request, IRequestContext context, RequestHandlerCallback<TResponse> nextStep, CancellationToken cancellationToken)
    {
        ...
        catch (OperationCanceledException ex) when (cancellationToken.IsCancellationRequested)
        {
            ...
            return Left<EncinaError, TResponse>(EncinaErrors.Create(EncinaErrorCodes.BehaviorCancelled, $"Behavior {GetType().Name} cancelled the {typeof(TCommand).Name} request.", ex)); // NOSONAR S6966: LanguageExt Left is a pure function
        }
        catch (Exception ex)
        {
            var elapsed = Stopwatch.GetElapsedTime(startedAt);
