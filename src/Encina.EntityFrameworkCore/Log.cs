...
internal static partial class Log
{
    ...
    [LoggerMessage(EventId = 3012, Level = LogLevel.Debug, Message = "Request {RequestType} is already in a transaction, reusing existing transaction (CorrelationId: {CorrelationId})")]
    public static partial void ReusingExistingTransaction(ILogger logger, string requestType, string correlationId);

    [LoggerMessage(EventId = 3013, Level = LogLevel.Debug, Message = "Beginning transaction for request {RequestType} with isolation level {IsolationLevel} (CorrelationId: {CorrelationId})")]
    ...
    [LoggerMessage(EventId = 3017, Level = LogLevel.Warning, Message = "Idempotent request {RequestType} received without MessageId/IdempotencyKey (CorrelationId: {CorrelationId})")]
    public static partial void MissingIdempotencyKey(ILogger logger, string requestType, string correlationId);

    [LoggerMessage(EventId = 3018, Level = LogLevel.Debug, Message = "Processing idempotent request {RequestType} with MessageId {MessageId} (CorrelationId: {CorrelationId})")]
    ...
    [LoggerMessage(EventId = 3019, Level = LogLevel.Information, Message = "Returning cached response for duplicate message {MessageId} (CorrelationId: {CorrelationId})")]
    public static partial void ReturningCachedResponse(ILogger logger, string messageId, string correlationId);

    [LoggerMessage(EventId = 3020, Level = LogLevel.Warning, Message = "Message {MessageId} exceeded max retries ({MaxRetries}) (CorrelationId: {CorrelationId})")]
    public static partial void MaxRetriesExceeded(ILogger logger, string messageId, int maxRetries, string correlationId);

    [LoggerMessage(EventId = 3021, Level = LogLevel.Information, Message = "Successfully processed and cached message {MessageId} (CorrelationId: {CorrelationId})")]
    public static partial void ProcessedAndCachedMessage(ILogger logger, string messageId, string correlationId);

    [LoggerMessage(EventId = 3022, Level = LogLevel.Error, Message = "Error processing message {MessageId} (CorrelationId: {CorrelationId})")]
    ...
    [LoggerMessage(EventId = 3023, Level = LogLevel.Debug, Message = "Storing {Count} notifications in outbox for request {RequestType} (CorrelationId: {CorrelationId})")]
    ...
    [LoggerMessage(EventId = 3036, Level = LogLevel.Trace, Message = "Module '{ModuleName}' schema access validated. Accessed schemas: [{AccessedSchemas}]")]
    public static partial void ModuleSchemaAccessValidated(ILogger logger, string moduleName, string accessedSchemas);

    [LoggerMessage(EventId = 3037, Level = LogLevel.Debug, Message = "Setting module execution context to '{ModuleName}' for request {RequestType} (CorrelationId: {CorrelationId})")]
    public static partial void SettingModuleContext(ILogger logger, string moduleName, string requestType, string correlationId);

    [LoggerMessage(EventId = 3038, Level = LogLevel.Debug, Message = "Clearing module execution context after request {RequestType} (CorrelationId: {CorrelationId})")]
    public static partial void ClearingModuleContext(ILogger logger, string requestType, string correlationId);

    [LoggerMessage(EventId = 3039, Level = LogLevel.Debug, Message = "No module found for handler {HandlerType}, skipping module context (CorrelationId: {CorrelationId})")]
    ...
    [LoggerMessage(EventId = 3040, Level = LogLevel.Debug, Message = "Dispatching {EventCount} domain events after SaveChanges")]
    public static partial void DispatchingDomainEvents(ILogger logger, int eventCount);

    [LoggerMessage(EventId = 3041, Level = LogLevel.Debug, Message = "Domain event {EventType} (EventId: {EventId}) published successfully")]
    public static partial void DomainEventPublished(ILogger logger, string eventType, Guid eventId);

    [LoggerMessage(EventId = 3042, Level = LogLevel.Warning, Message = "Domain event {EventType} does not implement INotification, skipping")]
    ...
    [LoggerMessage(EventId = 3046, Level = LogLevel.Information, Message = "Domain event dispatch completed, {RemainingCount} events remaining (if any, they were skipped)")]
    public static partial void DomainEventsDispatchCompleted(ILogger logger, int remainingCount);

    [LoggerMessage(EventId = 3047, Level = LogLevel.Debug, Message = "Pre-save event collection: {EventCount} domain events collected from change tracker")]
