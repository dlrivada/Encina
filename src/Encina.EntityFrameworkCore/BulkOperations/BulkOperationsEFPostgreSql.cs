...
/// <summary>
/// PostgreSQL implementation of <see cref="IBulkOperations{TEntity}"/> using batched operations
/// for high-performance bulk operations.
/// </summary>
/// <typeparam name="TEntity">The entity type.</typeparam>
/// <remarks>
/// <para>
/// This implementation uses PostgreSQL-specific features for optimized bulk operations:
/// </para>
/// <list type="bullet">
...
/// <item><description>BulkMerge: Uses INSERT ON CONFLICT DO UPDATE for upsert semantics</description></item>
/// </list>
/// </remarks>
public sealed class BulkOperationsEFPostgreSql<TEntity> : IBulkOperations<TEntity>
    where TEntity : class, new()
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="BulkOperationsEFPostgreSql{TEntity}"/> class.
    /// </summary>
    /// <param name="dbContext">The database context.</param>
    public BulkOperationsEFPostgreSql(DbContext dbContext)
    {
        ...
        try
        {
            ...
            if (connection is not NpgsqlConnection npgsqlConnection)
            {
                return Left<EncinaError, int>(
                    RepositoryErrors.BulkInsertFailed<TEntity>("This implementation requires a PostgreSQL connection"));
            }
            ...
            {
                inserted += await InsertBatchAsync(npgsqlConnection, batch.ToList(), propertiesToInclude, cancellationToken).ConfigureAwait(false);
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                return Left<EncinaError, int>(
                    RepositoryErrors.BulkUpdateFailed<TEntity>("This implementation requires a PostgreSQL connection"));
            }
            ...
            foreach (var batch in batches)
            {
                updated += await UpdateBatchAsync(npgsqlConnection, batch.ToList(), propertiesToInclude, cancellationToken).ConfigureAwait(false);
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                return Left<EncinaError, int>(
                    RepositoryErrors.BulkDeleteFailed<TEntity>("This implementation requires a PostgreSQL connection"));
            }
            ...
            var deleted = await DeleteByIdsAsync(npgsqlConnection, ids, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                return Left<EncinaError, int>(
                    RepositoryErrors.BulkMergeFailed<TEntity>("This implementation requires a PostgreSQL connection"));
            }
            ...
            foreach (var batch in batches)
            {
                affected += await MergeBatchAsync(npgsqlConnection, batch.ToList(), propertiesToInclude, cancellationToken).ConfigureAwait(false);
            }
        ...
        try
        {
            ...
            if (connection is not NpgsqlConnection npgsqlConnection)
            {
                return Left<EncinaError, IReadOnlyList<TEntity>>(
                    RepositoryErrors.BulkReadFailed<TEntity>("This implementation requires a PostgreSQL connection"));
            }

            await EnsureConnectionOpenAsync(npgsqlConnection, cancellationToken).ConfigureAwait(false);

            var entities = await ReadByIdsAsync(npgsqlConnection, idList, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            command.Transaction = currentTransaction.GetDbTransaction() as NpgsqlTransaction;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var entity in entities)
        {
            ...
            parameters.Add(new NpgsqlParameter("@Id", idValue));
    ...
    private async Task<int> MergeBatchAsync(
        ...
        CancellationToken cancellationToken)
    {
        // PostgreSQL uses INSERT ... ON CONFLICT DO UPDATE for upsert
        var columnNames = string.Join(", ", properties.Select(p => $"\"{p.Value}\""));
        var updateColumns = properties.Where(p => p.Value != _idColumnName).ToList();
        var updateClauses = string.Join(", ", updateColumns.Select(p => $"\"{p.Value}\" = EXCLUDED.\"{p.Value}\""));
        ...
        sql.Append(string.Join(", ", valueClauses));
        sql.Append(CultureInfo.InvariantCulture, $" ON CONFLICT (\"{_idColumnName}\") DO UPDATE SET {updateClauses}");
    ...
    private static bool IsDuplicateKeyException(Exception ex)
    {
        ...
        return message.Contains("duplicate key", StringComparison.OrdinalIgnoreCase)
            ...
            || message.Contains("23505", StringComparison.OrdinalIgnoreCase); // PostgreSQL error code for unique violation
    }
