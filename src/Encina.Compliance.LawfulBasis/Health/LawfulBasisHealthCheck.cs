...
namespace Encina.Compliance.LawfulBasis.Health;
...
/// <list type="bullet">
/// <item><description>The <see cref="ILawfulBasisService"/> is resolvable and populated.</description></item>
/// <item><description>Pending LIA reviews are reported.</description></item>
/// </list>
/// </para>
/// <para>
...
public sealed class LawfulBasisHealthCheck : IHealthCheck
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // 1. Verify service is resolvable
        var service = scopedProvider.GetService<ILawfulBasisService>();
        if (service is null)
        {
            return HealthCheckResult.Unhealthy(
                "ILawfulBasisService is not registered. Call AddEncinaLawfulBasis() in DI setup.",
                data: data);
        }
        ...
        try
        {
            var registrationsResult = await service.GetAllRegistrationsAsync(cancellationToken)
        ...
        // 3. Check pending LIA reviews
        try
        {
            var pendingResult = await service
                .GetPendingLIAReviewsAsync(cancellationToken)
                .ConfigureAwait(false);
