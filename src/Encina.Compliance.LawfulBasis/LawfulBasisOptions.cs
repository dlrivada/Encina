...
///     options.DefaultBasis&lt;GetUserProfileQuery&gt;(GDPR.LawfulBasis.Contract);
/// });
/// </code>
/// </example>
public sealed class LawfulBasisOptions
{
    ...
    /// <summary>
    /// Gets or sets whether all requests decorated with <see cref="GDPR.ProcessesPersonalDataAttribute"/>
    /// or <see cref="GDPR.ProcessingActivityAttribute"/> must have a declared lawful basis.
    /// </summary>
    /// <remarks>
    /// When <c>true</c>, the pipeline behavior requires a <see cref="GDPR.LawfulBasisAttribute"/> or
    /// <see cref="GDPR.ProcessingActivityAttribute"/> with a lawful basis on the request type.
    ...
    /// <summary>
    /// Gets or sets whether to validate active consent when the lawful basis is <see cref="GDPR.LawfulBasis.Consent"/>.
    /// </summary>
    /// <remarks>
    /// When <c>true</c> and an <see cref="GDPR.IConsentStatusProvider"/> is registered, the behavior
    ...
    /// is <see cref="GDPR.LawfulBasis.LegitimateInterests"/>.
    /// </summary>
    /// <remarks>
    /// When <c>true</c>, the behavior validates the LIA reference via <see cref="Abstractions.ILawfulBasisService"/>
    ...
    /// <summary>
    /// Gets or sets whether to automatically register lawful basis from <see cref="GDPR.LawfulBasisAttribute"/>
    /// decorations at startup.
    /// </summary>
    /// <remarks>
    /// When <c>true</c>, the assemblies in <see cref="AssembliesToScan"/> are scanned for
    /// <see cref="GDPR.LawfulBasisAttribute"/> decorations and the registrations are created in the
    /// <see cref="Abstractions.ILawfulBasisService"/>. Default is <c>true</c>.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// When <c>true</c>, a health check is registered that verifies access to the lawful basis
    /// service, counts registrations, and reports pending LIA reviews. Default is <c>false</c>.
    /// </remarks>
    public bool AddHealthCheck { get; set; }

    /// <summary>
    /// Gets the assemblies to scan for <see cref="GDPR.LawfulBasisAttribute"/> decorations.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Only used when <see cref="AutoRegisterFromAttributes"/> is <c>true</c>.
    /// Add assemblies that contain your request types decorated with <see cref="GDPR.LawfulBasisAttribute"/>.
    /// </para>
    /// <para>
    ...
    /// These are registered in the <see cref="Abstractions.ILawfulBasisService"/> at startup alongside
    ...
    /// </para>
    /// </remarks>
    public Dictionary<Type, GDPR.LawfulBasis> DefaultBases { get; } = [];
    ...
    /// <see cref="GDPR.LawfulBasisAttribute"/>. These registrations are applied at startup via the
    /// <see cref="Abstractions.ILawfulBasisService"/>.
    /// </para>
    /// <para>
    /// If the request type also has a <see cref="GDPR.LawfulBasisAttribute"/>, the attribute takes priority.
    /// </para>
    /// </remarks>
    /// <example>
    /// <code>
    ...
    /// {
    ///     options.DefaultBasis&lt;GetUserProfileQuery&gt;(GDPR.LawfulBasis.Contract);
    ///     options.DefaultBasis&lt;AnalyticsCommand&gt;(GDPR.LawfulBasis.LegitimateInterests);
    /// });
    /// </code>
    /// </example>
    public LawfulBasisOptions DefaultBasis<TRequest>(GDPR.LawfulBasis basis)
    {
    ...
    /// <summary>
    /// Adds an assembly to the list of assemblies to scan for <see cref="GDPR.LawfulBasisAttribute"/> decorations.
    /// </summary>
