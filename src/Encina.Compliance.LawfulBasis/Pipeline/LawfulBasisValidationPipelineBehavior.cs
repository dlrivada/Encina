...
/// <remarks>
/// <para>
/// This behavior performs targeted Article 6 validation:
/// </para>
/// <list type="number">
...
/// <item><description>For <see cref="GDPR.LawfulBasis.Consent"/>: validates active consent via <see cref="IConsentStatusProvider"/>.</description></item>
/// <item><description>For <see cref="GDPR.LawfulBasis.LegitimateInterests"/>: validates LIA approval via <see cref="ILawfulBasisService"/>.</description></item>
...
/// <see cref="ProcessingActivityAttribute"/> when both are present.
/// </para>
/// <para>
/// <b>Service fallback:</b> When a request is marked with <see cref="ProcessesPersonalDataAttribute"/>
/// but has no lawful basis attribute, the behavior queries <see cref="ILawfulBasisService"/> for
...
{
    /// <summary>
    /// Static per-generic-type attribute info. Each closed generic type resolves its attribute info
    /// exactly once via the CLR's static field guarantee.
    /// </summary>
    ...
    /// </summary>
    /// <param name="service">The lawful basis service for registration lookups and LIA validation.</param>
    ...
    /// <see cref="GDPR.LawfulBasis.Consent"/>, enforcement mode rules apply.
    /// </param>
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            GDPR.LawfulBasis.Consent when _options.ValidateConsentForConsentBasis
                => await ValidateConsentAsync(
                        request, context, attrInfo, requestType, cancellationToken)
                    .ConfigureAwait(false),

            GDPR.LawfulBasis.LegitimateInterests when _options.ValidateLIAForLegitimateInterests
    ...
    // ================================================================
    // No basis declared — service fallback
    // ================================================================
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Fallback: check the service for programmatically registered bases
        var requestTypeName = requestType.AssemblyQualifiedName ?? requestType.FullName ?? requestType.Name;
        var registrationLookup = await _service
            .GetRegistrationByRequestTypeAsync(requestTypeName, cancellationToken)
            .ConfigureAwait(false);

        var registryBasis = registrationLookup.MatchUnsafe(
            Right: opt => opt.MatchUnsafe(
                Some: reg => (GDPR.LawfulBasis?)reg.Basis,
                None: () => (GDPR.LawfulBasis?)null),
            Left: _ => (GDPR.LawfulBasis?)null);
        ...
        // No basis from attributes or service — enforce
        ...
        CancellationToken cancellationToken)
    {
        ...
        var liaResult = await _service
            .HasApprovedLIAAsync(attrInfo.LIAReference, cancellationToken)
            .ConfigureAwait(false);
    ...
    private void RecordFailed(Activity? activity, Type requestType, GDPR.LawfulBasis? basis, string failureReason)
    {
        LawfulBasisDiagnostics.ValidationsTotal.Add(1, new TagList
        {
            { LawfulBasisDiagnostics.TagBasis, basis?.ToString() ?? "none" },
