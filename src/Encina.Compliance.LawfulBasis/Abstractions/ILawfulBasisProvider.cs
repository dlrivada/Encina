...
/// </para>
/// <list type="number">
/// <item>Resolve the declared lawful basis for a request type from the <see cref="ILawfulBasisService"/></item>
...
public interface ILawfulBasisProvider
{
    ...
    /// <returns>
    /// An <see cref="Option{LawfulBasisReadModel}"/> containing the matching registration if found,
    /// or <see cref="Option{LawfulBasisReadModel}.None"/> if no lawful basis is declared,
    /// or an <see cref="EncinaError"/> on failure.
    /// </returns>
    ValueTask<Either<EncinaError, Option<LawfulBasisReadModel>>> GetBasisForRequestAsync(
        Type requestType,
    ...
    /// </para>
    /// <list type="bullet">
    /// <item>Verifying a lawful basis is declared in the service</item>
    /// <item>For <see cref="GDPR.LawfulBasis.Consent"/>: checking that active consent exists (if consent provider is available)</item>
    /// <item>For <see cref="GDPR.LawfulBasis.LegitimateInterests"/>: verifying LIA reference is present</item>
    /// <item>For <see cref="GDPR.LawfulBasis.LegalObligation"/>: verifying legal reference is present</item>
    /// <item>For <see cref="GDPR.LawfulBasis.Contract"/>: verifying contract reference is present</item>
    /// </list>
    /// </remarks>
