...
namespace Encina.Dapper.Oracle.Tenancy;

/// <summary>
/// Tenant-aware Dapper implementation of <see cref="IFunctionalRepository{TEntity, TId}"/>
/// with automatic tenant filtering, assignment, and validation for Oracle.
/// </summary>
    ...
    where TId : notnull
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(tenantFilter.filter))
            {
                ...
                parameters = new { Id = ConvertIdForOracle(id), tenantId = tenantFilter.tenantId };
            }
            else
            {
                sql = $"SELECT {columns} FROM \"{_mapping.TableName}\" WHERE \"{_mapping.IdColumnName}\" = :Id";
                parameters = new { Id = ConvertIdForOracle(id) };
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var (whereClause, parameters) = _sqlBuilder.BuildWhereClause(specification);
            // Oracle uses CASE WHEN EXISTS with FROM DUAL
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                // Oracle uses CASE WHEN EXISTS with FROM DUAL
                sql = $"SELECT CASE WHEN EXISTS (SELECT 1 FROM \"{_mapping.TableName}\" WHERE {tenantFilter.filter}) THEN 1 ELSE 0 END FROM DUAL";
                parameters = new { tenantId = tenantFilter.tenantId };
            }
            else
            {
                sql = $"SELECT CASE WHEN EXISTS (SELECT 1 FROM \"{_mapping.TableName}\") THEN 1 ELSE 0 END FROM DUAL";
                parameters = null;
            }
