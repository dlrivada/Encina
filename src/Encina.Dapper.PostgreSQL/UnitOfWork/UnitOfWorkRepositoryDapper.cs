...
namespace Encina.Dapper.PostgreSQL.UnitOfWork;

/// <summary>
/// Dapper repository implementation for use within a Unit of Work for PostgreSQL.
/// </summary>
...
/// <list type="bullet">
/// <item><description>Uses double-quoted identifiers for table and column names</description></item>
/// <item><description>Uses LIMIT for row limiting instead of TOP</description></item>
/// <item><description>Uses SELECT EXISTS for boolean existence queries (native boolean support)</description></item>
/// <item><description>Detects PostgreSQL error code 23505 for unique constraint violations</description></item>
/// </list>
/// </para>
/// </remarks>
    ...
    where TId : notnull
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var (whereClause, parameters) = _sqlBuilder.BuildWhereClause(specification);
            // PostgreSQL has native boolean support - SELECT EXISTS returns a proper boolean
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Get paginated items - PostgreSQL uses double quotes and LIMIT/OFFSET
            var columns = string.Join(", ", _mapping.ColumnMappings.Values.Select(c => $"\"{c}\""));
