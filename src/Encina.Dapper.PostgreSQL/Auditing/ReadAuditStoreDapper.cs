...
/// <summary>
/// Dapper implementation of <see cref="IReadAuditStore"/> for PostgreSQL.
/// </summary>
/// <remarks>
/// <para>
...
/// <item><description>CTE with RETURNING for purge affected row count</description></item>
/// <item><description>ILIKE for case-insensitive purpose filtering</description></item>
/// </list>
/// </para>
/// <para>
...
public sealed class ReadAuditStoreDapper : IReadAuditStore
{
    ...
    public ReadAuditStoreDapper(IDbConnection connection, string tableName = "ReadAuditEntries")
    {
        ...
        // Build and cache SQL statements using PostgreSQL syntax (double-quotes for identifiers)
        _insertSql = $@"
        ...
        // PostgreSQL uses CTE with RETURNING to get affected row count
        _purgeSql = $@"
            WITH deleted AS (
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var whereClause = new StringBuilder("WHERE true");
            ...
            {
                whereClause.Append(" AND \"Purpose\" ILIKE '%' || @Purpose || '%'");
            ...
            // Get paginated results using PostgreSQL LIMIT/OFFSET
            var selectSql = $@"
                SELECT ""Id"", ""EntityType"", ""EntityId"", ""UserId"", ""TenantId"", ""AccessedAtUtc"",
                       ""CorrelationId"", ""Purpose"", ""AccessMethod"", ""EntityCount"", ""Metadata""
                FROM ""{_tableName}""
