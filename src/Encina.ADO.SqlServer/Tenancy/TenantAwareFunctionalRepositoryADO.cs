...
/// <summary>
/// Tenant-aware ADO.NET implementation of <see cref="IFunctionalRepository{TEntity, TId}"/>
    ...
    where TId : notnull
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var columns = string.Join(", ", _mapping.ColumnMappings.Values.Select(c => $"[{c}]"));
            var sql = string.IsNullOrWhiteSpace(whereClause)
                ? $"SELECT {columns} FROM {_mapping.TableName} ORDER BY [{_mapping.IdColumnName}] OFFSET @Skip ROWS FETCH NEXT @PageSize ROWS ONLY"
                : $"SELECT {columns} FROM {_mapping.TableName} {whereClause} ORDER BY [{_mapping.IdColumnName}] OFFSET @Skip ROWS FETCH NEXT @PageSize ROWS ONLY";
