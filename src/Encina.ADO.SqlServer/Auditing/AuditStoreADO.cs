...
namespace Encina.ADO.SqlServer.Auditing;

/// <summary>
/// ADO.NET implementation of <see cref="IAuditStore"/> for SQL Server.
/// </summary>
/// <remarks>
/// <para>
/// This implementation uses raw SqlCommand and SqlDataReader for maximum performance.
/// SQL statements use SQL Server-specific syntax:
/// <list type="bullet">
/// <item><description>Square bracket identifier quoting (e.g., [EntityType])</description></item>
/// <item><description>NVARCHAR(MAX) for payload storage</description></item>
...
public sealed class AuditStoreADO : IAuditStore
{
    ...
    public AuditStoreADO(IDbConnection connection, string tableName = "SecurityAuditEntries")
    {
        ...
        // Build and cache SQL statements with square bracket quoting
        _insertSql = $@"
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Get paginated results using OFFSET/FETCH
            var selectSql = $@"
                SELECT [Id], [CorrelationId], [UserId], [TenantId], [Action], [EntityType], [EntityId],
                       [Outcome], [ErrorMessage], [TimestampUtc], [StartedAtUtc], [CompletedAtUtc],
                       [IpAddress], [UserAgent], [RequestPayloadHash], [RequestPayload], [ResponsePayload], [Metadata]
    ...
    private static AuditEntry MapToEntry(IDataReader reader) => new()
    {
        ...
        StartedAtUtc = reader.GetDateTimeOffset(reader.GetOrdinal("StartedAtUtc")),
        CompletedAtUtc = reader.GetDateTimeOffset(reader.GetOrdinal("CompletedAtUtc")),
    ...
    {
        if (reader is SqlDataReader sqlReader)
            return sqlReader.GetDateTimeOffset(ordinal);
...
/// <summary>
/// Extension methods for IDataReader to support DateTimeOffset.
/// </summary>
internal static class DataReaderExtensions
{
    /// <summary>
    /// Gets a DateTimeOffset value from the reader.
    /// </summary>
    public static DateTimeOffset GetDateTimeOffset(this IDataReader reader, int ordinal)
    {
        if (reader is SqlDataReader sqlReader)
            return sqlReader.GetDateTimeOffset(ordinal);
