...
/// </para>
/// <para>
/// For true async operations, the command is checked for <see cref="SqlCommand"/>
/// and native async methods are used when available.
/// </para>
/// </remarks>
/// <example>
/// <code>
    ...
    where TId : notnull
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Get paginated items - SQL Server uses OFFSET...FETCH NEXT (requires ORDER BY)
            var columns = string.Join(", ", _mapping.ColumnMappings.Values.Select(c => $"[{c}]"));
        ...
        if (specification is not Specification<TEntity> baseSpecification)
        {
            return Task.FromResult<Either<EncinaError, PagedResult<TEntity>>>(
                RepositoryErrors.InvalidOperation<TEntity>(
                    "GetPaged",
                    $"IPagedSpecification must inherit from Specification<{typeof(TEntity).Name}> for ADO.NET providers"));
        }
    ...
    private static async Task<bool> ReadAsync(IDataReader reader, CancellationToken cancellationToken)
    {
        if (reader is SqlDataReader sqlReader)
            return await sqlReader.ReadAsync(cancellationToken).ConfigureAwait(false);
