...
/// with correct <see cref="DbType"/> mappings for SQL Server.
/// </summary>
/// <remarks>
/// <para>
/// SQL Server has native support for <c>BIGINT</c>, <c>UNIQUEIDENTIFIER</c>, and
/// <c>NVARCHAR</c> types. These extension methods map each Encina ID type to the most
/// appropriate SQL Server type:
/// <list type="bullet">
///   <item><see cref="SnowflakeId"/> maps to <see cref="DbType.Int64"/> (<c>BIGINT</c>).</item>
///   <item><see cref="UlidId"/> maps to <see cref="DbType.Guid"/> (<c>UNIQUEIDENTIFIER</c>).</item>
///   <item><see cref="UuidV7Id"/> maps to <see cref="DbType.Guid"/> (<c>UNIQUEIDENTIFIER</c>).</item>
///   <item><see cref="ShardPrefixedId"/> maps to <see cref="DbType.String"/> (<c>NVARCHAR</c>).</item>
/// </list>
/// </para>
/// </remarks>
public static class IdParameterExtensions
{
    ...
    /// <remarks>
    /// Maps to SQL Server column type <c>BIGINT</c>. The raw 64-bit value is stored directly.
    /// </remarks>
    ...
    /// <remarks>
    /// Maps to SQL Server column type <c>UNIQUEIDENTIFIER</c>. The ULID is converted
    /// to a <see cref="Guid"/> via <see cref="UlidId.ToGuid()"/> for native 16-byte storage.
    /// </remarks>
    ...
    /// <remarks>
    /// Maps to SQL Server column type <c>UNIQUEIDENTIFIER</c>. The underlying
    /// <see cref="Guid"/> is stored directly in the native 16-byte format.
    /// </remarks>
    ...
    /// <remarks>
    /// Maps to SQL Server column type <c>NVARCHAR</c>. The full shard-prefixed string
