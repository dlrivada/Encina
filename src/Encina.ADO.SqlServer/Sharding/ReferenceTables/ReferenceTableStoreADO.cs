...
/// <summary>
/// SQL Server ADO.NET implementation of <see cref="IReferenceTableStore"/> using
/// <c>MERGE</c> for bulk upsert operations.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// SQL Server has a 2100 parameter limit per statement. The store automatically batches
...
{
    private const int MaxParametersPerStatement = 2100;
        ...
        where TEntity : class
    {
        ...
        var updateSet = string.Join(", ",
            metadata.NonKeyProperties.Select(p => $"target.[{p.ColumnName}] = source.[{p.ColumnName}]"));
        var insertColumns = string.Join(", ", columns);
        var insertValues = string.Join(", ",
            metadata.AllProperties.Select(p => $"source.[{p.ColumnName}]"));

        var sql = $"""
            MERGE INTO [{metadata.TableName}] AS target
            USING (VALUES {string.Join(", ", valueSets)}) AS source ({sourceColumns})
