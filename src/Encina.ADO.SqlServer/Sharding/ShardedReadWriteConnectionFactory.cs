...
/// <summary>
/// SQL Server implementation of sharded read/write connection factories for ADO.NET.
/// </summary>
/// <remarks>
/// <para>
...
/// (returning <see cref="SqlConnection"/>) for maximum flexibility.
/// </para>
/// <para>
...
public sealed class ShardedReadWriteConnectionFactory :
    ...
    IShardedReadWriteConnectionFactory<SqlConnection>
{
    ...
    async Task<Either<EncinaError, SqlConnection>>
        IShardedReadWriteConnectionFactory<SqlConnection>.GetReadConnectionAsync(
        string shardId,
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, SqlConnection>>
        IShardedReadWriteConnectionFactory<SqlConnection>.GetWriteConnectionAsync(
        string shardId,
        CancellationToken cancellationToken)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(shardId);
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, SqlConnection>>
        IShardedReadWriteConnectionFactory<SqlConnection>.GetConnectionAsync(
        string shardId,
        CancellationToken cancellationToken)
    {
        return DatabaseRoutingContext.IsReadIntent
            ? await ((IShardedReadWriteConnectionFactory<SqlConnection>)this)
    ...
    async Task<Either<EncinaError, IReadOnlyDictionary<string, SqlConnection>>>
        IShardedReadWriteConnectionFactory<SqlConnection>.GetAllReadConnectionsAsync(
        CancellationToken cancellationToken)
    {
        return await GetAllConnectionsInternalAsync(
            (shardId, ct) => ((IShardedReadWriteConnectionFactory<SqlConnection>)this)
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, IReadOnlyDictionary<string, SqlConnection>>>
        IShardedReadWriteConnectionFactory<SqlConnection>.GetAllWriteConnectionsAsync(
        CancellationToken cancellationToken)
    {
        return await GetAllConnectionsInternalAsync(
            (shardId, ct) => ((IShardedReadWriteConnectionFactory<SqlConnection>)this)
    ...
    {
        var result = await ((IShardedReadWriteConnectionFactory<SqlConnection>)this)
    ...
    private async Task<Either<EncinaError, IReadOnlyDictionary<string, SqlConnection>>>
        GetAllConnectionsInternalAsync(
        Func<string, CancellationToken, Task<Either<EncinaError, SqlConnection>>> getConnectionAsync,
        CancellationToken cancellationToken)
    {
        ...
        return Either<EncinaError, IReadOnlyDictionary<string, SqlConnection>>.Right(connections);
    }

    private void InitializeSelectors()
    {
        // Create a default selector based on global strategy
        _selectors[string.Empty] = ShardReplicaSelectorFactory.Create(_options.DefaultReplicaStrategy);

        // Create per-shard selectors for shards with strategy overrides
        foreach (var shard in _topology.GetAllShards())
        {
            var strategy = shard.ReplicaStrategy ?? _options.DefaultReplicaStrategy;

            if (shard.ReplicaStrategy.HasValue)
            {
                _selectors[shard.ShardId] = ShardReplicaSelectorFactory.Create(strategy);
            }
        }
    }
}
