...
namespace Encina.ADO.SqlServer.DPIA;

/// <summary>
/// ADO.NET implementation of <see cref="IDPIAStore"/> for SQL Server.
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var sql = $@"
                ...
                WHEN MATCHED THEN
                    UPDATE SET RequestTypeName = @RequestTypeName, StatusValue = @StatusValue, ProcessingType = @ProcessingType,
                               Reason = @Reason, ResultJson = @ResultJson, DPOConsultationJson = @DPOConsultationJson,
                               CreatedAtUtc = @CreatedAtUtc, ApprovedAtUtc = @ApprovedAtUtc, NextReviewAtUtc = @NextReviewAtUtc
                WHEN NOT MATCHED THEN
                    INSERT (Id, RequestTypeName, StatusValue, ProcessingType, Reason, ResultJson, DPOConsultationJson, CreatedAtUtc, ApprovedAtUtc, NextReviewAtUtc)
                    VALUES (@Id, @RequestTypeName, @StatusValue, @ProcessingType, @Reason, @ResultJson, @DPOConsultationJson, @CreatedAtUtc, @ApprovedAtUtc, @NextReviewAtUtc);";
            ...
            AddParameter(command, "@ApprovedAtUtc", entity.ApprovedAtUtc?.UtcDateTime);
            AddParameter(command, "@NextReviewAtUtc", entity.NextReviewAtUtc?.UtcDateTime);
    ...
    private static DPIAAssessmentEntity MapToEntity(IDataReader reader)
    {
        ...
        return new DPIAAssessmentEntity
        {
            ...
            ApprovedAtUtc = reader.IsDBNull(approvedAtUtcOrd)
                ? null
                : new DateTimeOffset(reader.GetDateTime(approvedAtUtcOrd), TimeSpan.Zero),
            NextReviewAtUtc = reader.IsDBNull(nextReviewAtUtcOrd)
                ? null
                : new DateTimeOffset(reader.GetDateTime(nextReviewAtUtcOrd), TimeSpan.Zero)
        };
    }
