...
/// <summary>
/// ADO.NET implementation of <see cref="IDPAStore"/> for MySQL.
...
/// <item><description>Boolean columns (HasSCCs, mandatory terms) are stored as <c>TINYINT(1)</c> and read via <c>GetBoolean</c>.</description></item>
/// <item><description>Processing purposes are stored as <c>JSON</c>.</description></item>
/// </list>
/// </para>
/// </remarks>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var sql = $@"
                ...
                (Id, ProcessorId, StatusValue, SignedAtUtc, ExpiresAtUtc, HasSCCs, ProcessingPurposesJson,
                 ProcessOnDocumentedInstructions, ConfidentialityObligations, SecurityMeasures, SubProcessorRequirements,
                 DataSubjectRightsAssistance, ComplianceAssistance, DataDeletionOrReturn, AuditRights,
                 TenantId, ModuleId, CreatedAtUtc, LastUpdatedAtUtc)
                VALUES
                (@Id, @ProcessorId, @StatusValue, @SignedAtUtc, @ExpiresAtUtc, @HasSCCs, @ProcessingPurposesJson,
                 @ProcessOnDocumentedInstructions, @ConfidentialityObligations, @SecurityMeasures, @SubProcessorRequirements,
                 @DataSubjectRightsAssistance, @ComplianceAssistance, @DataDeletionOrReturn, @AuditRights,
        ...
        catch (Exception ex)
        {
            return Left(EncinaErrors.Create(
                code: "processor.dpa_store_error",
        ...
        try
        {
            ...
            if (rowsAffected == 0)
            {
                return Left(EncinaErrors.Create(
                    ...
                    message: $"DPA '{agreement.Id}' not found.",
                    details: new Dictionary<string, object?> { ["dpaId"] = agreement.Id }));
            }
