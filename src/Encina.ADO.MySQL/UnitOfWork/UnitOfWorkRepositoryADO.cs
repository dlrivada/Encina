...
namespace Encina.ADO.MySQL.UnitOfWork;

/// <summary>
/// ADO.NET repository implementation for use within a Unit of Work for MySQL.
/// </summary>
...
/// <list type="bullet">
/// <item><description>Uses LIMIT for pagination instead of TOP</description></item>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (entity is IVersionedEntity versionedEntity)
            {
                ...
                using var command = CreateCommand(versionedSql);
                ...
                if (rowsAffected == 0)
                {
                    ...
                    using var existsCommand = CreateCommand($"SELECT CASE WHEN EXISTS (SELECT 1 FROM `{_mapping.TableName}` WHERE `{_mapping.IdColumnName}` = @Id) THEN 1 ELSE 0 END");
