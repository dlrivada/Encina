...
/// <summary>
/// MySQL implementation of <see cref="ISchemaIntrospector"/> that reads schema
/// metadata from <c>INFORMATION_SCHEMA</c>.
/// </summary>
internal sealed class MySqlSchemaIntrospector : ISchemaIntrospector
{
    private readonly IShardedConnectionFactory _connectionFactory;

    public MySqlSchemaIntrospector(IShardedConnectionFactory connectionFactory)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Query user tables from INFORMATION_SCHEMA using DATABASE() for current database
