...
/// <summary>
/// MySQL implementation of sharded read/write connection factories for ADO.NET.
/// </summary>
/// <remarks>
/// <para>
...
/// (returning <see cref="MySqlConnection"/>) for maximum flexibility.
/// </para>
/// <para>
...
public sealed class ShardedReadWriteConnectionFactory :
    ...
    IShardedReadWriteConnectionFactory<MySqlConnection>
{
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, MySqlConnection>>
        IShardedReadWriteConnectionFactory<MySqlConnection>.GetReadConnectionAsync(
        string shardId,
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, MySqlConnection>>
        IShardedReadWriteConnectionFactory<MySqlConnection>.GetWriteConnectionAsync(
        string shardId,
        CancellationToken cancellationToken)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(shardId);
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, MySqlConnection>>
        IShardedReadWriteConnectionFactory<MySqlConnection>.GetConnectionAsync(
        string shardId,
        CancellationToken cancellationToken)
    {
        return DatabaseRoutingContext.IsReadIntent
            ? await ((IShardedReadWriteConnectionFactory<MySqlConnection>)this)
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, IReadOnlyDictionary<string, MySqlConnection>>>
        IShardedReadWriteConnectionFactory<MySqlConnection>.GetAllReadConnectionsAsync(
        CancellationToken cancellationToken)
    {
        return await GetAllConnectionsInternalAsync(
            (shardId, ct) => ((IShardedReadWriteConnectionFactory<MySqlConnection>)this)
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, IReadOnlyDictionary<string, MySqlConnection>>>
        IShardedReadWriteConnectionFactory<MySqlConnection>.GetAllWriteConnectionsAsync(
        CancellationToken cancellationToken)
    {
        return await GetAllConnectionsInternalAsync(
            (shardId, ct) => ((IShardedReadWriteConnectionFactory<MySqlConnection>)this)
    ...
    {
        var result = await ((IShardedReadWriteConnectionFactory<MySqlConnection>)this)
    ...
    private async Task<Either<EncinaError, IReadOnlyDictionary<string, MySqlConnection>>>
        GetAllConnectionsInternalAsync(
        Func<string, CancellationToken, Task<Either<EncinaError, MySqlConnection>>> getConnectionAsync,
        CancellationToken cancellationToken)
    {
        var connections = new Dictionary<string, MySqlConnection>();
        ...
        foreach (var shardId in _topology.ActiveShardIds)
        {
            ...
            var matched = result.Match<Either<EncinaError, MySqlConnection>>(
                Right: conn =>
                {
            ...
            if (matched.IsLeft)
            {
                return matched.Match<Either<EncinaError, IReadOnlyDictionary<string, MySqlConnection>>>(
                    Right: _ => throw new InvalidOperationException("Unexpected Right after Left check"),
                    Left: error => error);
            }
        }

        return Either<EncinaError, IReadOnlyDictionary<string, MySqlConnection>>.Right(connections);
    }
