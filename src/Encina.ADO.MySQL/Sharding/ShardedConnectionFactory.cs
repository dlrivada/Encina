...
/// <summary>
/// MySQL implementation of sharded connection factories for ADO.NET.
/// </summary>
/// <remarks>
/// <para>
/// Creates <see cref="MySqlConnection"/> instances routed to the appropriate shard
...
/// and <see cref="IShardedConnectionFactory{TConnection}"/> (returning <see cref="MySqlConnection"/>)
...
/// var factory = serviceProvider.GetRequiredService&lt;IShardedConnectionFactory&lt;MySqlConnection&gt;&gt;();
/// var result = await factory.GetConnectionAsync("shard-0", ct);
/// result.Match(
///     Right: conn => { /* use MySqlConnection */ },
...
public sealed class ShardedConnectionFactory :
    ...
    IShardedConnectionFactory<MySqlConnection>
{
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, MySqlConnection>> IShardedConnectionFactory<MySqlConnection>.GetConnectionAsync(
        string shardId,
    ...
    async Task<Either<EncinaError, IReadOnlyDictionary<string, MySqlConnection>>>
        IShardedConnectionFactory<MySqlConnection>.GetAllConnectionsAsync(
        CancellationToken cancellationToken)
    {
        ...
        {
            var result = await ((IShardedConnectionFactory<MySqlConnection>)this)
        ...
        return Either<EncinaError, IReadOnlyDictionary<string, MySqlConnection>>
            .Right(connections);
    }

    /// <inheritdoc />
    async Task<Either<EncinaError, MySqlConnection>>
        IShardedConnectionFactory<MySqlConnection>.GetConnectionForEntityAsync<TEntity>(
        TEntity entity,
        CancellationToken cancellationToken)
    {
        ...
        return await shardKeyResult
            ...
            .MapAsync(async shardId =>
                await ((IShardedConnectionFactory<MySqlConnection>)this)
                    .GetConnectionAsync(shardId, cancellationToken)
                    .ConfigureAwait(false))
            .BindAsync(x => x)
            .ConfigureAwait(false);
