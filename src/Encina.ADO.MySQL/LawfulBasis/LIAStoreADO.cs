...
/// <summary>
/// ADO.NET implementation of <see cref="ILIAStore"/> for MySQL.
/// </summary>
/// <remarks>
/// <para>
...
public sealed class LIAStoreADO : ILIAStore
{
    ...
    private static void AddLIAParameters(MySqlCommand command, LIARecordEntity entity)
    {
    ...
    private static LIARecordEntity MapToEntity(MySqlDataReader reader) => new()
    {
        ...
        AssessedAtUtc = new DateTimeOffset(reader.GetDateTime(reader.GetOrdinal("AssessedAtUtc")), TimeSpan.Zero),
        ...
        NextReviewAtUtc = reader.IsDBNull(reader.GetOrdinal("NextReviewAtUtc"))
            ? null
            : new DateTimeOffset(reader.GetDateTime(reader.GetOrdinal("NextReviewAtUtc")), TimeSpan.Zero)
    };
}
