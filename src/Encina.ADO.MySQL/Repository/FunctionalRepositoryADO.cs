...
/// <item><description>Uses backticks for identifier quoting</description></item>
/// <item><description>GUIDs are stored as CHAR(36) and converted appropriately</description></item>
/// <item><description>Booleans are stored as TINYINT(1) and converted appropriately</description></item>
    ...
    where TId : notnull
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            countCommand.CommandText = _countSql;
            var countResult = await ExecuteScalarAsync(countCommand, cancellationToken).ConfigureAwait(false);
            var totalCount = Convert.ToInt32(countResult, CultureInfo.InvariantCulture);
        ...
        try
        {
            ...
            if (entity is IVersionedEntity versionedEntity)
            {
                ...
                if (rowsAffected == 0)
                {
                    ...
                    existsCommand.CommandText = $"SELECT CASE WHEN EXISTS (SELECT 1 FROM `{_mapping.TableName}` WHERE `{_mapping.IdColumnName}` = @Id) THEN 1 ELSE 0 END";
    ...
    private static object? ConvertValue(object value, Type targetType)
    {
        ...
        // Handle GUID conversion from MySQL CHAR(36)
        ...
        // Handle boolean conversion from MySQL TINYINT(1)
        // MySQL returns sbyte, int, or long depending on the driver version
    ...
    /// <summary>
    /// Converts an ID value for MySQL storage (GUIDs to strings).
    /// </summary>
    ...
    private static async Task<IDataReader> ExecuteReaderAsync(IDbCommand command, CancellationToken cancellationToken)
    {
        if (command is MySqlCommand mysqlCommand)
            return await mysqlCommand.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
    ...
    {
        if (command is MySqlCommand mysqlCommand)
            return await mysqlCommand.ExecuteScalarAsync(cancellationToken).ConfigureAwait(false);
    ...
    private static async Task<bool> ReadAsync(IDataReader reader, CancellationToken cancellationToken)
    {
        if (reader is MySqlDataReader mysqlReader)
            return await mysqlReader.ReadAsync(cancellationToken).ConfigureAwait(false);
    ...
    {
        // MySQL error code 1062 = ER_DUP_ENTRY (Duplicate entry for key)
