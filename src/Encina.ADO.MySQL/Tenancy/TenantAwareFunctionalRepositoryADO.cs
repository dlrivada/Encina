...
/// <summary>
/// Tenant-aware ADO.NET MySQL implementation of <see cref="IFunctionalRepository{TEntity, TId}"/>
    ...
    where TId : notnull
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var columns = string.Join(", ", _mapping.ColumnMappings.Values.Select(c => $"`{c}`"));
            var sql = string.IsNullOrWhiteSpace(whereClause)
                ? $"SELECT {columns} FROM `{_mapping.TableName}` LIMIT @PageSize OFFSET @Skip"
                : $"SELECT {columns} FROM `{_mapping.TableName}` {whereClause} LIMIT @PageSize OFFSET @Skip";
    ...
    private (string Sql, Action<IDbCommand> AddParameters) BuildSelectByIdWithTenantFilter(TId id)
    {
        ...
        else
        {
            ...
            return (sql, command => AddParameter(command, "@Id", ConvertIdForStorage(id)));
        }
    }
