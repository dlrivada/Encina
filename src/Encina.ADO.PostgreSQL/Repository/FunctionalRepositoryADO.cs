    ...
    where TId : notnull
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Get paginated items - PostgreSQL uses LIMIT/OFFSET
            var columns = string.Join(", ", _mapping.ColumnMappings.Values.Select(c => $"\"{c}\""));
        ...
        try
        {
            ...
            if (entity is IVersionedEntity versionedEntity)
            {
                ...
                if (rowsAffected == 0)
                {
                    ...
                    existsCommand.CommandText = $"SELECT EXISTS (SELECT 1 FROM \"{_mapping.TableName}\" WHERE \"{_mapping.IdColumnName}\" = @Id)";
                    AddParameter(existsCommand, "@Id", id);

                    var exists = Convert.ToBoolean(await ExecuteScalarAsync(existsCommand, cancellationToken).ConfigureAwait(false), CultureInfo.InvariantCulture);
