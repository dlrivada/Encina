...
/// <summary>
/// ADO.NET implementation of <see cref="IBreachRecordStore"/> for PostgreSQL.
...
public sealed class BreachRecordStoreADO : IBreachRecordStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            while (await ReadAsync(reader, cancellationToken))
            {
                ...
                var detectedAtUtc = new DateTimeOffset(
                    reader.GetDateTime(reader.GetOrdinal("DetectedAtUtc")), TimeSpan.Zero);
                var deadlineUtc = new DateTimeOffset(
                    reader.GetDateTime(reader.GetOrdinal("NotificationDeadlineUtc")), TimeSpan.Zero);
    ...
    private static void AddBreachRecordParameters(IDbCommand command, BreachRecordEntity entity)
    {
        ...
        AddParameter(command, "@NotifiedAuthorityAtUtc", entity.NotifiedAuthorityAtUtc.HasValue ? entity.NotifiedAuthorityAtUtc.Value.UtcDateTime : DBNull.Value);
        AddParameter(command, "@NotifiedSubjectsAtUtc", entity.NotifiedSubjectsAtUtc.HasValue ? entity.NotifiedSubjectsAtUtc.Value.UtcDateTime : DBNull.Value);
        ...
        AddParameter(command, "@ResolvedAtUtc", entity.ResolvedAtUtc.HasValue ? entity.ResolvedAtUtc.Value.UtcDateTime : DBNull.Value);
