...
/// <summary>
/// PostgreSQL ADO.NET implementation of <see cref="IFunctionalShardedRepository{TEntity, TId}"/>
    ...
    where TId : notnull
{
    ...
    private readonly IShardedConnectionFactory<NpgsqlConnection> _connectionFactory;
    ...
    public FunctionalShardedRepositoryADO(
        ...
        IShardedConnectionFactory<NpgsqlConnection> connectionFactory,
    ...
    private async Task<List<TEntity>> ReadEntitiesAsync(IDbCommand command, CancellationToken cancellationToken)
    {
        ...
        await using var reader = await ((NpgsqlCommand)command).ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
