...
/// <summary>
/// PostgreSQL implementation of sharded connection factories for ADO.NET.
/// </summary>
/// <remarks>
/// <para>
/// Creates <see cref="NpgsqlConnection"/> instances routed to the appropriate shard
...
/// and <see cref="IShardedConnectionFactory{TConnection}"/> (returning <see cref="NpgsqlConnection"/>)
...
/// var factory = serviceProvider.GetRequiredService&lt;IShardedConnectionFactory&lt;NpgsqlConnection&gt;&gt;();
/// var result = await factory.GetConnectionAsync("shard-0", ct);
/// result.Match(
///     Right: conn => { /* use NpgsqlConnection */ },
...
public sealed class ShardedConnectionFactory :
    ...
    IShardedConnectionFactory<NpgsqlConnection>
{
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, NpgsqlConnection>> IShardedConnectionFactory<NpgsqlConnection>.GetConnectionAsync(
        string shardId,
    ...
    async Task<Either<EncinaError, IReadOnlyDictionary<string, NpgsqlConnection>>>
        IShardedConnectionFactory<NpgsqlConnection>.GetAllConnectionsAsync(
        CancellationToken cancellationToken)
    {
        ...
        {
            var result = await ((IShardedConnectionFactory<NpgsqlConnection>)this)
        ...
        return Either<EncinaError, IReadOnlyDictionary<string, NpgsqlConnection>>
            .Right(connections);
    }

    /// <inheritdoc />
    async Task<Either<EncinaError, NpgsqlConnection>>
        IShardedConnectionFactory<NpgsqlConnection>.GetConnectionForEntityAsync<TEntity>(
        TEntity entity,
        CancellationToken cancellationToken)
    {
        ...
        return await shardKeyResult
            ...
            .MapAsync(async shardId =>
                await ((IShardedConnectionFactory<NpgsqlConnection>)this)
                    .GetConnectionAsync(shardId, cancellationToken)
                    .ConfigureAwait(false))
            .BindAsync(x => x)
            .ConfigureAwait(false);
