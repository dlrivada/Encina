...
/// <summary>
/// PostgreSQL ADO.NET implementation of <see cref="IReferenceTableStore"/> using
/// <c>INSERT ... ON CONFLICT DO UPDATE</c> for bulk upsert operations.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// PostgreSQL supports up to 65535 parameters per statement. The store batches
...
public sealed class ReferenceTableStoreADO(IDbConnection connection) : IReferenceTableStore, IDisposable
{
        ...
        where TEntity : class
    {
        ...
        var updateSet = string.Join(", ",
            metadata.NonKeyProperties.Select(p => $"\"{p.ColumnName}\" = EXCLUDED.\"{p.ColumnName}\""));
