...
/// <summary>
/// PostgreSQL implementation of sharded read/write connection factories for ADO.NET.
/// </summary>
/// <remarks>
/// <para>
...
/// (returning <see cref="NpgsqlConnection"/>) for maximum flexibility.
/// </para>
/// <para>
...
public sealed class ShardedReadWriteConnectionFactory :
    ...
    IShardedReadWriteConnectionFactory<NpgsqlConnection>
{
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, NpgsqlConnection>>
        IShardedReadWriteConnectionFactory<NpgsqlConnection>.GetReadConnectionAsync(
        string shardId,
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, NpgsqlConnection>>
        IShardedReadWriteConnectionFactory<NpgsqlConnection>.GetWriteConnectionAsync(
        string shardId,
        CancellationToken cancellationToken)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(shardId);
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, NpgsqlConnection>>
        IShardedReadWriteConnectionFactory<NpgsqlConnection>.GetConnectionAsync(
        string shardId,
        CancellationToken cancellationToken)
    {
        return DatabaseRoutingContext.IsReadIntent
            ? await ((IShardedReadWriteConnectionFactory<NpgsqlConnection>)this)
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, IReadOnlyDictionary<string, NpgsqlConnection>>>
        IShardedReadWriteConnectionFactory<NpgsqlConnection>.GetAllReadConnectionsAsync(
        CancellationToken cancellationToken)
    {
        return await GetAllConnectionsInternalAsync(
            (shardId, ct) => ((IShardedReadWriteConnectionFactory<NpgsqlConnection>)this)
    ...
    /// <inheritdoc />
    async Task<Either<EncinaError, IReadOnlyDictionary<string, NpgsqlConnection>>>
        IShardedReadWriteConnectionFactory<NpgsqlConnection>.GetAllWriteConnectionsAsync(
        CancellationToken cancellationToken)
    {
        return await GetAllConnectionsInternalAsync(
            (shardId, ct) => ((IShardedReadWriteConnectionFactory<NpgsqlConnection>)this)
    ...
    {
        var result = await ((IShardedReadWriteConnectionFactory<NpgsqlConnection>)this)
    ...
    private async Task<Either<EncinaError, IReadOnlyDictionary<string, NpgsqlConnection>>>
        GetAllConnectionsInternalAsync(
        Func<string, CancellationToken, Task<Either<EncinaError, NpgsqlConnection>>> getConnectionAsync,
        CancellationToken cancellationToken)
    {
        ...
        foreach (var shardId in _topology.ActiveShardIds)
        {
            ...
            var matched = result.Match<Either<EncinaError, NpgsqlConnection>>(
                Right: conn =>
                {
            ...
            if (matched.IsLeft)
            {
                return matched.Match<Either<EncinaError, IReadOnlyDictionary<string, NpgsqlConnection>>>(
                    Right: _ => throw new InvalidOperationException("Unexpected Right after Left check"),
                    Left: error => error);
            }
        }

        return Either<EncinaError, IReadOnlyDictionary<string, NpgsqlConnection>>.Right(connections);
    }
