...
/// <summary>
/// Tenant-aware ADO.NET PostgreSQL implementation of <see cref="IFunctionalRepository{TEntity, TId}"/>
    ...
    where TId : notnull
{
            ...
            if (entity is IVersionedEntity versionedEntity)
            {
                ...
                if (rowsAffected == 0)
                {
                    ...
                    var existsSql = !string.IsNullOrEmpty(tenantFilter) && _mapping.IsTenantEntity
                        ? $"SELECT EXISTS (SELECT 1 FROM \"{_mapping.TableName}\" WHERE \"{_mapping.IdColumnName}\" = @Id AND {tenantFilter})"
                    ...
                    var exists = await ExecuteScalarAsync(existsCommand, cancellationToken).ConfigureAwait(false);
            ...
            if (hasVersionedEntities)
            {
                ...
                foreach (var entity in entityList)
                {
                    if (entity is IVersionedEntity versionedEntity)
                    {
                        ...
                        command.CommandText = versionedSql;
                        AddEntityParameters(command, entity, forInsert: false);
                        AddParameter(command, "@Id", _mapping.GetId(entity));
                        AddParameter(command, "@OriginalVersion", originalVersion);
    ...
    private static object? ConvertValue(object value, Type targetType)
    {
        ...
        // Handle Guid conversion for PostgreSQL (stored as uuid type, maps to Guid directly)
    ...
    private static bool IsDuplicateKeyException(Exception ex)
    {
        ...
        // PostgreSQL specific duplicate key error messages
        return message.Contains("duplicate key value violates unique constraint", StringComparison.OrdinalIgnoreCase)
            ...
            || message.Contains("23505", StringComparison.OrdinalIgnoreCase); // PostgreSQL error code for unique_violation
    }
