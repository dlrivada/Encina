...
/// <summary>
/// Decorator that adds resilience (retry, circuit breaker, timeout) to any <see cref="ISecretWriter"/> implementation.
/// </summary>
/// <remarks>
/// <para>
/// Wraps secret write operations with a Polly <see cref="ResiliencePipeline"/> that applies
...
/// </para>
/// </remarks>
public sealed class ResilientSecretWriterDecorator : ISecretWriter
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ResilientSecretWriterDecorator"/> class.
    /// </summary>
    ...
    public ResilientSecretWriterDecorator(
        ISecretWriter inner,
        ResiliencePipeline pipeline,
        SecretsResilienceOptions options,
        ILogger<ResilientSecretWriterDecorator> logger,
    ...
    {
        using var activity = SecretsActivitySource.StartResilienceActivity(secretName, "set");
        ...
        try
        {
            ...
            {
                var innerResult = await _inner.SetSecretAsync(secretName, value, ct).ConfigureAwait(false);
