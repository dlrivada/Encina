...
/// with automatic tenant filtering, assignment, and validation for MySQL.
/// </summary>
...
/// </list>
/// <para>
/// <b>MySQL-specific features:</b>
/// <list type="bullet">
/// <item>Uses backticks for identifiers: `ColumnName`</item>
/// <item>Uses LIMIT 1 instead of TOP 1</item>
/// <item>Duplicate key detection: "Duplicate entry" or MySQL error code 1062</item>
/// </list>
/// </para>
/// </remarks>
/// <example>
    ...
    where TId : notnull
{
        ...
        TimeProvider? timeProvider = null)
    {
        ...
        // Ensure GUID type handler is registered for MySQL
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(tenantFilter.filter))
            {
                ...
                parameters = new { Id = ConvertIdForParameter(id), tenantId = tenantFilter.tenantId };
            }
            else
            {
                sql = $"SELECT {columns} FROM `{_mapping.TableName}` WHERE `{_mapping.IdColumnName}` = @Id";
                parameters = new { Id = ConvertIdForParameter(id) };
            }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(tenantFilter.filter))
            {
                sql = $"SELECT EXISTS (SELECT 1 FROM `{_mapping.TableName}` WHERE {tenantFilter.filter})";
                parameters = new { tenantId = tenantFilter.tenantId };
            }
            else
            {
                sql = $"SELECT EXISTS (SELECT 1 FROM `{_mapping.TableName}`)";
                parameters = null;
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(tenantFilter.filter))
            {
                sql = $"SELECT {columns} FROM `{_mapping.TableName}` WHERE {tenantFilter.filter} LIMIT @PageSize OFFSET @Skip";
                parameters = new { tenantId = tenantFilter.tenantId, pagination.PageSize, pagination.Skip };
            }
            else
            {
                sql = $"SELECT {columns} FROM `{_mapping.TableName}` LIMIT @PageSize OFFSET @Skip";
                parameters = new { pagination.PageSize, pagination.Skip };
            }
        ...
        try
        {
            ...
            if (entity is IVersionedEntity versionedEntity)
            {
                ...
                if (!string.IsNullOrEmpty(tenantFilter.filter) && _mapping.IsTenantEntity)
                {
                    ...
                    parameters["tenantId"] = tenantFilter.tenantId;
                }
                ...
                if (rowsAffected == 0)
                {
                    ...
                    var existsSql = !string.IsNullOrEmpty(tenantFilter.filter) && _mapping.IsTenantEntity
                        ? $"SELECT EXISTS (SELECT 1 FROM `{_mapping.TableName}` WHERE `{_mapping.IdColumnName}` = @Id AND {tenantFilter.filter})"
                    ...
                    var existsParams = !string.IsNullOrEmpty(tenantFilter.filter) && _mapping.IsTenantEntity
                        ? new { Id = ConvertIdForParameter(id), tenantId = tenantFilter.tenantId }
                        : (object)new { Id = ConvertIdForParameter(id) };
            ...
            if (!string.IsNullOrEmpty(tenantFilter.filter) && _mapping.IsTenantEntity)
            {
                ...
                var parameters = new DynamicParameters(BuildEntityParameters(entity));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var parameterList = entityList.Select(BuildEntityParameters);
            await _connection.ExecuteAsync(_insertSql, parameterList);
    ...
    /// <summary>
    /// Builds a dictionary of parameters for an entity, converting GUIDs to strings for MySQL.
    /// </summary>
    private Dictionary<string, object?> BuildEntityParameters(TEntity entity)
    {
        ...
        foreach (var (propertyName, columnName) in _mapping.ColumnMappings)
        {
            ...
            // Convert Guid to string for MySQL CHAR(36) storage
            if (value is Guid guidValue)
            {
                parameters[propertyName] = guidValue.ToString();
            }
            else
            {
    ...
    /// <summary>
    /// Converts an ID value to the appropriate parameter type for MySQL.
    /// GUIDs are converted to strings for CHAR(36) storage.
    /// </summary>
    private static object ConvertIdForParameter(TId id)
    {
    ...
    private static bool IsDuplicateKeyException(Exception ex)
    {
        ...
        // MySQL duplicate key error messages
