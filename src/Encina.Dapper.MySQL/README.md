...
MySQL/MariaDB implementation of Encina messaging patterns using Dapper, including Outbox, Inbox, Saga orchestration, and Scheduled messages.
...
```bash
dotnet add package Encina.Dapper.MySQL
```

> **Note**: This package is specifically for MySQL/MariaDB. For other databases, see:
>
...
```bash
# In your database
mysql -S localhost -d YourDatabase -i .github/scripts/001_CreateOutboxMessagesTable.sql
mysql -S localhost -d YourDatabase -i .github/scripts/002_CreateInboxMessagesTable.sql
mysql -S localhost -d YourDatabase -i .github/scripts/003_CreateSagaStatesTable.sql
mysql -S localhost -d YourDatabase -i .github/scripts/004_CreateScheduledMessagesTable.sql
```
...
builder.Services.AddEncinaDapper(
...
All patterns work identically to Encina.EntityFrameworkCore. See the [EntityFrameworkCore README](../Encina.EntityFrameworkCore/README.md) for detailed examples.
...
3. **SQL Scripts Required**: No automatic migrations
   ...
   mysql -i .github/scripts/000_CreateAllTables.sql
   ```
...
*Benchmark: .NET 10, MySQL/MariaDB 2022, 1,000 inserts with transaction*
...
```bash
dotnet remove package Encina.EntityFrameworkCore
dotnet add package Encina.Dapper
```
...
4. **Monitor Query Performance**: Use MySQL/MariaDB profiler
