...
/// with Railway Oriented Programming error handling for MySQL.
/// </summary>
...
/// <item><description>GUIDs are stored as CHAR(36) and handled by <see cref="GuidTypeHandler"/></description></item>
/// <item><description>Booleans are stored as TINYINT(1) - Dapper handles this automatically</description></item>
/// </list>
/// </para>
/// <para>
    ...
    where TId : notnull
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var entity = await _connection.QuerySingleOrDefaultAsync<TEntity>(
                _selectByIdSql,
                new { Id = ConvertIdForParameter(id) });
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Get paginated items with specification filter
            var columns = string.Join(", ", _mapping.ColumnMappings.Values.Select(c => $"`{c}`"));
            var selectSql = $"SELECT {columns} FROM `{_mapping.TableName}` {whereClause} LIMIT @PageSize OFFSET @Skip";

            // Merge pagination parameters with specification parameters
            var dynamicParams = new DynamicParameters(parameters);
            dynamicParams.Add("PageSize", pagination.PageSize);
            dynamicParams.Add("Skip", pagination.Skip);

            var entities = await _connection.QueryAsync<TEntity>(selectSql, dynamicParams);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return Left<EncinaError, PagedResult<TEntity>>(
                RepositoryErrors.InvalidOperation<TEntity>("GetPaged",
                    $"The specification {specification.GetType().Name} must inherit from Specification<{typeof(TEntity).Name}>."));
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var parameters = BuildEntityParameters(entity, excludeInsert: true);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var parameterList = entityList.Select(e => BuildEntityParameters(e, excludeInsert: true));
    ...
    /// </summary>
    private Dictionary<string, object?> BuildEntityParameters(TEntity entity, bool excludeInsert = false, bool excludeUpdate = false)
    {
