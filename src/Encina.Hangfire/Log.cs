...
internal static partial class Log
{
    ...
    [LoggerMessage(EventId = 4002, Level = LogLevel.Error, Message = "Hangfire job failed for request {RequestType} with error code {ErrorCode} ({Classification}): {ErrorMessage}")]
    public static partial void RequestJobFailed(ILogger logger, string requestType, string errorCode, string classification, string errorMessage);
    ...
    [LoggerMessage(EventId = 4007, Level = LogLevel.Error, Message = "Hangfire notification job failed for {NotificationType} with error code {ErrorCode} ({Classification}): {ErrorMessage}")]
    public static partial void NotificationJobFailed(ILogger logger, string notificationType, string errorCode, string classification, string errorMessage);
