...
namespace Encina.ADO.Sqlite.BreachNotification;

/// <summary>
/// ADO.NET implementation of <see cref="IBreachRecordStore"/> for SQLite.
...
public sealed class BreachRecordStoreADO : IBreachRecordStore
{
        ...
        try
        {
            ...
            while (await ReadAsync(reader, cancellationToken))
            {
                ...
                var detectedAtUtc = DateTimeOffset.Parse(
                    reader.GetString(reader.GetOrdinal("DetectedAtUtc")), null, DateTimeStyles.RoundtripKind);
                var deadlineUtc = DateTimeOffset.Parse(
                    reader.GetString(reader.GetOrdinal("NotificationDeadlineUtc")), null, DateTimeStyles.RoundtripKind);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            AddParameter(command, "@SubmittedAtUtc", entity.SubmittedAtUtc.ToString("O"));
    ...
    private static void AddBreachRecordParameters(IDbCommand command, BreachRecordEntity entity)
    {
        ...
        AddParameter(command, "@DetectedAtUtc", entity.DetectedAtUtc.ToString("O"));
        AddParameter(command, "@NotificationDeadlineUtc", entity.NotificationDeadlineUtc.ToString("O"));
        AddParameter(command, "@NotifiedAuthorityAtUtc", entity.NotifiedAuthorityAtUtc?.ToString("O"));
        AddParameter(command, "@NotifiedSubjectsAtUtc", entity.NotifiedSubjectsAtUtc?.ToString("O"));
        ...
        AddParameter(command, "@ResolvedAtUtc", entity.ResolvedAtUtc?.ToString("O"));
    ...
    private static BreachRecordEntity MapToBreachRecordEntity(IDataReader reader)
    {
        ...
        return new BreachRecordEntity
        {
            ...
            DetectedAtUtc = DateTimeOffset.Parse(reader.GetString(reader.GetOrdinal("DetectedAtUtc")), null, DateTimeStyles.RoundtripKind),
            NotificationDeadlineUtc = DateTimeOffset.Parse(reader.GetString(reader.GetOrdinal("NotificationDeadlineUtc")), null, DateTimeStyles.RoundtripKind),
            NotifiedAuthorityAtUtc = reader.IsDBNull(notifiedAuthorityOrd)
                ? null
                : DateTimeOffset.Parse(reader.GetString(notifiedAuthorityOrd), null, DateTimeStyles.RoundtripKind),
            NotifiedSubjectsAtUtc = reader.IsDBNull(notifiedSubjectsOrd)
                ? null
                : DateTimeOffset.Parse(reader.GetString(notifiedSubjectsOrd), null, DateTimeStyles.RoundtripKind),
            ...
            ResolvedAtUtc = reader.IsDBNull(resolvedAtOrd)
                ? null
                : DateTimeOffset.Parse(reader.GetString(resolvedAtOrd), null, DateTimeStyles.RoundtripKind),
    ...
    private static PhasedReportEntity MapToPhasedReportEntity(IDataReader reader)
    {
        ...
        return new PhasedReportEntity
        {
            ...
            SubmittedAtUtc = DateTimeOffset.Parse(reader.GetString(reader.GetOrdinal("SubmittedAtUtc")), null, DateTimeStyles.RoundtripKind),
