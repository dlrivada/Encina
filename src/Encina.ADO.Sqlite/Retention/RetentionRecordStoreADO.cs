...
/// <summary>
/// ADO.NET implementation of <see cref="IRetentionRecordStore"/> for SQLite.
...
public sealed class RetentionRecordStoreADO : IRetentionRecordStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            AddParameter(command, "@DeletedAtUtc", entity.DeletedAtUtc?.ToString("O"));
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            command.CommandText = sql;
            AddParameter(command, "@NowUtc", nowUtc.ToString("O"));
            AddParameter(command, "@WindowEnd", windowEnd.ToString("O"));
            AddParameter(command, "@ActiveStatus", (int)RetentionStatus.Active);
    ...
    private static RetentionRecordEntity MapToEntity(IDataReader reader)
    {
        ...
        return new RetentionRecordEntity
        {
            ...
            ExpiresAtUtc = DateTimeOffset.Parse(reader.GetString(reader.GetOrdinal("ExpiresAtUtc")), null, DateTimeStyles.RoundtripKind),
            StatusValue = reader.GetInt32(reader.GetOrdinal("StatusValue")),
            DeletedAtUtc = reader.IsDBNull(deletedAtOrd)
                ? null
                : DateTimeOffset.Parse(reader.GetString(deletedAtOrd), null, DateTimeStyles.RoundtripKind),
