...
namespace Encina.ADO.Sqlite.Retention;

/// <summary>
/// ADO.NET implementation of <see cref="ILegalHoldStore"/> for SQLite.
...
public sealed class LegalHoldStoreADO : ILegalHoldStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            AddParameter(command, "@AppliedAtUtc", entity.AppliedAtUtc.ToString("O"));
            AddParameter(command, "@ReleasedAtUtc", entity.ReleasedAtUtc?.ToString("O"));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            command.CommandText = sql;
            AddParameter(command, "@Id", holdId);
            AddParameter(command, "@ReleasedAtUtc", releasedAtUtc.ToString("O"));
    ...
    private static LegalHoldEntity MapToEntity(IDataReader reader)
    {
        ...
        return new LegalHoldEntity
        {
            ...
            AppliedAtUtc = DateTimeOffset.Parse(reader.GetString(reader.GetOrdinal("AppliedAtUtc")), null, DateTimeStyles.RoundtripKind),
            ReleasedAtUtc = reader.IsDBNull(releasedAtOrd)
                ? null
                : DateTimeOffset.Parse(reader.GetString(releasedAtOrd), null, DateTimeStyles.RoundtripKind),
