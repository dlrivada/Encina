...
/// <summary>
/// ADO.NET implementation of <see cref="IProcessorRegistry"/> for SQLite.
...
public sealed class ProcessorRegistryADO : IProcessorRegistry
{
        ...
        try
        {
            if (processor.Depth > MaxSubProcessorDepth)
            {
                return Left(EncinaErrors.Create(
                    code: "processor_agreements.depth_exceeded",
                    message: $"Processor depth {processor.Depth} exceeds maximum allowed depth of {MaxSubProcessorDepth}.",
            ...
            AddParameter(existsCommand, "@Id", processor.Id);
            ...
            if (count > 0)
            {
                return Left(EncinaErrors.Create(
                    ...
                    message: $"Processor with ID '{processor.Id}' already exists.",
        ...
        try
        {
            ...
            if (count == 0)
            {
                return Left(EncinaErrors.Create(
                    ...
                    message: $"Processor with ID '{processor.Id}' not found.",
                    details: new Dictionary<string, object?> { ["processorId"] = processor.Id }));
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            using var existsCommand = _connection.CreateCommand();
            existsCommand.CommandText = existsSql;
            AddParameter(existsCommand, "@Id", processorId);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            // Load all processors and perform BFS in memory bounded by MaxSubProcessorDepth
            var sql = $@"
                SELECT Id, Name, Country, ContactEmail, ParentProcessorId, Depth, SubProcessorAuthorizationTypeValue, TenantId, ModuleId, CreatedAtUtc, LastUpdatedAtUtc
                FROM {_tableName}";
            ...
            // Build parent-to-children lookup
    ...
    private static ProcessorEntity MapToProcessorEntity(IDataReader reader)
    {
        ...
        return new ProcessorEntity
        {
            ...
            Depth = Convert.ToInt32(reader.GetValue(reader.GetOrdinal("Depth")), CultureInfo.InvariantCulture),
            SubProcessorAuthorizationTypeValue = Convert.ToInt32(reader.GetValue(reader.GetOrdinal("SubProcessorAuthorizationTypeValue")), CultureInfo.InvariantCulture),
