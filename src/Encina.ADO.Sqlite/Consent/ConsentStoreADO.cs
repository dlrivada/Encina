...
/// <summary>
/// ADO.NET implementation of <see cref="IConsentStore"/> for SQLite.
...
public sealed class ConsentStoreADO : IConsentStore
{
    ...
    {
        AddParameter(command, "@Id", consent.Id.ToString());
        ...
        AddParameter(command, "@GivenAtUtc", consent.GivenAtUtc.ToString("O"));
        AddParameter(command, "@WithdrawnAtUtc", consent.WithdrawnAtUtc?.ToString("O"));
        AddParameter(command, "@ExpiresAtUtc", consent.ExpiresAtUtc?.ToString("O"));
    ...
    private static ConsentRecord MapToConsentRecord(IDataReader reader)
    {
        ...
        return new ConsentRecord
        {
            ...
            GivenAtUtc = DateTimeOffset.Parse(reader.GetString(reader.GetOrdinal("GivenAtUtc")), null, DateTimeStyles.RoundtripKind),
            WithdrawnAtUtc = reader.IsDBNull(reader.GetOrdinal("WithdrawnAtUtc"))
                ? null
                : DateTimeOffset.Parse(reader.GetString(reader.GetOrdinal("WithdrawnAtUtc")), null, DateTimeStyles.RoundtripKind),
