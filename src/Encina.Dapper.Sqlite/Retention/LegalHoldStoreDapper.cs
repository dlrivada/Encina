...
/// <summary>
/// Dapper implementation of <see cref="ILegalHoldStore"/> for SQLite.
...
public sealed class LegalHoldStoreDapper : ILegalHoldStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await _connection.ExecuteAsync(sql, new
            {
                ...
                AppliedAtUtc = entity.AppliedAtUtc.ToString("O"),
                ReleasedAtUtc = entity.ReleasedAtUtc?.ToString("O"),
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                Id = holdId,
                ReleasedAtUtc = releasedAtUtc.ToString("O"),
                ReleasedByUserId = releasedByUserId
            });
    ...
    private static LegalHoldEntity MapToEntity(dynamic row)
    {
        return new LegalHoldEntity
        {
            ...
            AppliedAtUtc = DateTimeOffset.Parse((string)row.AppliedAtUtc, null, DateTimeStyles.RoundtripKind),
            ReleasedAtUtc = row.ReleasedAtUtc is null or DBNull
                ? null
                : DateTimeOffset.Parse((string)row.ReleasedAtUtc, null, DateTimeStyles.RoundtripKind),
