...
namespace Encina.Dapper.Sqlite.ReadWriteSeparation;

/// <summary>
/// SQLite implementation of <see cref="IReadWriteConnectionFactory"/> that routes
/// connections to the primary database or read replicas based on routing context.
/// </summary>
/// <remarks>
/// <para>
/// This factory creates <see cref="SqliteConnection"/> instances configured for either
...
///     config.ReadWriteSeparationOptions.WriteConnectionString = "Data Source=primary.db";
///     config.ReadWriteSeparationOptions.ReadConnectionStrings.Add("Data Source=replica.db");
/// });
///
/// // Usage
...
public sealed class ReadWriteConnectionFactory : IReadWriteConnectionFactory
{
    ...
    public async ValueTask<IDbConnection> CreateWriteConnectionAsync(CancellationToken cancellationToken = default)
    {
        ...
        var connection = (SqliteConnection)CreateWriteConnection();
    ...
    public async ValueTask<IDbConnection> CreateReadConnectionAsync(CancellationToken cancellationToken = default)
    {
        ...
        var connection = (SqliteConnection)CreateReadConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        return connection;
    }
