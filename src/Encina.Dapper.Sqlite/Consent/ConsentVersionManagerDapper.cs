...
/// <summary>
/// Dapper implementation of <see cref="IConsentVersionManager"/> for SQLite.
...
public sealed class ConsentVersionManagerDapper : IConsentVersionManager
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            return Right(new ConsentVersion
            {
                ...
                EffectiveFromUtc = DateTimeOffset.Parse((string)row.EffectiveFromUtc, null, DateTimeStyles.RoundtripKind),
                Description = (string)row.Description,
                RequiresExplicitReconsent = Convert.ToInt64(row.RequiresExplicitReconsent) != 0
            });
        }
        catch (Exception ex)
        {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await _connection.ExecuteAsync(insertSql, new
            {
                version.VersionId,
                version.Purpose,
                EffectiveFromUtc = version.EffectiveFromUtc.ToString("O"),
                version.Description,
                RequiresExplicitReconsent = version.RequiresExplicitReconsent ? 1 : 0
            });
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var versionRows = await _connection.QueryAsync(versionSql, new { Purpose = purpose });
            ...
            var requiresReconsent = Convert.ToInt64(versionRow.RequiresExplicitReconsent) != 0;
