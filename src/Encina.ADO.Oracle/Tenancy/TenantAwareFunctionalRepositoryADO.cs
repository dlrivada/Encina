...
/// <summary>
/// Tenant-aware ADO.NET Oracle implementation of <see cref="IFunctionalRepository{TEntity, TId}"/>
    ...
    where TId : notnull
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Oracle uses FETCH FIRST 1 ROW ONLY instead of LIMIT 1
            var sql = $"SELECT {columns} FROM \"{_mapping.TableName}\" {whereClause} FETCH FIRST 1 ROW ONLY";
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // Oracle requires FROM DUAL for SELECT without a table
            var sql = string.IsNullOrWhiteSpace(whereClause)
                ? $"SELECT CASE WHEN EXISTS (SELECT 1 FROM \"{_mapping.TableName}\") THEN 1 ELSE 0 END FROM DUAL"
                : $"SELECT CASE WHEN EXISTS (SELECT 1 FROM \"{_mapping.TableName}\" {whereClause}) THEN 1 ELSE 0 END FROM DUAL";
    ...
    private (string filter, string? tenantId) GetTenantFilter()
    {
        ...
        // Oracle uses double quotes for identifier quoting and colon for parameters
