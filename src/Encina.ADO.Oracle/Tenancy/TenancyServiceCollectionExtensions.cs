...
namespace Encina.ADO.Oracle.Tenancy;

/// <summary>
/// Extension methods for configuring Encina ADO.NET Oracle with multi-tenancy support.
/// </summary>
public static class TenancyServiceCollectionExtensions
{
    /// <summary>
    /// Adds Encina messaging patterns with ADO.NET Oracle persistence and multi-tenancy support.
    /// </summary>
    ...
    /// <remarks>
    /// <para>
    /// This method registers all necessary services for multi-tenant ADO.NET Oracle operations:
    /// </para>
    /// <list type="bullet">
    /// <item><see cref="ITenantConnectionFactory"/> for tenant-aware connection routing</item>
    /// <item><see cref="ADOTenancyOptions"/> for configuring tenant filtering behavior</item>
    /// <item>Standard ADO.NET messaging stores (Outbox, Inbox)</item>
    /// </list>
    /// </remarks>
    /// <example>
    /// <code>
    ...
    /// </code>
    /// </example>
    public static IServiceCollection AddEncinaADOOracleWithTenancy(
        this IServiceCollection services,
        Action<MessagingConfiguration> configure,
        Action<ADOTenancyOptions>? configureTenancy = null)
    {
        ArgumentNullException.ThrowIfNull(services);
        ArgumentNullException.ThrowIfNull(configure);

        // Configure ADO tenancy options
        var tenancyOptions = new ADOTenancyOptions();
    ...
    /// <summary>
    /// Registers a tenant-aware functional repository for an entity type using ADO.NET Oracle.
    /// </summary>
    ...
    /// <item><see cref="ADOTenancyOptions"/> to be registered (via <see cref="AddEncinaADOOracleWithTenancy"/>)</item>
    /// </list>
    /// </para>
    /// </remarks>
    /// <example>
        ...
        where TId : notnull
    {
        ...
        services.AddScoped<IFunctionalRepository<TEntity, TId>>(sp =>
        {
            ...
            var options = sp.GetRequiredService<IOptions<ADOTenancyOptions>>().Value;
            var entityMapping = sp.GetRequiredService<ITenantEntityMapping<TEntity, TId>>();

            return new TenantAwareFunctionalRepositoryADO<TEntity, TId>(
                connection,
                entityMapping,
                tenantProvider,
                options);
    ...
    /// <summary>
    /// Registers a read-only tenant-aware repository for an entity type using ADO.NET Oracle.
    /// </summary>
    /// <typeparam name="TEntity">The entity type.</typeparam>
    /// <typeparam name="TId">The entity identifier type.</typeparam>
    /// <param name="services">The service collection.</param>
