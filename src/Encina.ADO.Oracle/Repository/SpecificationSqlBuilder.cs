...
/// <summary>
/// Translates <see cref="Specification{T}"/> expressions to SQL clauses with ADO.NET parameters for Oracle.
/// </summary>
/// <typeparam name="TEntity">The entity type.</typeparam>
/// <remarks>
/// <para>
/// This class provides specification-to-SQL translation for common predicates.
/// It generates parameterized SQL using <see cref="IDbCommand.Parameters"/> to prevent SQL injection.
/// </para>
/// <para>
/// <b>Oracle-specific syntax:</b>
/// <list type="bullet">
/// <item><description>Uses double-quotes for identifiers: "ColumnName" (Oracle is case-sensitive with quotes)</description></item>
/// <item><description>Pagination uses OFFSET/FETCH (Oracle 12c+ syntax)</description></item>
/// <item><description>Parameters use colon prefix: :paramName</description></item>
/// <item><description>Supports schema-qualified names</description></item>
/// </list>
/// </para>
/// <para>
...
/// var (whereClause, parameterAction) = builder.BuildWhereClause(spec);
/// // whereClause: "WHERE "Status" = :p0"
/// // parameterAction adds :p0 = "Active" to the command
/// </code>
/// </example>
    ...
    where TEntity : class
{
    ...
    /// <summary>
    /// Builds a WHERE clause from a specification with parameter binding action.
    /// </summary>
        ...
        Specification<TEntity> specification)
    {
        ...
        return (whereClause, command => AddParametersToCommand(command, parameters));
    }

    /// <summary>
    /// Builds a WHERE clause from a query specification with parameter binding action.
    /// </summary>
    ...
    /// </summary>
    internal (string WhereClause, Action<IDbCommand> AddParameters) BuildWhereClauseInternal(
    ...
    /// <summary>
    /// Builds pagination SQL (OFFSET/FETCH for Oracle 12c+).
    /// </summary>
    /// <param name="specification">The query specification to translate.</param>
    /// <returns>The pagination clause.</returns>
    /// <remarks>
    /// Oracle 12c+ uses OFFSET/FETCH syntax for pagination.
    /// Note: ORDER BY is required when using OFFSET/FETCH in Oracle.
    /// </remarks>
    public string BuildPaginationClause(IQuerySpecification<TEntity> specification)
    {
        ...
        if (specification.KeysetPaginationEnabled)
        {
            ...
            return specification.Take.HasValue
                ? $"FETCH FIRST {specification.Take.Value} ROWS ONLY"
                : string.Empty;
        }
        ...
        if (take.HasValue)
        {
            return $"FETCH FIRST {take.Value} ROWS ONLY";
        }
    ...
    /// <returns>A tuple containing the SQL statement and empty parameter action.</returns>
    public (string Sql, Action<IDbCommand> AddParameters) BuildSelectStatement(string tableName)
    {
        ...
        return ($"SELECT {columns} FROM \"{validatedTableName}\"", _ => { });
    }
    ...
    /// </summary>
    internal (string Sql, Action<IDbCommand> AddParameters) BuildSelectStatementInternal(
        string tableName,
        IQuerySpecification<TEntity> specification)
    {
        ArgumentNullException.ThrowIfNull(specification);
        ...
        var (whereClause, addParameters) = BuildWhereClauseInternal(specification);
    ...
    private string TranslateExpression(Expression expression, List<(string Name, object? Value)> parameters)
    {
    ...
    private string TranslateBinaryExpression(BinaryExpression binary, List<(string Name, object? Value)> parameters)
    {
        // Handle AND/OR
    ...
    private string TranslateUnaryExpression(UnaryExpression unary, List<(string Name, object? Value)> parameters)
    {
    ...
    private string TranslateMethodCallExpression(MethodCallExpression methodCall, List<(string Name, object? Value)> parameters)
    {
    ...
    private static string BuildLikeClause(string columnName, string pattern, string paramName, List<(string Name, object? Value)> parameters)
    {
        parameters.Add((paramName, pattern));
        return $"\"{columnName}\" LIKE {paramName}";
    }

    private static string BuildEqualsClause(string columnName, object? value, string paramName, List<(string Name, object? Value)> parameters)
    {
        if (value is null)
        {
    ...
    private static void AddParametersToCommand(IDbCommand command, List<(string Name, object? Value)> parameters)
    {
