...
| `.claude/hooks/block-prohibited-commands.ps1` | every agent in this folder | the executables `CLAUDE.md` prohibits (`python`, `grep`, `sed`, `awk`, `head`, `tail`, `wc`, `xargs`, `curl`, `cut`, `paste`, `shuf`, `unzip`, `basename`, `xxd`, `dd`, `bash -c`/`sh -c`; in Bash also `find`, `cat`, `ls`, `sort`, `tee`), Bash `for`/`while`/`until`/`if` statements and Bash `$( ... )` substitution; the message names the PowerShell or tool equivalent (#1181) |
