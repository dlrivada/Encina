...
/// <summary>
/// ADO.NET implementation of <see cref="IAuditStore"/> for SQLite.
/// </summary>
/// <remarks>
/// <para>
/// This implementation uses raw SqliteCommand and SqliteDataReader for maximum performance.
/// SQL statements use SQLite-specific syntax:
/// <list type="bullet">
...
public sealed class AuditStoreADO : IAuditStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            AddParameter(command, "@TimestampUtc", entry.TimestampUtc.ToString("o", CultureInfo.InvariantCulture));
            // SQLite stores DateTimeOffset as ISO 8601 TEXT
            AddParameter(command, "@StartedAtUtc", entry.StartedAtUtc.ToString("o", CultureInfo.InvariantCulture));
            AddParameter(command, "@CompletedAtUtc", entry.CompletedAtUtc.ToString("o", CultureInfo.InvariantCulture));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            AddParameter(command, "@FromUtc", fromUtc?.ToString("o", CultureInfo.InvariantCulture));
            AddParameter(command, "@ToUtc", toUtc?.ToString("o", CultureInfo.InvariantCulture));
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // SQLite stores DateTime as TEXT in ISO 8601 format
            AddParameter(command, "@OlderThanUtc", olderThanUtc.ToString("o", CultureInfo.InvariantCulture));

            if (_connection.State != ConnectionState.Open)
                await OpenConnectionAsync(cancellationToken);

            // SQLite returns affected rows from ExecuteNonQuery via sqlite3_changes()
    ...
    private (string whereClause, IDbCommand command) BuildWhereClauseAndCommand(AuditQuery query)
    {
        ...
        if (query.FromUtc.HasValue)
        {
            ...
            AddParameter(command, "@FromUtc", query.FromUtc.Value.ToString("o", CultureInfo.InvariantCulture));
        }

        if (query.ToUtc.HasValue)
        {
            whereClause.Append(@" AND ""TimestampUtc"" <= @ToUtc");
            AddParameter(command, "@ToUtc", query.ToUtc.Value.ToString("o", CultureInfo.InvariantCulture));
        }
    ...
    {
        // SQLite stores GUID as TEXT, parse it back
        ...
        // SQLite stores DateTime as TEXT, parse from ISO 8601
        TimestampUtc = DateTime.Parse(reader.GetString(reader.GetOrdinal("TimestampUtc")), CultureInfo.InvariantCulture),
        // SQLite stores DateTimeOffset as TEXT, parse from ISO 8601
        StartedAtUtc = DateTimeOffset.Parse(reader.GetString(reader.GetOrdinal("StartedAtUtc")), CultureInfo.InvariantCulture),
        CompletedAtUtc = DateTimeOffset.Parse(reader.GetString(reader.GetOrdinal("CompletedAtUtc")), CultureInfo.InvariantCulture),
    ...
    {
        if (command is SqliteCommand sqliteCommand)
            return await sqliteCommand.ExecuteScalarAsync(cancellationToken);
