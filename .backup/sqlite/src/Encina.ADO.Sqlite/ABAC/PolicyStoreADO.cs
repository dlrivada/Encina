...
/// <summary>
/// SQLite ADO.NET implementation of <see cref="IPolicyStore"/> for persistent ABAC policy storage.
/// Uses raw <see cref="SqliteCommand"/> and <see cref="SqliteDataReader"/> for maximum performance.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// Save operations use SQLite <c>INSERT ... ON CONFLICT DO UPDATE</c> for upsert semantics.
/// DateTime values are stored as ISO 8601 text strings, consistent with SQLite conventions.
/// </para>
/// </remarks>
public sealed class PolicyStoreADO : IPolicyStore
{
    ...
    private static void AddPolicySetParameters(IDbCommand command, PolicySetEntity entity)
    {
        ...
        AddParameter(command, "@IsEnabled", entity.IsEnabled ? 1 : 0);
        AddParameter(command, "@Priority", entity.Priority);
        AddParameter(command, "@CreatedAtUtc", entity.CreatedAtUtc.ToString("O"));
        AddParameter(command, "@UpdatedAtUtc", entity.UpdatedAtUtc.ToString("O"));
    }
