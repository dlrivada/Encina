...
/// <summary>
/// SQLite ADO.NET implementation of <see cref="IFunctionalShardedRepository{TEntity, TId}"/>
    ...
    where TId : notnull
{
    ...
    private readonly IShardedConnectionFactory<SqliteConnection> _connectionFactory;
    ...
    public FunctionalShardedRepositoryADO(
        ...
        IShardedConnectionFactory<SqliteConnection> connectionFactory,
        ...
        var scatterResult = await _queryExecutor.ExecuteAllAsync<IReadOnlyList<TEntity>>(
            async (shardId, ct) =>
            {
                ...
                return await connResult.MapAsync(async connection =>
                {
                    ...
                    IReadOnlyList<TEntity> items = await ReadEntitiesAsync(cmd, ct).ConfigureAwait(false);
    ...
    private async Task<List<TEntity>> ReadEntitiesAsync(IDbCommand command, CancellationToken cancellationToken)
    {
        ...
        await using var reader = await ((SqliteCommand)command).ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
