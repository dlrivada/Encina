...
10. **After merge.** The issue closes through `Fixes #N`; `CHANGELOG.md` gets its entry under `[Unreleased]` when the change is user-visible; the tag and the release are cut by the maintainer per milestone block (SPEC-000 DEC-005).
