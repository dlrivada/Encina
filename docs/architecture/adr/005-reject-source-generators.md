...
2. Generate strongly-typed Encina dispatching code
...
public static class GeneratedEncinaRegistration
{
...
{
    public static ValueTask<Either<EncinaError, User>> DispatchGetUserQuery(
        GetUserQueryHandler handler,
        GetUserQuery request,
        CancellationToken ct)
    {
        return new ValueTask<Either<EncinaError, User>>(
...
```
Before (current):
  Edit handler → Save → dotnet run

After (with generators):
  Edit handler → Save → Generator runs → Compilation → Potential errors → dotnet run
```
...
// ❌ Can't work with source generators - assembly unknown at compile time
```
...
```csharp
[Generator]
public class EncinaSourceGenerator : ISourceGenerator
{
...
```csharp
[GenerateEncina]
public partial class MyEncina : IEncina { }
```
...
- ✅ **Simplicity:** No generator code to maintain
- ✅ **Build Time:** Fast, incremental builds
- ✅ **IDE Experience:** No generator lag or intellisense issues
- ✅ **Flexibility:** Supports dynamic assembly loading
- ✅ **Debugging:** Easy to debug expression tree compilation
- ✅ **Stability:** No dependency on Roslyn version
...
1. **Eager Warmup:**
   ...
   await Encina.Send(new WarmupCommand());
   ```
...
cfg.RegisterServicesFromAssemblies(LoadPluginAssembly("Plugin.dll")); // ❌
```
