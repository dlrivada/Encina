...
> **Estimated Scope**: ~2,500-3,500 lines of new production code + ~2,000-3,000 lines of tests; ~8,000+ lines removed from 13-provider stores
...
The `ConsentRequiredPipelineBehavior`, `ConsentOptions`, `ConsentDiagnostics`, `ConsentHealthCheck`, and `[RequireConsent]` attribute are **preserved** (they are not store-dependent). The in-memory implementations are replaced with an in-memory service backed by a `List<object>` event store for testing.
...
<details>
<summary><strong>3. Event Model — 6 immutable domain events covering full lifecycle</strong></summary>

### Options Considered

| Option | Pros | Cons |
|--------|------|------|
| **A) Fine-grained events (6 event types)** | Each event is a single fact, clear lifecycle transitions | More event types to handle |
| **B) Generic `ConsentStateChanged` event** | One event type, simpler projections | Loses semantic meaning, harder to query specific transitions |
| **C) CRUD-style events (Created, Updated, Deleted)** | Familiar pattern | Doesn't capture domain intent (why was consent changed?) |

### Chosen Option: **A — Fine-grained events**

### Events

| Event | Trigger | GDPR Article |
|-------|---------|:------------:|
| `ConsentGranted` | Data subject gives consent | Art. 6(1)(a), 7(1) |
| `ConsentWithdrawn` | Data subject withdraws consent | Art. 7(3) |
| `ConsentExpired` | Consent passes expiration date | Art. 7 (implied) |
| `ConsentRenewed` | Data subject re-confirms consent | Art. 7(1) |
| `ConsentVersionChanged` | Terms updated, reconsent may be required | Art. 7 |
| `ConsentReconsentProvided` | Data subject consents under new terms | Art. 7(1) |

### Rationale

- Each event captures a specific GDPR-relevant fact with precise timestamps and actor context
- Regulators can query specific event types (e.g., "show all withdrawals in Q4")
- Projections are straightforward: each event maps to a clear state transition
- Matches CrossBorderTransfer's fine-grained event model (TIACreated, TIARiskAssessed, etc.)
...
<details>
<summary><strong>5. In-Memory Implementation — InMemoryConsentService for testing</strong></summary>

### Options Considered

| Option | Pros | Cons |
|--------|------|------|
| **A) In-memory service with internal event list** | Fast, no infrastructure, follows pattern | Not a real event store |
| **B) No in-memory — always require Marten** | Simpler codebase | Can't use consent in unit tests without PostgreSQL |
| **C) In-memory Marten session mock** | Realistic | Complex, fragile, mocking an event store is an anti-pattern |

### Chosen Option: **A — In-memory service with internal event list**

### Rationale

- Development and testing MUST work without PostgreSQL
- The current module has `InMemoryConsentStore` — we need an equivalent
- `InMemoryConsentService` stores events in a `ConcurrentDictionary<Guid, List<object>>` and applies them to build state
- Registered by default via `TryAdd` — Marten registration overrides it in production
- Follows the same TryAdd pattern used by CrossBorderTransfer
...
1. **`Events/ConsentEvents.cs`** — All 6 immutable domain event records
   - `ConsentGranted(Guid ConsentId, string DataSubjectId, string Purpose, string ConsentVersionId, string Source, string? IpAddress, string? ProofOfConsent, IReadOnlyDictionary<string, object?> Metadata, DateTimeOffset? ExpiresAtUtc, string GrantedBy, string? TenantId, string? ModuleId, DateTimeOffset OccurredAtUtc)`
   - `ConsentWithdrawn(string WithdrawnBy, string? Reason, DateTimeOffset OccurredAtUtc)`
   - `ConsentExpired(DateTimeOffset OccurredAtUtc)`
   - `ConsentRenewed(string ConsentVersionId, DateTimeOffset? NewExpiresAtUtc, string RenewedBy, string? Source, DateTimeOffset OccurredAtUtc)`
   - `ConsentVersionChanged(string NewVersionId, string Description, bool RequiresReconsent, string ChangedBy, DateTimeOffset OccurredAtUtc)`
   - `ConsentReconsentProvided(string NewConsentVersionId, string Source, string? IpAddress, string? ProofOfConsent, IReadOnlyDictionary<string, object?> Metadata, DateTimeOffset? ExpiresAtUtc, string GrantedBy, DateTimeOffset OccurredAtUtc)`

2. **`Aggregates/ConsentAggregate.cs`** — Event-sourced aggregate
...
TASK:
1. Create Events/ConsentEvents.cs with 6 sealed record event types (ConsentGranted, ConsentWithdrawn, ConsentExpired, ConsentRenewed, ConsentVersionChanged, ConsentReconsentProvided)
2. Create Aggregates/ConsentAggregate.cs extending AggregateBase with full lifecycle support
3. Each event must include TenantId/ModuleId where applicable (for cross-cutting integration)
4. Aggregate MUST validate invariants (e.g., cannot withdraw non-active consent)
...
- Follow exact patterns from src/Encina.Compliance.CrossBorderTransfer/Aggregates/TIAAggregate.cs
...
### Phase 3: Service Interface & Default Implementation

> **Goal**: Define `IConsentService` replacing the 3 old interfaces, and implement `DefaultConsentService` using `IAggregateRepository<ConsentAggregate>`.
...
3. **`InMemoryConsentService.cs`** — In-memory implementation (replaces InMemoryConsentStore + InMemoryConsentAuditStore + InMemoryConsentVersionManager)
   - Thread-safe via `ConcurrentDictionary<Guid, List<object>>` for event streams
   - Applies events to reconstruct `ConsentReadModel` on read
   - No Marten dependency — pure in-memory for dev/test

#### Files to DELETE from `src/Encina.Compliance.Consent/`

4. **DELETE** `Abstractions/IConsentStore.cs` — Replaced by `IConsentService`
5. **DELETE** `Abstractions/IConsentAuditStore.cs` — Inherent in event stream
6. **DELETE** `Abstractions/IConsentVersionManager.cs` — Part of `IConsentService` (version changes are aggregate commands)
7. **DELETE** `InMemoryConsentStore.cs` — Replaced by `InMemoryConsentService`
8. **DELETE** `InMemoryConsentAuditStore.cs` — Audit is inherent in event stream
9. **DELETE** `InMemoryConsentVersionManager.cs` — Part of `InMemoryConsentService`
...
- InMemoryConsentService provides dev/test implementation without Marten
...
3. Create InMemoryConsentService.cs (thread-safe, ConcurrentDictionary-based)
4. DELETE old interfaces: IConsentStore.cs, IConsentAuditStore.cs, IConsentVersionManager.cs
5. DELETE old in-memory implementations: InMemoryConsentStore.cs, InMemoryConsentAuditStore.cs, InMemoryConsentVersionManager.cs
...
- InMemoryConsentService must be thread-safe (ConcurrentDictionary)
- Use TryAdd in DI — InMemoryConsentService is the default, Marten overrides
...
2. **`ServiceCollectionExtensions.cs`** — Update DI registration
   - Replace `IConsentStore → InMemoryConsentStore` with `IConsentService → InMemoryConsentService`
...
CONTEXT:
- Phases 1-3 completed: ConsentAggregate, events, read model, IConsentService, DefaultConsentService, InMemoryConsentService
...
2. Update ServiceCollectionExtensions.cs — replace old store registrations with IConsentService
...
KEY RULES:
- AddEncinaConsent() registers InMemoryConsentService by default (TryAddScoped)
...
16. `src/Encina.Dapper.Sqlite/Consent/ConsentStoreDapper.cs`
17. `src/Encina.Dapper.Sqlite/Consent/ConsentAuditStoreDapper.cs`
18. `src/Encina.Dapper.Sqlite/Consent/ConsentVersionManagerDapper.cs`
19. `src/Encina.Dapper.SqlServer/Consent/ConsentStoreDapper.cs`
20. `src/Encina.Dapper.SqlServer/Consent/ConsentAuditStoreDapper.cs`
21. `src/Encina.Dapper.SqlServer/Consent/ConsentVersionManagerDapper.cs`
22. `src/Encina.Dapper.PostgreSQL/Consent/ConsentStoreDapper.cs`
23. `src/Encina.Dapper.PostgreSQL/Consent/ConsentAuditStoreDapper.cs`
24. `src/Encina.Dapper.PostgreSQL/Consent/ConsentVersionManagerDapper.cs`
25. `src/Encina.Dapper.MySQL/Consent/ConsentStoreDapper.cs`
26. `src/Encina.Dapper.MySQL/Consent/ConsentAuditStoreDapper.cs`
27. `src/Encina.Dapper.MySQL/Consent/ConsentVersionManagerDapper.cs`

#### DELETE from EF Core (9 files)

28. `src/Encina.EntityFrameworkCore/Consent/ConsentStoreEF.cs`
29. `src/Encina.EntityFrameworkCore/Consent/ConsentAuditStoreEF.cs`
30. `src/Encina.EntityFrameworkCore/Consent/ConsentVersionManagerEF.cs`
31. `src/Encina.EntityFrameworkCore/Consent/ConsentRecordEntity.cs`
32. `src/Encina.EntityFrameworkCore/Consent/ConsentAuditEntryEntity.cs`
33. `src/Encina.EntityFrameworkCore/Consent/ConsentVersionEntity.cs`
34. `src/Encina.EntityFrameworkCore/Consent/ConsentRecordEntityConfiguration.cs`
35. `src/Encina.EntityFrameworkCore/Consent/ConsentAuditEntryEntityConfiguration.cs`
36. `src/Encina.EntityFrameworkCore/Consent/ConsentVersionEntityConfiguration.cs`
37. `src/Encina.EntityFrameworkCore/Consent/ConsentModelBuilderExtensions.cs`

#### DELETE from MongoDB (6 files)

38. `src/Encina.MongoDB/Consent/ConsentStoreMongoDB.cs`
39. `src/Encina.MongoDB/Consent/ConsentAuditStoreMongoDB.cs`
40. `src/Encina.MongoDB/Consent/ConsentVersionManagerMongoDB.cs`
41. `src/Encina.MongoDB/Consent/ConsentRecordDocument.cs`
42. `src/Encina.MongoDB/Consent/ConsentAuditEntryDocument.cs`
43. `src/Encina.MongoDB/Consent/ConsentVersionDocument.cs`

#### MODIFY ServiceCollectionExtensions in each provider (13 files)

44. Remove `if (config.UseConsent) { ... }` blocks from:
...
45. Remove `UseConsent` property from provider config options if present
...
5. **NEW** `Services/InMemoryConsentServiceTests.cs`
   - Test full lifecycle without Marten
   - Test thread safety (concurrent grant/withdraw)
   - Test query methods return correct state
   - ~15-20 tests

6. **UPDATE** `ConsentRequiredPipelineBehaviorTests.cs`
   - Change mocked dependency from `IConsentStore` to `IConsentService`
   - All test logic remains the same

7. **DELETE** old store tests:
...
8. **KEEP** (no changes needed):
   ...
   - `Events/ConsentGrantedEventTests.cs`, etc. (existing domain events, not ES events)
   - `ConsentErrorsTests.cs`

#### Guard Tests — `tests/Encina.GuardTests/Compliance/Consent/`

9. **NEW** `ConsentAggregateGuardTests.cs` — null checks on factory/command parameters
10. **NEW** `DefaultConsentServiceGuardTests.cs` — null checks on service method parameters
11. **NEW** `InMemoryConsentServiceGuardTests.cs` — null checks on in-memory service
12. **UPDATE** `ConsentRequiredPipelineBehaviorGuardTests.cs` — update constructor dependencies
13. **DELETE** `InMemoryConsentStoreGuardTests.cs`, `InMemoryConsentVersionManagerGuardTests.cs`

#### Property Tests — `tests/Encina.PropertyTests/Compliance/Consent/`

14. **NEW** `ConsentAggregatePropertyTests.cs`
...
15. **UPDATE** `IConsentStoreContractTests.cs` → rename to `IConsentServiceContractTests.cs`
    - Test InMemoryConsentService and DefaultConsentService follow same contract

#### Integration Tests — `tests/Encina.IntegrationTests/`

16. **DELETE** all 13-provider consent integration tests:
...
17. **NEW** `Compliance/Consent/ConsentMartenIntegrationTests.cs`
...
18. **UPDATE** `Compliance/Consent/ConsentPipelineIntegrationTests.cs`
    - Update to use `IConsentService` instead of `IConsentStore`

#### Load/Benchmark Tests

19. **UPDATE** `tests/Encina.LoadTests/Compliance/Consent/ConsentValidationLoadTests.cs` — update dependencies
20. **UPDATE** `tests/Encina.BenchmarkTests/Encina.Benchmarks/Compliance/Consent/` — update dependencies
...
TASK:
1. Create unit tests: ConsentAggregateTests, ConsentEventsTests, ConsentProjectionTests, DefaultConsentServiceTests, InMemoryConsentServiceTests
...
4. **`src/Encina.Compliance.Consent/README.md`** — Update
   ...
   - Document PostgreSQL requirement for production
...
- README should show both dev (InMemory) and production (Marten) registration
...
| Service Implementations | 2 | 0 | 3 | 5 |
...
- Create Events/ConsentEvents.cs with 6 sealed record events (ConsentGranted, ConsentWithdrawn, ConsentExpired, ConsentRenewed, ConsentVersionChanged, ConsentReconsentProvided)
...
Phase 3 — Service Interface & Implementations:
...
- Create InMemoryConsentService.cs (dev/test, no Marten)
- DELETE old interfaces and in-memory implementations

Phase 4 — Configuration & DI:
- Update ServiceCollectionExtensions.cs (IConsentService → InMemoryConsentService by default)
...
Phase 7 — Testing:
- NEW: ConsentAggregateTests, ConsentEventsTests, ConsentProjectionTests, DefaultConsentServiceTests, InMemoryConsentServiceTests
- UPDATE: Pipeline behavior tests, guard tests, contract tests
- DELETE: Old store tests, 13-provider integration tests
- NEW: Marten integration test
...
- InMemoryConsentService registered by default; AddConsentAggregates() overrides for Marten
