...
The following APIs were validated during Phase 2 and Phase 3:
...
| Encina.Testing.Bogus | `OutboxMessageFaker`, `InboxMessageFaker` | ✅ Working |
...
| Encina.Testing.FsCheck | `PropertyTestBase`, `[EncinaProperty]` | ✅ Working |
| Encina.Testing.FsCheck | `[QuickProperty]`, `[ThoroughProperty]` | ✅ Working |
| Encina.Testing.FsCheck | `EncinaArbitraries.*`, auto-discovery | ✅ Working |
| Encina.Testing.FsCheck | `EncinaProperties.*`, pre-built validators | ✅ Working |
| Encina.Testing.FsCheck | `GenExtensions.*`, generator helpers | ✅ Working |
| Encina.Testing.Pact | `EncinaPactConsumerBuilder` | ✅ Working |
| Encina.Testing.Pact | `WithQueryExpectation()`, `WithNotificationExpectation()` | ✅ Working |
| Encina.Testing.Pact | `PactExtensions.CreatePactHttpClient()` | ✅ Working |
| Encina.Testing.Fakes | `FakeOutboxStore`, `FakeInboxStore` | ✅ Working |
| Encina.Testing.Fakes | `WasMessageAdded<T>()`, `AddedMessages` | ✅ Working |
| Encina.Testing.Messaging | `OutboxTestHelper`, `InboxTestHelper` | ✅ Working |

### 13.6 Phase 3 Additional Findings

During Phase 3 (Migration Enablement), additional API details were discovered:

#### 13.6.1 FsCheck Integration

**Required imports for property tests**:
```csharp
...
**Key pattern**: `.ToProperty()` extension requires `FsCheck.Fluent` namespace.

**Messaging type namespaces**:
- `Encina.Messaging.Outbox` for `IOutboxMessage`
- `Encina.Messaging.Inbox` for `IInboxMessage`
- `Encina.Messaging.Sagas` (with 's') for `ISagaState`
- `Encina.Messaging.Scheduling` for `IScheduledMessage`

#### 13.6.2 Type Ambiguity Resolution

`FakeOutboxMessage` exists in two packages:
- `Encina.Testing.Bogus.FakeOutboxMessage` (generated by faker)
- `Encina.Testing.Fakes.Models.FakeOutboxMessage` (store model)

**Solution**: Use type aliases when both are needed:
```csharp
using FakeOutboxMsg = Encina.Testing.Fakes.Models.FakeOutboxMessage;
```

#### 13.6.3 Pact API Notes

- `EncinaPactConsumerBuilder` requires `ITestOutputHelper` for output
- HTTP paths follow convention: `/api/queries/{QueryTypeName}`, `/api/commands/{CommandTypeName}`
- Error code prefixes map to HTTP status codes:
  - `encina.notfound.*` → 404
  - `encina.validation.*` → 400

### 13.7 Updated Reference Project Structure

```
Encina.Testing.Examples/
├── Domain/                           # Sample domain types
│   ├── CreateOrderCommand.cs         # ICommand<Guid>
│   ├── GetOrderQuery.cs              # IQuery<OrderDto>
│   ├── OrderCreatedEvent.cs          # INotification
...
│   └── EitherAssertionExamples.cs    # Shouldly Either extensions
├── Integration/                      # ModuleTestFixture patterns
│   └── ModuleIntegrationExamples.cs
├── Fixtures/                         # EncinaWireMockFixture examples
│   └── WireMockFixtureExamples.cs
├── TestData/                         # Test data generation
│   ├── BogusExamples.cs              # EncinaFaker patterns
│   └── MessagingFakerExamples.cs     # FakeOutboxStore, OutboxTestHelper
├── PropertyBased/                    # Property-based testing
│   └── PropertyTestExamples.cs       # FsCheck with EncinaProperty
├── ContractTests/                    # Consumer-driven contracts
│   └── PactConsumerExamples.cs       # EncinaPactConsumerBuilder
