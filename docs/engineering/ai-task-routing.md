...
- **Contention on the one llama-server slot (#1345):** `llama-server` serves one request at a time (`-np 1`), and the SPEC-003 audit pipeline's pre-draft queue (`tools/ai/audit/qwen-predraft.ps1`) shares that slot with a worker's own local-model call. `tools/ai/local-ai-ask.cs` retries its health check for about two minutes (six attempts, 20 s apart) before reporting the server down, so a busy server is not mistaken for a dead one from a single failed probe. When the orchestrator needs the model for a worker right away, it creates `artifacts/knowledge/predraft/PAUSE` (any content) under the main root; the queue loop (not `-Issue` single-issue mode, which `audit-next.ps1` needs to complete immediately) waits, rechecking every 30 s, until that file is removed.
