...
| `audit-draft-remediation.ps1` | The remediation stage: splits each finding out individually, classifies and deduplicates it against open issues with the free local model, then drafts the non-duplicates into the matching issue template (`bug_report.md`/`[BUG]`, `test_implementation.md`/`[TEST]`, `technical_debt.md`/`[DEBT]`) | the `## Findings` sections of `code.md`, `tests.md`, `docs.md` | `artifacts/knowledge/remediation/<n>-<stage>-<id>-<slug>.md`, `stages/remediation.md` |
