...
- Structured logging: every `[LoggerMessage]` event id belongs to a range registered in one file, and architecture tests assert global uniqueness, range membership and non-overlap ([`CLAUDE.md`](../../CLAUDE.md), "Structured Logging & EventId Allocation").
