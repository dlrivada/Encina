...
**Status:** design note, awaiting a maintainer decision. `crap-gate.cs` exists and is tested; no workflow calls it. `.github/workflows/ci.yml` is out of scope for the worktree that wrote this note — wiring the gate in is a follow-up change to that file.
**Issue:** #1346
