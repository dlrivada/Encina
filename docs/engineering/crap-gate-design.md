...
**Tradeoff:** no new test execution — the only added CI cost is `crap-gate.cs` parsing the downloaded Cobertura files and the diff, which has not been timed against a real run (see §5). The gap: a PR that changes only docs, `.github/`, or an area none of the `paths-filter` entries cover skips every test job, so there is no coverage data for the gate to read. `crap-gate.cs` itself does not degrade gracefully in that case — given zero Cobertura files it prints an error and exits `2` (the `coberturaFiles.Count == 0` check near the top of the script). The wired job never lets that happen, through two guards outside the script: the `crap-gate` job's own `if:` condition requires `github.event_name == 'pull_request' && needs.changes.outputs.src == 'true'`, so a docs-only or `.github`-only PR never runs the job at all; and, within a run, the "Run CRAP gate" step globs for `coverage.cobertura.xml` among the downloaded artifacts before invoking the script, and exits `0` with a message ("No Cobertura coverage files found among the downloaded artifacts; nothing to gate.") if none are found. The "nothing to check" behavior is correct, but it is the job's wiring that guarantees it, not the script's own handling of empty input.
