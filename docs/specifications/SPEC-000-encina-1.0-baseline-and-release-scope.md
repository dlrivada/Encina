...
- **REQ-020** The SDK version is pinned (`global.json`) and `main` is protected with `enforce_admins`, one approving review, linear history and the required checks `build`, `Check formatting`, every `test-*` shard of `ci.yml` and CodeQL `Analyze` (DEC-006).
...
| AC-020 | REQ-020 | `global.json` present; `GET /repos/dlrivada/Encina/branches/main/protection` output in the evidence report shows `enforce_admins: true` and the required-check list of REQ-020; #98 closed. |
