...
| Post-merge review (#1119, adversarial reviewer) | Eight verified findings, fixed in the follow-up PR: (1) the gate now fails on any covref-looking token that is not a well-formed block and checks prose on marker lines; (2) a block cannot contain another opener, so an unclosed marker no longer swallows text (INV-001; same fix in `mut-docs-render.cs`); (3) "empty index" means "no entry with data" in the publish guard and the CI Full check, because REQ-003 makes the index never literally empty; (4) CommonMark fence closing (same character, at least as long, no info string), also in `mut-docs-render.cs`; (5) inline fields are the REQ-002 scalars plus one per flag, so `metObligations`, `noData`, `package`, `path` and `dashboardUrl` are citable; (6) inline code spans are literal for markers, cited-by and the gate; (7) file-level `coverage` is `null` when no flag has data; (8) the dashboard escapes `cited-by.json` values, ignores a malformed hash and clears the previous highlight. Root `.md` files are now scanned too, and the render step is `continue-on-error` (INV-002). | REQ-005 says the gate fails any citation of a file or field that does not exist; the first implementation had false negatives. |
