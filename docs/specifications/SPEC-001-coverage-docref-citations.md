...
- **REQ-002 (index)** Every CI Full run emits `docref-index.json` for coverage with one entry per source file that has at least one applicable flag in its manifest, carrying `package, path, coverage, obligations, metObligations, flags, perFlag{flag: total, covered, coverage, target}, lastRun, dashboardUrl`.
- **REQ-003 (stable IDs)** The set of IDs is derived from the coverage manifests, not from which files happened to produce Cobertura data in the run: a file with applicable flags but no data is present with `noData: true`. An ID disappears only when the file is removed or its manifest entry has no applicable flags.
...
- **REQ-005 (dangling gate)** A pull request that cites an ID or field that does not exist in the live index fails a required check. Because CI Full is not triggered by pull requests, the check runs in `ci.yml` against the index published on Pages.
...
| AC-003 | REQ-003 | Removing a Cobertura XML for one test type from the input and re-running produces the same ID set, with the affected files marked `noData: true`. |
| AC-004 | REQ-004 | A `publish-coverage` run leaves `_site/coverage/data/docref-index.json` and `cited-by.json` on Pages, and the rendered `.md` diff appears in the persist-back commit (or the warning about branch protection, as for mutations). |
| AC-005 | REQ-005 | A test PR citing `cov:Encina/DoesNotExist.cs` fails the `ci.yml` check; the same PR with a valid ID passes. |
...
| Where the dangling gate runs | `ci.yml` against the Pages index | CI Full only (not PR-triggered, would not protect PRs) |
