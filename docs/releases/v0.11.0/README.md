...
### January 30 - Immutable Records Support for IUnitOfWork and IFunctionalRepository (#572)

**Issue**: [#572 - Immutable Records Support for IUnitOfWork and IFunctionalRepository](https://github.com/dlrivada/Encina/issues/572)
**Milestone**: v0.12.0 - Database & Repository

#### Overview

Extended immutable record support from DbContext extension methods to `IUnitOfWork` and `IFunctionalRepository` interfaces. Also implemented `ImmutableAggregateHelper` for non-EF Core providers (Dapper, ADO.NET, MongoDB) that lack change tracking.

#### Phase 1: Interface Changes

**IUnitOfWork Updates** (`src/Encina.DomainModeling/IUnitOfWork.cs`):
...
**IFunctionalRepository Updates** (`src/Encina.DomainModeling/IFunctionalRepository.cs`):
...
**RepositoryErrors New Error Type** (`src/Encina.DomainModeling/RepositoryErrors.cs`):

```csharp
public static EncinaError OperationNotSupported(string operationName) => new(
    Code: "REPO_OPERATION_NOT_SUPPORTED",
    Message: $"Operation '{operationName}' is not supported by this repository implementation",
    Metadata: new Dictionary<string, object> { ["OperationName"] = operationName });
```

#### Phase 2: EF Core Implementation

**UnitOfWorkEF** (`src/Encina.EntityFrameworkCore/Infrastructure/UnitOfWork/UnitOfWorkEF.cs`):

- Uses `DbContext.UpdateImmutable()` internally
- Validates entity is tracked before update
- Returns ROP `Either<EncinaError, Unit>` on all paths

**FunctionalRepositoryEF** (`src/Encina.EntityFrameworkCore/Infrastructure/Repository/FunctionalRepositoryEF.cs`):

- Delegates to `DbContext.UpdateImmutable()`
- Consistent with UnitOfWork pattern

**UnitOfWorkRepository** (internal):

- Added `UpdateImmutableAsync` delegating to parent UnitOfWork

#### Phase 3: Non-EF Core Implementations

**ImmutableAggregateHelper** (`src/Encina.DomainModeling/ImmutableAggregateHelper.cs`):

Utility class for non-EF Core providers:
...
**Dapper/ADO.NET/MongoDB Implementations**:

All return `RepositoryErrors.OperationNotSupported("UpdateImmutableAsync")`:

- `FunctionalRepositoryDapper.UpdateImmutableAsync()` - All 4 providers
- `FunctionalRepositoryADO.UpdateImmutableAsync()` - All 4 providers
- `FunctionalRepositoryMongoDB.UpdateImmutableAsync()`

**TenantAwareFunctionalRepository Updates**:

Updated tenant-aware repositories for all providers with the same pattern.

#### Phase 4: Testing

**Unit Tests** (`tests/Encina.UnitTests/`):

| Test File | Tests | Description |
|-----------|-------|-------------|
| `DomainModeling/ImmutableAggregateHelperTests.cs` | 16 | Full behavior coverage |
| `DomainModeling/OperationNotSupportedTests.cs` | 7 | Error factory tests |
| `EntityFrameworkCore/UnitOfWork/UnitOfWorkEFUpdateImmutableTests.cs` | 20+ | EF Core integration |

**Guard Tests** (`tests/Encina.GuardTests/`):

| Test File | Tests | Description |
|-----------|-------|-------------|
| `DomainModeling/ImmutableAggregateHelperGuardTests.cs` | 3 | Null parameter validation |
| `EntityFrameworkCore/UnitOfWork/UnitOfWorkEFGuardTests.cs` | 4 | UpdateImmutable null checks |
| `EntityFrameworkCore/Repository/FunctionalRepositoryEFGuardTests.cs` | 3 | UpdateImmutableAsync guards |

**Contract Tests** (`tests/Encina.ContractTests/`):

| Test File | Tests | Description |
|-----------|-------|-------------|
| `Database/Repository/UpdateImmutableAsyncContractTests.cs` | 14 | API consistency across providers |

**Property Tests** (`tests/Encina.PropertyTests/`):

| Test File | Tests | Description |
|-----------|-------|-------------|
| `DomainModeling/ImmutableAggregateHelperPropertyTests.cs` | 12 | Invariants (event copying, tracking) |
| `Database/UnitOfWork/UnitOfWorkPropertyTests.cs` | 4 | UpdateImmutable invariants |

**Integration Tests** (`tests/Encina.IntegrationTests/`):

| Test File | Tests | Description |
|-----------|-------|-------------|
| `Infrastructure/EntityFrameworkCore/UnitOfWork/UnitOfWorkEFIntegrationTests.cs` | 4 | SQLite integration |

**Total Tests**: 108 tests passing

#### Key Design Decisions

1. **ROP Consistency**: All methods return `Either<EncinaError, Unit>` for consistent error handling.

2. **Provider Strategy**:
   - EF Core: Full support via change tracker manipulation
   - Non-EF Core: Returns `OperationNotSupported`, users use `ImmutableAggregateHelper`

3. **CopyEventsFrom Behavior**: Events are APPENDED to the modified aggregate's existing events (not cleared first). This matches the behavior documented in the original `IAggregateRoot.CopyEventsFrom()` method.

4. **Same-Instance Edge Case**: Calling `PrepareForUpdate(aggregate, aggregate, collector)` throws `InvalidOperationException` because `CopyEventsFrom` modifies the collection while iterating over it.

5. **Contract Tests Via Interface**: Internal `UnitOfWorkRepository` classes are tested via the public `IUnitOfWork` interface, not directly.

#### Files Modified

| File | Changes |
|------|---------|
| `src/Encina.DomainModeling/IUnitOfWork.cs` | Added UpdateImmutable/Async methods |
| `src/Encina.DomainModeling/IFunctionalRepository.cs` | Added UpdateImmutableAsync method |
| `src/Encina.DomainModeling/RepositoryErrors.cs` | Added OperationNotSupported factory |
| `src/Encina.DomainModeling/ImmutableAggregateHelper.cs` | **New file** |
| `src/Encina.EntityFrameworkCore/.../UnitOfWorkEF.cs` | Implemented UpdateImmutable |
| `src/Encina.EntityFrameworkCore/.../FunctionalRepositoryEF.cs` | Implemented UpdateImmutableAsync |
| All Dapper providers (4) | UpdateImmutableAsync → OperationNotSupported |
| All ADO.NET providers (4) | UpdateImmutableAsync → OperationNotSupported |
| MongoDB provider | UpdateImmutableAsync → OperationNotSupported |
| All TenantAware repositories (13) | UpdateImmutableAsync support |

#### Documentation Updated

- `docs/features/immutable-domain-models.md` - Added IUnitOfWork, IFunctionalRepository, and non-EF sections
- `README.md` - Updated example with IUnitOfWork pattern
- `CHANGELOG.md` - Added #572 entry

**Related Issue**: [#572](https://github.com/dlrivada/Encina/issues/572)

---

### January 30 - Audit Trail Pattern (#286)
...
Created fine-grained interfaces in `Encina.DomainModeling` for selective audit tracking:
...
| `IAuditableEntity` | All above | - | Composite for EF Core interceptor |
| `IAuditable` | All above (read-only) | - | For immutable records |
| `ISoftDeletable` | `IsDeleted`, `DeletedAtUtc`, `DeletedBy` | - | Soft delete tracking |

Files Modified:

- `src/Encina.DomainModeling/IAuditable.cs` - All interfaces defined

#### Phase 2: EF Core Audit Interceptor

Implemented `AuditInterceptor` as a `SaveChangesInterceptor` that automatically populates audit fields:

| Component | Description |
|-----------|-------------|
| `AuditInterceptor` | SaveChangesInterceptor with SavingChanges/SavedChanges hooks |
| `AuditInterceptorOptions` | Configuration (TrackCreatedAt, TrackCreatedBy, etc.) |
| `ServiceCollectionExtensions` | `UseAuditing` configuration option |

Features:

- Detects entities implementing granular interfaces
- Gets user ID from `IRequestContext.UserId`
- Uses injected `TimeProvider` for timestamps
- Supports optional `IAuditLogStore` for detailed audit logging
- Async-local pending entries for transactional consistency

Files Created/Modified:

- `src/Encina.EntityFrameworkCore/Auditing/AuditInterceptor.cs`
- `src/Encina.EntityFrameworkCore/Auditing/AuditInterceptorOptions.cs`
- `src/Encina.EntityFrameworkCore/ServiceCollectionExtensions.cs`

#### Phase 3: Non-EF Core Provider Support

Created `AuditFieldPopulator` and extension methods for manual audit field population:
...
- `src/Encina.DomainModeling/AuditFieldPopulator.cs`
- `src/Encina.ADO.Sqlite/Extensions/AuditExtensions.cs`
- `src/Encina.ADO.SqlServer/Extensions/AuditExtensions.cs`
- `src/Encina.ADO.PostgreSQL/Extensions/AuditExtensions.cs`
- `src/Encina.ADO.MySQL/Extensions/AuditExtensions.cs`
- `src/Encina.Dapper.Sqlite/Extensions/AuditExtensions.cs`
- `src/Encina.Dapper.SqlServer/Extensions/AuditExtensions.cs`
- `src/Encina.Dapper.PostgreSQL/Extensions/AuditExtensions.cs`
- `src/Encina.Dapper.MySQL/Extensions/AuditExtensions.cs`
- `src/Encina.MongoDB/Extensions/AuditExtensions.cs`

#### Phase 4: Optional Audit Log Store

Implemented detailed audit logging infrastructure:

| Component | Description |
|-----------|-------------|
| `AuditAction` | Enum (Created, Updated, Deleted) |
| `AuditLogEntry` | Immutable record with old/new values |
| `IAuditLogStore` | Interface for audit persistence |
| `InMemoryAuditLogStore` | Thread-safe in-memory implementation for testing |

Files Created:

- `src/Encina.DomainModeling/Auditing/AuditAction.cs`
- `src/Encina.DomainModeling/Auditing/AuditLogEntry.cs`
- `src/Encina.DomainModeling/Auditing/IAuditLogStore.cs`
- `src/Encina.DomainModeling/Auditing/InMemoryAuditLogStore.cs`
...
| Test File | Tests | Description |
|-----------|-------|-------------|
| `DomainModeling/Auditing/AuditInterfaceTests.cs` | 26 | Interface implementation tests |
| `DomainModeling/Auditing/AuditedEntityTests.cs` | - | Base class tests |
| `DomainModeling/Auditing/AuditedAggregateRootTests.cs` | 20 | All aggregate base classes |
| `DomainModeling/Auditing/AuditFieldPopulatorTests.cs` | 20 | Static helper tests |
| `DomainModeling/Auditing/AuditLogEntryTests.cs` | 12 | Record and enum tests |
| `DomainModeling/Auditing/InMemoryAuditLogStoreTests.cs` | 15 | Thread-safe store tests |
| `EntityFrameworkCore/Auditing/AuditInterceptorTests.cs` | 30 | Interceptor with InMemory DB |
| `EntityFrameworkCore/Auditing/AuditInterceptorOptionsTests.cs` | 2 | Options configuration |

**Guard Tests** (`tests/Encina.GuardTests/`):

| Test File | Tests | Description |
|-----------|-------|-------------|
| `DomainModeling/Auditing/AuditFieldPopulatorGuardTests.cs` | 10 | Null parameter validation |
| `DomainModeling/Auditing/InMemoryAuditLogStoreGuardTests.cs` | 3 | Null parameter validation |
| `Infrastructure/EntityFrameworkCore/AuditInterceptorGuardTests.cs` | 6 | Constructor null checks |

**Total Tests**: 154 tests passing

#### Key Design Decisions

1. **Granular Interfaces**: Users can implement only the interfaces they need (e.g., only `ICreatedAtUtc` without user tracking).

2. **Interceptor + Helpers Duality**: EF Core uses automatic interceptor; Dapper/ADO/MongoDB use explicit helpers for consistent API.

3. **TimeProvider Injection**: All timestamps use `TimeProvider` for testability with `FakeTimeProvider`.

4. **Async-Local Pattern**: Pending audit entries use `AsyncLocal<T>` to ensure thread isolation during concurrent saves.

5. **Soft Delete as Update**: When `IsDeleted` changes to `true`, the interceptor detects this as a soft delete operation.

#### Documentation

- Created `docs/features/audit-tracking.md` - Complete guide with examples
- Updated `CHANGELOG.md` - Added #286 entry
- Updated `ROADMAP.md` - Marked #286 as completed in v0.12.0

**Related Issue**: [#286](https://github.com/dlrivada/Encina/issues/286)
