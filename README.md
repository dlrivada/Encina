...
- **Database Sharding**: Horizontal partitioning with 4 routing strategies (Hash, Range, Directory, Geo), compound shard keys for multi-field routing, entity co-location for optimized local JOINs, time-based sharding with automatic tier lifecycle (Hot/Warm/Cold/Archived), scatter-gather queries, and MongoDB dual-mode across all 13 providers (see [docs](docs/sharding/configuration.md), [compound keys](docs/features/compound-shard-keys.md), [co-location](docs/features/sharding-colocation.md), [time-based](docs/features/time-based-sharding.md))
