...
- **CRAP ≤ 10 on changed methods (MUST).** A PR that touches `src/` fails the blocking `crap-gate` job (in `ci-result`) if any method it adds or changes has `CRAP = comp² × (1 − cov)³ + comp` above 10 (comp = cyclomatic complexity; cov = line coverage combined across all flags). Reduce the complexity or add tests; methods the diff does not touch only count as backlog. The one exemption is a single `switch` or pattern match that answers one question, marked on the line above the declaration with `// crap-exempt: single-question switch — <reason>`. Known attribution limits: #1355, #1360. Details: [coverage methodology](docs/testing/coverage-measurement-methodology.md), [gate design](docs/engineering/crap-gate-design.md). (why: #1346)
