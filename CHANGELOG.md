...
- **Marten inline projections never ran, so no read model was ever populated (#1095)**. `IInlineProjectionDispatcher` had no caller: the aggregate repositories appended events and returned without projecting them, `ProjectionRegistry` was registered empty (the `IProjectionRegistrar` instances were never applied to it), and the dispatcher's reflection looked `LoadAsync`/`Store`/`Delete` up on `IDocumentSession` itself, where they do not live. `MartenAggregateRepository` and `SnapshotAwareAggregateRepository` now hand every persisted event to the dispatcher after the commit (`InlineProjectionRelay`, which re-reads the appended envelopes so the `ProjectionContext` carries the persisted version, global sequence, timestamp and correlation data exactly as a rebuild does; `ProjectionOptions.ThrowOnProjectionError` decides whether a projection failure surfaces as the save's `Left`, and an exception escaping the dispatcher is reported as `PROJECTION_APPLY_FAILED`, never as a failed save), `AddProjection<TProjection, TReadModel>` registers the projection infrastructure and the dispatcher on its own, and the registry is built from the registrars on first resolution. `AddProcessorAgreementAggregates` registers `ProcessorProjection` and `DPAProjection`, which it had omitted. The 22 Compliance integration tests that read models back through the services (DPIA, Breach Notification, Cross-Border Transfer, Processor Agreements) pass again; `Options_ConfiguresCorrectly` in the Breach Notification suite now sets alert thresholds consistent with its 48-hour deadline, and `Pipeline_BlockMode_WithValidDPA_AllowsRequest` executes its DPA for the processor id the command declares in `[RequiresProcessor]` (the behavior reads the attribute, not the payload).
- **`LoadAsync` accepted a stream that belongs to another aggregate type.** Marten's `FetchStreamAsync` does not check the stream's aggregate type, and Encina's replay folds unknown events as no-ops, so loading a DPA stream as a `ProcessorAggregate` returned a blank aggregate instead of an error. Both repositories now return `marten.aggregate_not_found` when the replayed aggregate does not carry the requested identifier.
- **`SnapshotAwareAggregateRepository.SaveAsync` passed the pre-append stream version as Marten's expected version**, the same mistake fixed in `MartenAggregateRepository` for #1093; Marten expects the version after the append, so every save to an existing stream would have failed the optimistic-concurrency check. Now passes `aggregate.Version`.
- **Coverage citation gate and renderer hardening** (#1092 follow-up, adversarial review of #1119). The `coverage-citations` gate missed citations with an invalid or missing field, dangling prose on a marker line and markers split over two lines; it now fails on any covref-looking token that is not a well-formed block. A table or inline marker without its closer no longer swallows the hand-written text up to a later closer (same fix in `mut-docs-render.cs`). Fences close per CommonMark (same character, at least as long), so four-backtick fences can quote examples (also in `mut-docs-render.cs`). Markers and IDs inside inline code are literal. All REQ-002 scalar fields are citable. The publish guard and the CI Full check treat an index with no entry carrying data as empty, since manifest-derived IDs make it never literally empty; file-level `coverage` is `null` without data. The coverage dashboard escapes `cited-by.json` values, survives a malformed `#pkg-` hash and clears the previous highlight. Root `.md` files are scanned, and a rendering failure no longer blocks the Pages deploy.
- **`Encina.Marten` EventIds were unregistered and two log classes collided (#1115)**. The package used five unregistered EventId blocks (1-32, 100-153, 3210-3234, 4001-4076), and `VersioningLog` and `SnapshotLog` both used 100-104, 110-113 and 120-122. `EventIdRanges.Marten = (2600, 2799)` is now registered and all 101 EventIds are renumbered sequentially inside it (repositories 2600-2616, versioning 2617-2628, snapshots 2629-2649, event metadata 2650-2663, projections 2664-2700). `MartenEventIdAllocationTests` pins uniqueness, range membership and packing; the solution-wide ADR-021 test is tracked in #1120. Log filters or dashboards keyed on the old Marten EventIds must be updated.
- **ADR-021 is now enforced across the solution, and 754 EventIds in 45 packages were renumbered into registered ranges (#1120, #1050)**. No test had applied `EventIdUniquenessRule` to the Encina assemblies, and the rule ignored duplicates inside one assembly. The new `EncinaEventIdAllocationTests` loads every shipped `Encina*` assembly and fails on any duplicate `[LoggerMessage]` EventId (same or different assembly), any EventId outside the ranges mapped to its assembly, any logging package missing from the map, and any `src/` package with `[LoggerMessage]` methods that the test does not load. It found 45 of 69 logging packages outside the registry: transports, caching, locks, resilience, scheduling adapters, serverless hosts, CDC, the ADO.NET/Dapper/EF Core/MongoDB providers, core mediator and sharding, and parts of Messaging, Security.Audit, Security.Secrets and ABAC. 115 EventIds were each shared by several packages (one id by 33), eight were duplicated only inside one package (seven in `Encina.Cdc`, one in `Encina.Messaging`), and the 13 dead-letter messages of `Encina.Messaging` had no EventId at all, so the generator derived one from a hash of the method name; they now use 2945-2957, and a new `AssertEveryLoggerMessageHasEventId` makes a missing EventId a violation. 44 ranges are registered in `EventIdRanges` (including `OpenTelemetry = (7000, 7099)`, #1050), `SecuritySecrets` moves from 8950-8999 to 5100-5199 because the package's 51 ids did not fit, and every out-of-range EventId is renumbered into its package's range as one contiguous block per source file, after the ids the package already had there; ids that already sat inside a range of their package are unchanged. `AssertEventIdsAreGloballyUnique` now reports duplicates within an assembly, and `AssertEventIdsWithinRegisteredRanges` takes a list of range names per assembly. ADR-021 and CLAUDE.md carry the new range map. EventIds created with `LoggerMessage.Define` are not covered yet (#1125). Log filters or dashboards keyed on the old EventIds must be updated.
